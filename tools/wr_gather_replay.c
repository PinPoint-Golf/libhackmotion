...
/*
 * wr_gather_replay.c — drive the history gather with REAL bytes off a wG3.
 *
 ...
 *
 * What this does: replays a `.wrwire` capture through a real wr_session with
 ...
 * WORK ON A CAPTURE NOBODY CURATED.  `.gitignore` still excludes `*.wrwire`
 ...
 *
 *   wr_gather_replay FILE [--json PATH]
 *
 ...
 * on stdout byte-identical.  tools/wr_replay_py.py drives the same gather
 ...
 *
 * docs/capture-findings.md §4 records what it reported against swings.wrwire;
 ...
 * never fires leaves wr_history_resident_range() answering from
 * WR_HISTORY_DEPTH_SEED_US — §7.3's figure, measured ONCE after 20 s of
 ...
 * (WR_OK measured, WR_PENDING seeded), and nothing in a fake-transport test can
...
/* State                                                                     */
/* ------------------------------------------------------------------------ */
static wr_session *g_session;
static uint64_t    g_pending_id;
static unsigned    g_blocks;
static unsigned    g_status_count[WR_HISTORY_STATUS_COUNT];
...
static wr_status   g_depth_status = WR_ERR_NO_FIT;
static wr_time_us  g_depth_width;
...
static void depth_sample(void)
{
    ...
    st = wr_history_resident_range(g_session, &range);
    if (st != WR_OK && st != WR_PENDING) {
        /*
         * ⚠ NOT AN ANSWER, SO NOT AN OBSERVATION.  WR_ERR_NO_FIT is §6.1.1: the
         * fit re-anchored at the bracket close and has not seen a live frame
         * yet.  WR_ERR_NO_STREAM is a capture that ended with the pull.  Neither
    ...
    g_depth_owed = false;
    g_depth_status = st;
    /* ⚠ ONE OBSERVATION PER PULL, and WR_PENDING is a FAILURE here rather than
     ...
     * never fires. */
    check_note(&g_depth, st == WR_OK);
    if (st == WR_OK) {
        g_depth_measured++;
        g_depth_width = range.end_us - range.start_us;
    } else if (st == WR_PENDING) {
        g_depth_seeded++;
    }
}
...
static void step_note(uint32_t step)
{
     ...
     * WR_WARN_HISTORY_HOLED fires, so it is worth its own verdict. */
...
static void drain(void)
{
    ...
    while (wr_session_poll_writes(g_session, w, 8u) > 0u) {
    }
    while (wr_session_poll_events(g_session, ev, 16u) > 0u) {
    }
    while (wr_session_poll_live(g_session, live, 64u) > 0u) {
    }
}
...
static void collect_if_ready(void)
{
    ...
    if (wr_history_collect(g_session, g_pending_id, &b) != WR_OK || b == NULL) {
        return;
    }
    g_pending_id = 0u;
    g_blocks++;
    g_depth_owed = true;
    if (b->status < (uint8_t)WR_HISTORY_STATUS_COUNT) {
    ...
    printf("  block   %-9s n=%-5zu coverage=%.3f density=%.3f (step %s) "
           ...
           wr_history_status_name((wr_history_status)b->status), b->sample_count,
    ...
    if (g_json != NULL) {
        fprintf(g_json,
                ...
                g_json_first ? "" : ",",
                (unsigned)b->status,
                wr_history_status_name((wr_history_status)b->status),
    ...
    for (size_t i = 0; i < b->sample_count; ++i) {
        ...
        check_note(&g_dated, sm->host_time_us == wr_clock_to_host_us(&b->fit, sm->sample_index));
        /* Bulk arrival timestamps carry no information at all (§10.1). */
        check_note(&g_history_src, sm->source == (uint8_t)WR_SOURCE_HISTORY &&
                                       sm->host_recv_us == WR_TIME_UNKNOWN);
        if (i > 0u) {
 ...
 * at-or-below deliberately: `swings.wrwire`'s sixth pull asked 200 indices PAST
 ...
 * why `swings.wrwire` never needed it; `session1.wrwire`'s pull sits at 173 s,
...
static void on_recorded_a1(const wr_wire_chunk *c, unsigned *reserved, unsigned *refused)
{
    ...
    st = wr_session_clock(g_session, &snap);
    if (st != WR_OK) {
        /* Worth printing rather than skipping: it is how session1.wrwire — the
        ...
        printf("  a1 [%u..%u]  no fit: %s (flags=0x%02x, observations=%d)\n", first, last,
               wr_status_str(st), (unsigned)snap.flags, snap.observations);
        (*refused)++;
        return;
    }
     ...
     * harness asked for.  swings.wrwire contains one pull whose `a1` ran 200
     * indices PAST the head — a window whose last sample did not exist yet, so
     * wr_history_reserve() would rightly have held it (AR C1: no radio traffic
     ...
     * boundary of wr_clock_index_range_for_time(), pushed off the rounding edge
     * rather than sat on it. */
    req.window.start_us = wr_clock_to_host_us(&snap, first) - 1;
    req.window.end_us = wr_clock_to_host_us(&snap, last) + 1;
    req.deadline_us = c->host_time_us + (wr_time_us)60 * 1000 * 1000;
    req.max_attempts = 1u;

    st = wr_history_reserve(g_session, &req, &id);
    if (st != WR_OK) {
        printf("  a1 [%u..%u]  reserve refused: %s\n", first, last, wr_status_str(st));
        (*refused)++;
        return;
    }
    (*reserved)++;
...
{
    fputs("usage: wr_gather_replay FILE.wrwire [--json PATH]\n"
          "\n"
          ...
          "                against tools/wr_replay_py.py.  stdout is unchanged.\n"
          "\n"
...
int main(int argc, char **argv)
{
     ...
     * default is two orders of margin over that.  `session1.wrwire`'s first
     ...
     */
    cfg.policy.stream_start_timeout_us = (wr_time_us)15 * 1000 * 1000;
    if (wr_session_create(&cfg, &g_session) != WR_OK) {
        return 2;
    }
    if (wr_replay_open(argv[1], &rp) != WR_OK) {
        fprintf(stderr, "wr_gather_replay: cannot read %s\n", argv[1]);
        wr_session_destroy(g_session);
        return 2;
    }
    ...
    if (json_path != NULL) {
        ...
        if (g_json == NULL) {
            fprintf(stderr, "wr_gather_replay: cannot write %s\n", json_path);
            wr_replay_close(rp);
            wr_session_destroy(g_session);
            return 2;
        }
    ...
    printf("wr_gather_replay — %s\n"
           "the history gather, driven by a real device's own bytes\n\n",
           argv[1]);

    while (wr_replay_next(rp, &c) == WR_OK) {
        if (c.direction == (uint8_t)WR_WIRE_META) {
            /* tools/wr_capture.py writes the same short vocabulary the session
             * does, so one reader handles both. */
            if (c.length >= 7u && memcmp(c.data, "link_up", 7u) == 0) {
                wr_session_on_link_up(g_session, 247, c.host_time_us);
            } else if (c.length >= 12u && memcmp(c.data, "stream_start", 12u) == 0) {
                (void)wr_session_start_stream(g_session);
            }
            drain();
            continue;
        }
        if (c.direction == (uint8_t)WR_WIRE_HOST_TO_DEVICE) {
            wr_session_tick(g_session, c.host_time_us);
            drain();
        ...
        wr_session_on_bytes(g_session, c.data, c.length, c.host_time_us);
        drain();
        collect_if_ready();
        depth_sample();
    }
    ...
    if (g_pending_id != 0u) {
        printf("  ⚠ request %llu was never answered — pending %zu at end of capture\n",
               (unsigned long long)g_pending_id, wr_history_pending(g_session));
    }
    wr_replay_close(rp);
    ...
    for (int i = 0; i < (int)WR_HISTORY_STATUS_COUNT; ++i) {
        if (g_status_count[i] != 0u) {
            printf("  %-20s %u\n", wr_history_status_name((wr_history_status)i),
                   g_status_count[i]);
        }
    }
    /* ⚠ §8.7: every reply is holed and the holes are not an error.  A run with
     * no WR_HIST_COMPLETE in it is the expected shape, not a failure. */
    ...
    if (g_depth_measured > 0u) {
        printf("  verified reach-back  %.3f s   ⚠ a LOWER bound, and no wider than the\n"
               ...
               (double)g_depth_width / 1e6, (double)WR_HISTORY_DEPTH_SEED_US / 1e6);
    }
     ...
     * sample decode rather than only the block's scalars — a ctypes wr_sample
