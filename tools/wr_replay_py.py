...
"""wr_replay_py.py — tools/wr_gather_replay.c, through the Python binding.
...
wr_gather_replay.c before simplifying it — several of these lines are load
...
def unwrap_near(head: int, raw16: int) -> int:
    ...
    NEAREST the head rather than at-or-below: swings.wrwire's sixth pull asks 200
...
class Replayer:
    def __init__(self):
        ...
        # takes.  session1.wrwire's first frame arrived at 4.08 s — a property of
            ...
            self.blocks.append(
                {
                    ...
                    "status_name": wr.history_status_name(block.status),
    ...
    def on_recorded_a1(self, chunk) -> None:
        ...
        if snapshot is None or not (snapshot.flags & T.ClockFlag.HAS_FIT):
            snapshot = snapshot or T.wr_clock_snapshot()
        ...
        # asked for.  swings.wrwire contains one pull whose `a1` ran 200 indices
...
def main() -> int:
    ...
    if not path.is_file():
        ...
        print(f"wr_replay_py: {path} not found.", file=sys.stderr)
        print("⚠ NOTHING WAS CHECKED.  tests/fixtures/ holds the tracked captures.",
              file=sys.stderr)
        return 3

    print(f"wr_replay_py — {path}")
    ...
    for status, count in sorted(replayer.status_count.items()):
        name = wr.history_status_name(status)
