...
/*
 * wr_abi_table.h — the public structs' layout, as the compiler built them.
 *
 * ⚠ WHY THIS EXISTS.  wr_abi_check() compares NINE STRUCT SIZES
 * (include/wrist/version.h) and says nothing whatever about where the
 ...
 * still guards itself with wr_abi_check() at load.
 */
#ifndef WR_ABI_TABLE_H
#define WR_ABI_TABLE_H

#include <stdio.h>

/* Emits the table as JSON on `out`.  Returns the number of self-check problems
 * found — see wr_abi_selfcheck() — which is 0 for a healthy build. */
int wr_abi_print_json(FILE *out, FILE *err);

#endif /* WR_ABI_TABLE_H */
