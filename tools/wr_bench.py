...
"""wr_bench.py — the exemplar: a whole session against a real sensor.

    ./tools/wr_bench.py --out bench.wrwire --duration 120 --calibrate
    ./build/dev/wrwire verify bench.wrwire
    ./build/dev/wr_gather_replay bench.wrwire
...
The radio is `wrist.bleak_transport`, which is a translator and not a
...
=============================================================================
It is NOT `tools/wr_capture.py`.  That script is FROZEN and deliberately does
...
library built it against the allowlist in `src/wr_command.c`.  There is no
...
import wrist as wr  # noqa: E402
from wrist import _types as T  # noqa: E402
from wrist.bleak_transport import BleakTransport, TransportError, monotonic_us  # noqa: E402
...
# it is served (§8.6) — the library says so with WR_EV_HISTORY_EVICTION_RISK,
...
async def prompt(text: str) -> None:
    ...
    fabricated arrival times, and `session1.wrwire` still needs a relaxed
...
class Bench:
    def __init__(self, args, now_us=monotonic_us):
        ...
        self.session = wr.Session(
            args.device_id or "bench",
            wire_ring=wr.WR_WIRE_RING_RECOMMENDED,
            digest_ring=wr.WR_DIGEST_RING_RECOMMENDED,
        )
        self.recorder: wr.Recorder | None = None
    ...
    def on_wire(self, chunks) -> None:
        if self.recorder is not None:
            ...
            # written here goes through `record/wr_record.c`, so there is no
    ...
    def on_live(self, samples) -> None:
        ...
        for sample in samples:
            self.live_samples += 1
            if sample.host_time_us == T.WR_TIME_UNKNOWN:
    ...
    def maybe_pull(self) -> None:
        ...
        request = wr.request_around(peak_us, user_tag=self.swings_seen)
    ...
    def report_block(self, block) -> None:
        ...
        print(f"    block  {wr.history_status_name(block.status)}  "
        ...
        if fit.flags & T.ClockFlag.HAS_FIT and block.sample_count:
            ...
            error = wr.clock_error_at(fit, block.samples[-1].sample_index)
    ...
    async def calibrate(self) -> None:
        ...
        try:
            ...
            # calling early gets WR_ERR_INVALID_STATE, and skipping the call
    ...
    async def run(self) -> int:
        ...
        try:
            ...
            # fired — refuses `stop_stream()` with WR_ERR_INVALID_STATE.  Letting
    ...
    def report(self) -> None:
        ...
        if snapshot is None or not (snapshot.flags & T.ClockFlag.HAS_FIT):
            ...
            error = wr.clock_error_at(snapshot, snapshot.last_index)
...
def find_wrwire(explicit: str | None) -> str | None:
    if explicit:
        return explicit
    for candidate in (ROOT / "build/dev/wrwire", ROOT / "build/rel/wrwire"):
        if candidate.is_file():
            return str(candidate)
    return shutil.which("wrwire")


def main() -> int:
    ap = argparse.ArgumentParser(
        description="A whole HackMotion wrist-sensor session through libwrist.",
        epilog="Then: wrwire verify FILE && wr_gather_replay FILE",
    )
    ap.add_argument("--out", default="bench.wrwire", help="output .wrwire path")
    ...
    ap.add_argument("--wrwire", default=None, help="path to the built wrwire tool")
    args = ap.parse_args()

    print(f"wr_bench — libwrist {wr.VERSION}, abi {wr.ABI_VERSION}")
    print(f"  library  {wr.library_path}")
    ...
    print("  allowlist " + " ".join(f"{b:02x}" for b in wr.command_allowlist()))
    ...
    wrwire = find_wrwire(args.wrwire)
    if wrwire is None:
        print("\n⚠ no `wrwire` tool found, so the recording was NOT verified.")
        return status or 3
    print(f"\nverifying {args.out}")
    if subprocess.run([wrwire, "verify", args.out]).returncode != 0:
        print("wr_bench: ⚠ the recording did not verify — see above")
        return 1
    replay = ROOT / "build/dev/wr_gather_replay"
    print(f"\nNext: {replay if replay.is_file() else 'wr_gather_replay'} {args.out}")
