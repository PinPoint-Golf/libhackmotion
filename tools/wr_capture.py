...
"""
wr_capture.py — record real HackMotion wG3 traffic into a `.wrwire` file.

    ./tools/wr_capture.py --out session.wrwire --duration 120
    ./build/dev/wrwire reconcile session.wrwire
...
real bytes onto disk so `wrwire reconcile` can check the specification against
a sensor (design §11).  Nothing here is part of libwrist, nothing
...
# wr_looks_like_sensor() and agrees.
...
# --- the container, design §5.6 and include/wrist/record.h -------------
MAGIC = b"WRWIRE1\n"
...
async def prompt(text: str) -> None:
    ...
    delay and the estimator is built for exactly that (wr_fit.h).  It was robust
...
def monotonic_us() -> int:
    ...
    looks like a sensor fault (include/wrist/types.h).  The epoch is
...
class Recorder:
    """Writes the `.wrwire` container.  The C reader is the authority on the
    format; `wrwire verify` is run against the result before this script exits,
...
def load_allowlist(wrwire: str) -> set[int]:
    ...
    """
    out = subprocess.run([wrwire, "allowlist"], capture_output=True, text=True, check=True)
...
async def find_device(args) -> str:
    ...
    async with BleakScanner(detection_callback=seen, **kwargs):
        ...
        try:
            ...
            raise SystemExit(
                f"wr_capture: no '{DEVICE_NAME_PREFIX}*' in {args.scan:.0f} s.\n"
...
async def capture(args, allowed: set[int]):
    ...
    try:
        async with BleakClient(address, **({"adapter": args.adapter} if args.adapter else {})) as client:
            ...
            if mtu == 0:
                ...
                raise SystemExit(
                    f"wr_capture: negotiated MTU {mtu} < {MIN_ATT_MTU}; refusing to "
    ...
    if session.max_notify_len:
        ...
        if implied >= MIN_ATT_MTU:
            ...
            print("    TRUNCATION if wrwire reconcile also reports odd frame lengths (§2.4).")
    if session.frames == 0:
...
def main():
    ap = argparse.ArgumentParser(
        description="Record HackMotion wG3 traffic to a .wrwire file.",
        epilog="Then: wrwire verify FILE && wrwire reconcile FILE",
    )
    ap.add_argument("--out", default="capture.wrwire", help="output .wrwire path")
    ...
    ap.add_argument("--wrwire", default=None,
                    help="path to the built wrwire tool (default: search build/ then PATH)")
    args = ap.parse_args()

    if _BLEAK_ERROR is not None:
        sys.exit(f"wr_capture: needs `bleak` (pip install bleak): {_BLEAK_ERROR}")
    ...
    if wrwire is None:
        ...
        for candidate in (root / "build/dev/wrwire", root / "build/rel/wrwire"):
            if candidate.is_file():
                wrwire = str(candidate)
                break
        else:
            wrwire = shutil.which("wrwire")
    if wrwire is None:
        sys.exit(
            "wr_capture: cannot find the `wrwire` tool, so it cannot ask the library\n"
            "  which command bytes are allowed — and it will not guess.  Build it:\n"
            "    cmake -S . -B build/dev && cmake --build build/dev -j"
        )

    try:
        allowed = load_allowlist(wrwire)
    except Exception as exc:  # noqa: BLE001 - any failure here means send nothing
        sys.exit(f"wr_capture: could not read the allowlist from {wrwire}: {exc}")
    print(f"  allowlist from {wrwire}: " + " ".join(f"{b:02x}" for b in sorted(allowed)))
    ...
    print()
    verify = subprocess.run([wrwire, "verify", args.out])
    if verify.returncode != 0:
        sys.exit("wr_capture: ⚠ the recording did not verify — see above")
    print(f"\nNext: {wrwire} reconcile {args.out}")
