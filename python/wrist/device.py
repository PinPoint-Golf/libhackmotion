...
``wr_looks_like_sensor()``.  A transport built on this holds no second copy
...
_UUID_STRING_SIZE = 37  # WR_UUID_STRING_SIZE


def uuid_str(uuid: T.wr_uuid) -> str:
    """The canonical lower-case 8-4-4-4-12 form, rendered by the library."""
    buf = ctypes.create_string_buffer(_UUID_STRING_SIZE)
    lib.wr_uuid_format(ctypes.byref(uuid), buf, len(buf))
    return buf.value.decode()


def parse_uuid(text: str) -> T.wr_uuid:
    ...
    out = T.wr_uuid()
    check(lib.wr_uuid_parse(text.encode(), ctypes.byref(out)), f"wr_uuid_parse({text!r})")
    return out


def _from_dll(symbol: str) -> T.wr_uuid:
    return T.wr_uuid.in_dll(lib, symbol)
...
UUID_TRANSPARENT_UART_SERVICE = _from_dll("WR_UUID_TRANSPARENT_UART_SERVICE")
UUID_DATA_CHARACTERISTIC = _from_dll("WR_UUID_DATA_CHARACTERISTIC")
...
UUID_OTA_SERVICE_FORBIDDEN = _from_dll("WR_UUID_OTA_SERVICE_FORBIDDEN")
...
UUID_ISSC_PIPE_INERT = _from_dll("WR_UUID_ISSC_PIPE_INERT")


def looks_like_sensor(
    local_name: str | None,
    advertised_services: Iterable[str | T.wr_uuid] | None = None,
) -> bool:
    ...
    """
    uuids: list[T.wr_uuid] = []
    for entry in advertised_services or ():
        if isinstance(entry, T.wr_uuid):
            uuids.append(entry)
            continue
        try:
            ...
            continue
    array = (T.wr_uuid * len(uuids))(*uuids) if uuids else None
    return bool(
        lib.wr_looks_like_sensor(
