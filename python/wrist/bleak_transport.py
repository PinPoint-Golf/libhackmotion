...
"""A reference BLE transport over `bleak`, for Linux, macOS and Windows.
    ...
    with wr.Session("bench") as session:
...
⚠ OPTIONAL, AND OUTSIDE THE CORE.  ``import wrist`` does not import this
...
against the allowlist in `src/wr_command.c`.  There is no ``send_raw()``, there
...
def monotonic_us() -> int:
    ...
    looks like a sensor fault (AR C8, `include/wrist/types.h`).  The epoch
    is arbitrary and never interpreted; only differences matter, and the session
    warns with `WR_WARN_HOST_CLOCK_REGRESSION` if this ever goes backwards.
    """
    return time.monotonic_ns() // 1000


# The library's own bounds, read from the library rather than transcribed here.
_POLICY = lib.wr_session_policy_default()
...
async def negotiated_mtu(client) -> int:
    ...
    ``wr_session_on_link_up()`` already has the right word for this case — pass
    0 for "the platform will not tell you", which is treated as
    unknown-and-proceed with `WR_WARN_MTU_UNKNOWN` rather than as a failure —
    ...
    ⚠ This is deliberately a SECOND copy of `tools/wr_capture.py`'s routine.
...
def _same_uuid(text: str, uuid: T.wr_uuid) -> bool:
...
class BleakTransport:
    ...
    def __init__(
        ...
        *,
        ...
        on_wire: Callable[[Sequence[T.wr_wire_chunk]], None] | None = None,
    ...
    async def discover(
        *,
        ...
        The match is ``wr_looks_like_sensor()``, so the advertised name
        ...
        def seen(device, adv):
            ...
            if found.done():
                return
            if looks_like_sensor(name, adv.service_uuids or ()):
    ...
    async def close(self, cause: T.LinkDownCause = T.LinkDownCause.LOCAL_REQUEST) -> None:
        ...
        ⚠ ONE LAST PUMP BEFORE THE RADIO GOES.  ``wr_session_close()`` is not
    ...
    async def _write(self, write: WriteRequest) -> None:
        ...
        composed it against the allowlist in `src/wr_command.c`.  The assertion
    ...
    def classify_disconnect(self) -> LinkClassification:
        ...
        ⚠ AND IT MATTERS AT EXACTLY ONE PLACE.  `wr_session.c` turns
