# SPDX-License-Identifier: MIT
# Copyright (C) 2026 Mark Liversedge
"""The `.wrwire` container, read and written by the library's own code.

⚠ THIS IS THE C IMPLEMENTATION, NOT A SECOND ONE.  `tools/wr_capture.py` writes
the container in Python and is pinned against the C reader by
`tests/test_capture_format.py`, because it predates the binding and is
deliberately kept independent.  Anything written through *this* module goes
through `record/wr_record.c`, so there is no second format to keep in step.

⚠ RECORD THE WIRE BYTES, NOT THE DECODED SAMPLES.  Undecoded fields remain — two
bytes of the battery reply, two configuration bits,
the status byte on the short-form calibration result — and the calibration
payload is understood but decoded by nothing here.  When one is settled a
byte-level recording can be
RE-DECODED with the fix applied.  A sample-level one cannot: it already threw
away the bytes the fix would have read differently.
"""

from __future__ import annotations

import ctypes
from pathlib import Path
from typing import Iterator

from . import _types as T
from ._library import check, lib

__all__ = ["Recorder", "Replay", "recording_info_default"]


def recording_info_default() -> T.wr_recording_info:
    return lib.wr_recording_info_default()


class Recorder:
    """Appends wire chunks to a `.wrwire` file.

    Feed it whatever `Session.poll_wire()` returns.  ⚠ The session's wire ring
    is OFF unless you asked for one (`Session(wire_ring=...)`), and with it off
    `poll_wire()` is always empty — which is an absent recording, not an empty
    session."""

    __slots__ = ("_handle", "_path")

    def __init__(
        self,
        path: str | Path,
        *,
        device_id: str = "",
        config_bits: int = T.WR_CONFIG_OBSERVED_DEFAULT,
        record_identifiers: bool = False,
    ):
        info = lib.wr_recording_info_default()
        info.device_id = device_id.encode()[: T.WR_DEVICE_ID_MAX - 1]
        info.config_bits = config_bits
        # ⚠ Redaction is the default here as it is everywhere else: the MAC and
        # the serial name a specific unit and a specific owner.
        info.identifiers_recorded = record_identifiers

        handle = ctypes.c_void_p()
        check(
            lib.wr_recorder_open(
                str(path).encode(), ctypes.byref(info), ctypes.byref(handle)
            ),
            f"wr_recorder_open({path})",
        )
        self._handle = handle
        self._path = Path(path)

    def __enter__(self) -> "Recorder":
        return self

    def __exit__(self, *exc) -> None:
        self.close()

    @property
    def path(self) -> Path:
        return self._path

    def write(self, chunks) -> None:
        chunks = list(chunks)
        if not chunks:
            return
        array = (T.wr_wire_chunk * len(chunks))(*chunks)
        check(lib.wr_recorder_write(self._handle, array, len(chunks)), "wr_recorder_write")

    @property
    def chunks(self) -> int:
        return lib.wr_recorder_chunks(self._handle)

    @property
    def bytes_written(self) -> int:
        return lib.wr_recorder_bytes(self._handle)

    def close(self) -> None:
        """⚠ Returns the first write error the recorder saw, so a full disk
        cannot end a capture quietly — raised here rather than swallowed."""
        if self._handle:
            handle, self._handle = self._handle, ctypes.c_void_p()
            check(lib.wr_recorder_close(handle), "wr_recorder_close")

    def __del__(self) -> None:
        try:
            self.close()
        except Exception:  # pragma: no cover - interpreter teardown
            pass


class Replay:
    """Reads a `.wrwire` file back, byte-exact.

        with Replay("swings.wrwire") as replay:
            for chunk in replay:
                ...
    """

    __slots__ = ("_handle", "_path")

    def __init__(self, path: str | Path):
        handle = ctypes.c_void_p()
        check(lib.wr_replay_open(str(path).encode(), ctypes.byref(handle)),
              f"wr_replay_open({path})")
        self._handle = handle
        self._path = Path(path)

    def __enter__(self) -> "Replay":
        return self

    def __exit__(self, *exc) -> None:
        self.close()

    @property
    def info(self) -> T.wr_recording_info:
        ptr = lib.wr_replay_info(self._handle)
        if not ptr:
            raise RuntimeError("replay has no header")
        return T.wr_recording_info.from_buffer_copy(ptr.contents)

    def __iter__(self) -> Iterator[T.wr_wire_chunk]:
        """⚠ Yields a COPY per chunk.  A shared buffer would be faster and would
        hand every caller a value that changes under them on the next step."""
        chunk = T.wr_wire_chunk()
        while True:
            status = lib.wr_replay_next(self._handle, ctypes.byref(chunk))
            if status == T.Status.DONE:
                return
            check(status, "wr_replay_next")
            yield T.wr_wire_chunk.from_buffer_copy(chunk)

    @property
    def chunks_read(self) -> int:
        return lib.wr_replay_chunks_read(self._handle)

    def close(self) -> None:
        if self._handle:
            handle, self._handle = self._handle, ctypes.c_void_p()
            lib.wr_replay_close(handle)

    def __del__(self) -> None:
        try:
            self.close()
        except Exception:  # pragma: no cover - interpreter teardown
            pass
