...
from ._library import WristError, check, lib
...
class WriteRequest:
    ...
    ⛔ The library composed these against the allowlist in `src/wr_command.c`.
...
def _format_event(raw: T.wr_event, include_identifiers: bool) -> str:
    buf = ctypes.create_string_buffer(_EVENT_TEXT_MAX)
    lib.wr_event_format(ctypes.byref(raw), buf, len(buf), include_identifiers)
...
# ---------------------------------------------------------------------------
def clock_to_host_us(fit: T.wr_clock_snapshot, sample_index: int) -> int:
    ...
    return lib.wr_clock_to_host_us(ctypes.byref(fit), sample_index)


def clock_error_at(fit: T.wr_clock_snapshot, sample_index: int) -> T.wr_clock_error:
    """⚠ Gate on `precision_us`, record `total_us`.  Gating on the total refuses
    every pull after the first second of a session."""
    return lib.wr_clock_error_at(ctypes.byref(fit), sample_index)


def history_status_name(status) -> str:
    return lib.wr_history_status_name(int(status)).decode()


def gap_kind_name(kind) -> str:
    return lib.wr_gap_kind_name(int(kind)).decode()
...
def warning_code_name(code) -> str:
    ...
    return lib.wr_warning_code_name(int(code)).decode()


def relative_angle_deg(sample: T.wr_sample) -> float:
    ...
    return lib.wr_relative_angle_deg(ctypes.byref(sample))
...
def command_is_allowed(command_byte: int) -> bool:
    ...
    return bool(lib.wr_command_is_allowed(command_byte))
...
def command_allowlist() -> bytes:
    ...
    n = lib.wr_command_allowlist(buf, len(buf))
    return bytes(buf[: min(n, len(buf))])


class Samples(Sequence):
    """A window onto an array of `wr_sample`.
    ...
    def copy(self) -> "Samples":
        ...
        self._alive()
        out = (T.wr_sample * self._count)()
        ctypes.memmove(out, self._array, ctypes.sizeof(T.wr_sample) * self._count)
    ...
    def step_density(self) -> float:
        ...
        self._alive()
        return lib.wr_sample_step_density(self._array, self._count)
...
def request_around(
    event_host_us: int,
    policy: T.wr_session_policy | None = None,
    *,
    ...
    than clamped."""
    req = lib.wr_history_request_around(
...
class HistoryBlock:
    ...
    freed by `wr_history_block_release()`.  Use it as a context manager:
    ...
    def release(self) -> None:
        if not self._released and self._ptr:
            lib.wr_history_block_release(self._ptr)
            self._released = True
            self._ptr = None
    ...
    @property
    def _b(self) -> T.wr_history_block:
        if self._released:
            raise RuntimeError(
    ...
    def samples(self) -> Samples:
        ...
        if block.sample_count == 0 or not block.samples:
            return Samples((T.wr_sample * 0)(), 0, self)
        array = (T.wr_sample * block.sample_count).from_address(
    ...
    def __getattr__(self, name: str):
        ...
        try:
            ...
            raise AttributeError(
                f"wr_history_block has no field {name!r}"
            ) from None
    ...
    def summary(self) -> str:
        ...
        return (
            f"{lib.wr_history_status_name(b.status).decode():<10} "
...
class Session:
    ...
    def __init__(
        ...
        *,
        ...
        stream_config: T.wr_stream_config | None = None,
    ):
        config = lib.wr_session_config_default()
        config.device_id = device_id.encode()[: T.WR_DEVICE_ID_MAX - 1]
        ...
        for field, value in (policy or {}).items():
            if not hasattr(config.policy, field):
                raise TypeError(f"wr_session_policy has no field {field!r}")
            setattr(config.policy, field, value)

        handle = ctypes.c_void_p()
        check(
            lib.wr_session_create(ctypes.byref(config), ctypes.byref(handle)),
            "wr_session_create",
        )
        self._handle = handle
        self._closed = False
    ...
    def close(self) -> None:
        ...
        if not self._closed and self._handle:
            lib.wr_session_close(self._handle)
            self._closed = True

    def destroy(self) -> None:
        if self._handle:
            lib.wr_session_destroy(self._handle)
    ...
    def on_link_up(self, negotiated_mtu: int, now_us: int) -> None:
        ...
        bad one."""
        lib.wr_session_on_link_up(self._handle, negotiated_mtu, now_us)
    ...
    def on_link_down(self, cause: T.LinkDownCause, now_us: int) -> None:
        ...
        lib.wr_session_on_link_down(self._handle, int(cause), now_us)
    ...
    def on_bytes(self, data: bytes, host_recv_us: int) -> None:
        ...
        lib.wr_session_on_bytes(self._handle, buf, len(data), host_recv_us)

    def on_advertising_seen(self, now_us: int) -> None:
        lib.wr_session_on_advertising_seen(self._handle, now_us)
    ...
    def next_due_us(self) -> int:
        ...
        return lib.wr_session_next_due_us(self._handle)

    def tick(self, now_us: int) -> None:
        lib.wr_session_tick(self._handle, now_us)
    ...
    def poll_writes(self, max_count: int = 8) -> list[WriteRequest]:
        ...
        out = (T.wr_write_request * max_count)()
        n = lib.wr_session_poll_writes(self._handle, out, max_count)
        return [
            WriteRequest(
    ...
    def poll_events(self, max_count: int = 32) -> list[Event]:
        out = (T.wr_event * max_count)()
        n = lib.wr_session_poll_events(self._handle, out, max_count)
        events = []
        for i in range(n):
            raw = T.wr_event.from_buffer_copy(out[i])
            try:
            ...
            events.append(
                Event(
                    ...
                    sensitive=bool(lib.wr_event_is_sensitive(ctypes.byref(raw))),
                    raw=raw,
                )
            )
        return events
    ...
    def poll_live(self, max_count: int = 256) -> Samples:
        ...
        out = (T.wr_sample * max_count)()
        n = lib.wr_session_poll_live(self._handle, out, max_count)
        return Samples(out, n)

    def poll_wire(self, max_count: int = 64) -> list[T.wr_wire_chunk]:
        out = (T.wr_wire_chunk * max_count)()
        n = lib.wr_session_poll_wire(self._handle, out, max_count)
        return [T.wr_wire_chunk.from_buffer_copy(out[i]) for i in range(n)]
    ...
    def dropped_live(self) -> int:
        ...
        return lib.wr_session_dropped_live(self._handle)

    @property
    def dropped_events(self) -> int:
        return lib.wr_session_dropped_events(self._handle)

    @property
    def dropped_wire(self) -> int:
        return lib.wr_session_dropped_wire(self._handle)
    ...
    def start_stream(self) -> None:
        ...
        UNKNOWN."""
        check(lib.wr_session_start_stream(self._handle), "start_stream")

    def stop_stream(self) -> None:
        check(lib.wr_session_stop_stream(self._handle), "stop_stream")
    ...
    def power_off(self) -> None:
        ...
        check(lib.wr_session_power_off(self._handle), "power_off")

    @property
    def is_streaming(self) -> bool:
        return bool(lib.wr_session_is_streaming(self._handle))

    @property
    def stream_id(self) -> int:
        return lib.wr_session_stream_id(self._handle)

    # --- queries ----------------------------------------------------------
    def clock(self) -> T.wr_clock_snapshot:
        """Raises `WristError` with `ERR_NO_FIT` until a fit exists."""
        snap = T.wr_clock_snapshot()
        check(lib.wr_session_clock(self._handle, ctypes.byref(snap)), "session_clock")
        return snap

    def clock_or_none(self) -> T.wr_clock_snapshot | None:
        """The fit if there is one, else None — for a caller that treats "no fit
        yet" as ordinary rather than exceptional."""
        snap = T.wr_clock_snapshot()
        if lib.wr_session_clock(self._handle, ctypes.byref(snap)) < T.Status.OK:
            return None
        return snap

    def device_info(self) -> T.wr_device_info:
        info = T.wr_device_info()
        check(lib.wr_session_device_info(self._handle, ctypes.byref(info)), "device_info")
        return info

    def stream_config(self) -> T.wr_stream_config:
        return lib.wr_session_stream_config(self._handle)

    def set_clock_correction(self, correction: T.wr_clock_correction) -> None:
        """⚠ `correction.fields` is REQUIRED; a zero bitmask is an error, and a
        FLAGGED ZERO IS A MEASURED ZERO."""
        check(
            lib.wr_session_set_clock_correction(self._handle, ctypes.byref(correction)),
    ...
    def calibration_state(self) -> T.CalibrationState:
        ...
        return T.CalibrationState(lib.wr_session_calibration_state(self._handle))

    @property
    def calibration_phase(self) -> T.CalibrationPhase:
        return T.CalibrationPhase(lib.wr_calibration_current_phase(self._handle))
    ...
    def presence_angle_deg(self) -> float:
        ...
        return lib.wr_calibration_presence_angle_deg(self._handle)

    def calibration_begin(self) -> None:
        check(lib.wr_calibration_begin(self._handle), "calibration_begin")

    def calibration_confirm_horizontal(self) -> None:
        check(
            lib.wr_calibration_confirm_horizontal(self._handle),
            "calibration_confirm_horizontal",
        )

    def calibration_confirm_raise(self) -> None:
        check(lib.wr_calibration_confirm_raise(self._handle), "calibration_confirm_raise")
    ...
    def calibration_confirm_reference_pose(self) -> None:
        ...
        check(
            lib.wr_calibration_confirm_reference_pose(self._handle),
    ...
    def calibration_abort(self) -> None:
        ...
        check(lib.wr_calibration_abort(self._handle), "calibration_abort")

    # --- history ----------------------------------------------------------
    def history_reserve(self, request: T.wr_history_request) -> int:
        request_id = ctypes.c_uint64()
        check(
            lib.wr_history_reserve(
    ...
    def history_collect(self, request_id: int) -> HistoryBlock | None:
        ...
        or call `.release()`."""
        ptr = ctypes.POINTER(T.wr_history_block)()
        status = lib.wr_history_collect(self._handle, request_id, ctypes.byref(ptr))
    ...
    def history_cancel(self, request_id: int) -> None:
        check(lib.wr_history_cancel(self._handle, request_id), "history_cancel")

    @property
    def history_pending(self) -> int:
        return lib.wr_history_pending(self._handle)

    def history_resident_range(self) -> tuple[T.Status, T.wr_time_range]:
        ...
        has lost the swing."""
        out = T.wr_time_range()
        status = lib.wr_history_resident_range(self._handle, ctypes.byref(out))
    ...
    def history_coverage_available(self, start_us: int, end_us: int) -> bool:
        return bool(
            lib.wr_history_coverage_available(self._handle, start_us, end_us)
        )
