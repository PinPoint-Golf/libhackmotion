...
"""Python bindings for libhackmotion — the sans-I/O HackMotion wrist-sensor library.

    import hackmotion

    with hackmotion.Session(device_id="bench") as s:
...
``hackmotion.bleak_transport`` is an OPTIONAL reference transport over `bleak`;
...
``hackmotion.device`` is the other half of that split — the discovery and GATT
...
against the allowlist in ``src/hm_command.c``.  Do not add one.
"""
...
from ._types import (
    HM_CONFIG_OBSERVED_DEFAULT,
    HM_DIGEST_RING_RECOMMENDED,
    HM_MIN_ATT_MTU,
    HM_TIME_NEVER,
    HM_TIME_UNKNOWN,
    HM_WIRE_RING_RECOMMENDED,
