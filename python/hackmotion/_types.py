...
and offset against `hmwire abi`, which reads them out of the compiler.  Enum
...
summary that drifts is worse than a pointer.  Read `include/hackmotion/` before
changing a meaning.
"""
...
HM_TIME_UNKNOWN = -(2**63)  # INT64_MIN — structurally unavailable, e.g. a
#                             history record's arrival instant
HM_TIME_NEVER = 2**63 - 1  # INT64_MAX — the session has no pending deadline

# --- sizes that appear in struct layouts -----------------------------------
HM_MAX_COMMAND_LEN = 8
HM_WIRE_CHUNK_MAX = 256
HM_DEVICE_ID_MAX = 64
HM_SERIAL_MAX = 16
...
HM_LIVE_RING_RECOMMENDED = 2048
HM_EVENT_RING_RECOMMENDED = 256
HM_WIRE_RING_RECOMMENDED = 1024
HM_HISTORY_GATHER_RECOMMENDED = 12288
HM_COVERAGE_RECOMMENDED = 512
HM_DIGEST_RING_RECOMMENDED = 1024

HM_MIN_ATT_MTU = 96
HM_CONFIG_OBSERVED_DEFAULT = 0x7E
...
class GapKind(enum.IntEnum):
    ...
    the enum table in `hmwire abi` is what said so — nothing about a mislabelled
...
#
# ⚠ hm_warning_code is DELIBERATELY ABSENT.  The binding renders warnings
# through hm_warning_code_name() and keeps no copy of that list, so there is no
...
PINNED_ENUMS = {
    Status: ("hm_status", "HM_"),
    Unit: ("hm_unit", "HM_UNIT_"),
    Channel: ("hm_channel", "HM_CH_"),
    SampleSource: ("hm_sample_source", "HM_SOURCE_"),
    CalibrationState: ("hm_calibration_state", "HM_CAL_"),
    SampleFlag: ("hm_sample_flag", "HM_SAMPLE_"),
    WireDirection: ("hm_wire_direction", "HM_WIRE_"),
    WireFlag: ("hm_wire_flag", "HM_WIRE_"),
    HistoryStatus: ("hm_history_status", "HM_HIST_"),
    GapKind: ("hm_gap_kind", "HM_GAP_"),
    ClockFlag: ("hm_clock_flag", "HM_CLOCK_"),
    LinkDownCause: ("hm_link_down_cause", "HM_LINK_DOWN_"),
    RecoveryAdvice: ("hm_recovery_advice", "HM_RECOVER_"),
    CalibrationPhase: ("hm_calibration_phase", "HM_CALP_"),
    CalibrationAbortReason: ("hm_calibration_abort_reason", "HM_CAL_ABORT_"),
    EventType: ("hm_event_type", "HM_EV_"),
    CorrectionField: ("hm_correction_field", "HM_CORRECTION_"),
    DeviceInfoField: ("hm_device_info_field", "HM_INFO_"),
    ConfigMask: ("hm_config_mask", "HM_CFG_"),
}
...
# Structs — types.h
# ---------------------------------------------------------------------------
class hm_uuid(Structure):
    _fields_ = [("bytes", c_uint8 * 16)]


class hm_index_range(Structure):
    """Inclusive on both ends; ``first == last`` is one sample."""

    _fields_ = [("first", c_uint32), ("last", c_uint32)]


class hm_time_range(Structure):
    """⚠ HALF-OPEN [start_us, end_us) — where ``hm_index_range`` is INCLUSIVE.
    That difference is where an off-by-one is born; never convert one to the
    other by hand."""

    _fields_ = [("start_us", hm_time_us), ("end_us", hm_time_us)]


class hm_allocator(Structure):
...
# --- config.h --------------------------------------------------------------
class hm_stream_config(Structure):
    ...
    _fields_ = [
        ...
        ("legacy", c_uint8),
        ("reserved", c_uint8 * 2),
        ("justification", c_char * HM_CONFIG_JUSTIFICATION_MAX),
    ]


# --- device.h --------------------------------------------------------------
class hm_device_info(Structure):
    ...
    _fields_ = [
        ...
        ("serial", c_char * HM_SERIAL_MAX),
        ("valid", c_uint32),
    ]


# --- sample.h --------------------------------------------------------------
class hm_unit_sample(Structure):
    """One unit's reading.
    ...
    _fields_ = [
        ...
        ("gyro_dps", c_float * 3),
        ("device_time_us", hm_time_us),
    ]


class hm_sample(Structure):
    ...
    _fields_ = [
        ...
        ("host_time_us", hm_time_us),
        ("host_recv_us", hm_time_us),  # HM_TIME_UNKNOWN for history records
        ("precision_us", c_uint32),
        ("uncertainty_us", c_uint32),
        ("lower_arm", hm_unit_sample),  # wire block 0
        ("palm", hm_unit_sample),  # wire block 1
    ]


class hm_pinned_counts(Structure):
    _fields_ = [
        ("n", (c_uint32 * HM_CHANNEL_COUNT) * HM_UNIT_COUNT),
        ("total", c_uint32),
    ]


# --- clock.h ---------------------------------------------------------------
class hm_clock_snapshot(Structure):
    ...
    _fields_ = [
        ...
        ("provenance", c_char * HM_PROVENANCE_MAX),
    ]


class hm_clock_error(Structure):
...
class hm_clock_correction(Structure):
...
# --- event.h ---------------------------------------------------------------
class hm_event_payload(Union):
    """⚠ THE PAYLOAD IS NOT DECODED HERE YET.  ``hm_event_format()`` renders any
    event to one line with identifiers redacted, which is what the binding uses.
    Adding a typed member means adding it to ``tools/hm_abi_table.c`` too, or it
    ...
    _fields_ = [
        ("clock", hm_clock_snapshot),
        ("mtu", c_int32),
        ("reserved_abi", c_uint8 * HM_EVENT_PAYLOAD_MAX),
    ]


class hm_event(Structure):
    _fields_ = [
        ("type", c_uint16),
        ("reserved", c_uint16),
...
# --- history.h -------------------------------------------------------------
class hm_history_request(Structure):
    ...
    _fields_ = [
        ("window", hm_time_range),
        ("deadline_us", hm_time_us),
        ("refill_gaps", c_bool),
...
class hm_gap(Structure):
    _fields_ = [
        ("span", hm_time_range),
        ("indices", hm_index_range),
        ("kind", c_uint8),  # GapKind
        ("reserved", c_uint8 * 7),
    ]


class hm_calibration_span(Structure):
...
class hm_history_block(Structure):
    ...
    _fields_ = [
        ...
        ("stream_id", c_uint64),
        ("user_tag", c_uint64),
        ("requested", hm_time_range),
        ("requested_indices", hm_index_range),
        ("delivered", POINTER(hm_time_range)),
        ("delivered_count", c_size_t),
        ("delivered_indices", POINTER(hm_index_range)),
        ("gaps", POINTER(hm_gap)),
        ("gap_count", c_size_t),
        ...
        ("self_recording_gap", hm_time_range),  # the hole this pull itself caused
        ("fit", hm_clock_snapshot),
        ("calibration", hm_calibration_span),
        ("config", hm_stream_config),
        ("reserved2", c_uint8 * 7),
        ("pinned", hm_pinned_counts),
        ("requested_at_us", hm_time_us),
        ("completed_at_us", hm_time_us),
    ]


# --- session.h -------------------------------------------------------------
class hm_write_request(Structure):
    ...
    _fields_ = [
        ("data", c_uint8 * HM_MAX_COMMAND_LEN),
        ("length", c_uint8),
...
class hm_wire_chunk(Structure):
    _fields_ = [
        ...
        ("sequence", c_uint32),
        ("data", c_uint8 * HM_WIRE_CHUNK_MAX),
    ]


class hm_session_policy(Structure):
    ...
    default" while the same-named knob on ``hm_clock_correction`` treats a
    flagged 0 as a measured zero.  This one is a starting assumption; that one is
    a measurement."""

    _fields_ = [
        ("keepalive_period_us", hm_time_us),
        ("calibration_raise_limit_us", hm_time_us),
        ("calibration_result_timeout_us", hm_time_us),
        ("bringup_timeout_us", hm_time_us),
        ("stream_start_timeout_us", hm_time_us),
        ("keepalive_alarm_us", hm_time_us),
        ("live_gap_alarm_us", hm_time_us),
        ("pinned_report_period_us", hm_time_us),
        ("history_pre_roll_us", hm_time_us),
        ("history_post_roll_us", hm_time_us),
        ("accuracy_drift_us_per_s", c_double),
        ("residual_alarm_us", c_uint32),
        ("clock_event_period_us", hm_time_us),
        ("record_identifiers", c_bool),
    ]


class hm_live_digest(Structure):
    _fields_ = [
        ("sample_index", c_uint32),
        ("reserved", c_uint32),
        ("digest", c_uint64),
...
class hm_session_memory(Structure):
...
class hm_session_config(Structure):
    _fields_ = [
        ("stream_config", hm_stream_config),
        ("policy", hm_session_policy),
        ("memory", hm_session_memory),
        ("allocator", hm_allocator),
        ("device_id", c_char * HM_DEVICE_ID_MAX),  # ⚠ opaque label, never a MAC
    ]


# --- version.h -------------------------------------------------------------
class hm_abi_sizes(Structure):
    _fields_ = [
...
# --- record.h --------------------------------------------------------------
class hm_recording_info(Structure):
    _fields_ = [
        ("device_id", c_char * HM_DEVICE_ID_MAX),
        ("config_bits", c_uint8),
        ("config_legacy", c_uint8),
        ("reserved", c_uint8 * 2),
        ("layout_version", c_uint32),
        ("clock", c_char * HM_RECORD_CLOCK_MAX),
...
PINNED_STRUCTS = {
    "hm_uuid": hm_uuid,
    "hm_index_range": hm_index_range,
    "hm_time_range": hm_time_range,
    "hm_allocator": hm_allocator,
    "hm_stream_config": hm_stream_config,
    "hm_device_info": hm_device_info,
    "hm_unit_sample": hm_unit_sample,
    "hm_sample": hm_sample,
    "hm_pinned_counts": hm_pinned_counts,
    "hm_clock_snapshot": hm_clock_snapshot,
    "hm_clock_error": hm_clock_error,
    "hm_clock_correction": hm_clock_correction,
    "hm_event": hm_event,
    "hm_history_request": hm_history_request,
    "hm_gap": hm_gap,
    "hm_calibration_span": hm_calibration_span,
    "hm_history_block": hm_history_block,
    "hm_write_request": hm_write_request,
    "hm_wire_chunk": hm_wire_chunk,
    "hm_session_policy": hm_session_policy,
    "hm_live_digest": hm_live_digest,
    "hm_session_memory": hm_session_memory,
    "hm_session_config": hm_session_config,
    "hm_abi_sizes": hm_abi_sizes,
    "hm_recording_info": hm_recording_info,
}
