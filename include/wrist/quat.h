...
/*
 * wrist/quat.h — the minimum quaternion algebra, and the one place in this
...
WR_API void wr_quat_conjugate(const float q[4], float out[4]);
WR_API void wr_quat_multiply(const float a[4], const float b[4], float out[4]);
WR_API void wr_quat_normalise(float q[4]);
WR_API float wr_quat_dot(const float a[4], const float b[4]);
 ...
 */
WR_API void wr_quat_relative(const wr_sample *sample, float out_q_rel[4]);
 ...
 * no-raise calibration BEST.  See wr_calibration_presence_event.
 */
WR_API float wr_relative_angle_deg(const wr_sample *sample);
 ...
 * dt_us <= 0.
 */
WR_API float wr_angular_rate_dps(const float q_prev[4], const float q_next[4],
                                 wr_time_us dt_us, float out_axis[3]);
...
WR_API float wr_quat_raw_norm(const int16_t q_raw[4]);
