...
/*
 * wrist/device.h — device constants, GATT identifiers and discovery
 * matching data.
 *
 ...
 */
#ifndef WRIST_DEVICE_H
#define WRIST_DEVICE_H
 ...
 */
#define WR_ADVERTISED_LOCAL_NAME "HackMotion wG3"
 ...
 * this library can speak to it — the MTU floor (WR_MIN_ATT_MTU), the version
 * reply, and WR_WARN_UNVERIFIED_PRODUCT for a product id the specification was
 * not measured on.  Being found and being understood are separate questions
 * and are answered in that order.
 */
#define WR_ADVERTISED_NAME_PREFIX "HackMotion w"
 ...
 * on this one product.  wr_session raises WR_WARN_UNVERIFIED_PRODUCT so the
 ...
 */
#define WR_PRODUCT_ID_MEASURED 0x14u
 ...
 */
#define WR_RECOMMENDED_SCAN_WINDOW_US ((wr_time_us)90 * 1000 * 1000)
 ...
 */
WR_API extern const wr_uuid WR_UUID_TRANSPARENT_UART_SERVICE; /* 49535343-fe7d-4ae5-8fa9-9fafd205e455 */
WR_API extern const wr_uuid WR_UUID_DATA_CHARACTERISTIC;      /* 413c3893-b7e8-4231-9673-7af7aed06ddc */
 ...
 */
WR_API extern const wr_uuid WR_UUID_OTA_SERVICE_FORBIDDEN;    /* 1d14d6ee-fd63-4fa1-bfa4-8f47b42119f0 */
...
WR_API extern const wr_uuid WR_UUID_ISSC_PIPE_INERT;          /* 49535343-1e4d-4bd9-ba61-23c647249616 */
 ...
 *
 * The name test is WR_ADVERTISED_NAME_PREFIX, so "HackMotion wG3" and
 ...
 *
 * ⚠ A MATCH IS NOT A COMPATIBILITY CLAIM.  See WR_ADVERTISED_NAME_PREFIX: this
 ...
 */
WR_API bool wr_looks_like_sensor(const char *local_name,
                                     const wr_uuid *advertised_services,
 ...
 */
#define WR_DEVICE_ID_MAX      64
#define WR_SERIAL_MAX         16
#define WR_MAC_STRING_MAX     18
 ...
 * record decoder uses (wr_codec.h).
 */
#define WR_SENSOR_LOCATION_MAX 4

typedef struct wr_device_info {
     ...
     *
     * At most WR_SENSOR_LOCATION_MAX codes are kept.  ⚠ `sensor_count` is what
     * the reply SAYS and may exceed that; compare the two before indexing.
     */
    uint8_t sensor_count;
    uint8_t sensor_location[WR_SENSOR_LOCATION_MAX];
     ...
     */
    char mac[WR_MAC_STRING_MAX];              /* from 0x85, ASCII hex, colon-separated */
    char serial[WR_SERIAL_MAX];               /* from 0x86, ASCII */

    uint32_t valid;                           /* bitmask of wr_device_info_field */
} wr_device_info;

typedef enum wr_device_info_field {
    WR_INFO_VERSIONS   = 1u << 0,
    WR_INFO_SENSOR_MAP = 1u << 1,
    WR_INFO_BATTERY    = 1u << 2,
    WR_INFO_MAC        = 1u << 3,
    WR_INFO_SERIAL     = 1u << 4
} wr_device_info_field;
...
#define WR_IDLE_SHUTDOWN_US            ((wr_time_us)300 * 1000 * 1000)
...
#define WR_KEEPALIVE_PERIOD_US         ((wr_time_us)30 * 1000 * 1000)
...
#define WR_POWER_OFF_LINGER_US         ((wr_time_us)12 * 1000 * 1000)

/* The sample counter wraps every 82.0 s at ≈799.2 Hz (§6.5). */
#define WR_SAMPLE_INDEX_MODULUS        65536u
...
#define WR_HISTORY_DEPTH_SEED_US       ((wr_time_us)7500 * 1000)
...
#define WR_DEFAULT_PRE_ROLL_US         ((wr_time_us)3000 * 1000)
#define WR_DEFAULT_POST_ROLL_US        ((wr_time_us)1500 * 1000)

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* WRIST_DEVICE_H */
