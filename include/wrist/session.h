...
/*
 * wrist/session.h — the sans-I/O core.
 *
 * ============================================================================
 * THE THREADING CONTRACT.  Three sentences, as api-request §2.0.7 asked.
 * ============================================================================
 * 1. Every wr_session_* / wr_history_* / wr_calibration_* function must be
 ...
 *    are wr_history_block values, which are immutable and owned by the caller
 *    from the moment wr_history_collect() returns them, and wr_sample / wr_event
 *    copies, which are POD.
 * 3. After wr_session_close() returns, no further sample or event can be
 ...
 */
#ifndef WRIST_SESSION_H
#define WRIST_SESSION_H
...
typedef struct wr_session wr_session;
 ...
 *
 * wr_command_is_allowed() is the gate every write passes through, and the test
 ...
 */
#define WR_MAX_COMMAND_LEN 8

typedef struct wr_write_request {
    uint8_t data[WR_MAX_COMMAND_LEN];
    uint8_t length;
 ...
 * the host drains them with wr_session_poll_wire().  Writing them anywhere is
 * the host's business (or the optional wrist_record module's).
 */
#define WR_WIRE_CHUNK_MAX 256

typedef enum wr_wire_direction {
    WR_WIRE_HOST_TO_DEVICE = 0,
    WR_WIRE_DEVICE_TO_HOST = 1,
    WR_WIRE_META           = 2   /* link up/down, MTU, stream boundaries */
} wr_wire_direction;

typedef enum wr_wire_flag {
    ...
    WR_WIRE_REDACTED = 1u << 0,
    WR_WIRE_LOST     = 1u << 1   /* chunks were dropped before this one */
} wr_wire_flag;

typedef struct wr_wire_chunk {
    wr_time_us host_time_us;
    uint16_t   length;
    uint8_t    direction;   /* wr_wire_direction */
    uint8_t    flags;       /* wr_wire_flag */
    uint32_t   sequence;
    uint8_t    data[WR_WIRE_CHUNK_MAX];
} wr_wire_chunk;

/* ------------------------------------------------------------------------ */
/* Policy                                                                    */
/* ------------------------------------------------------------------------ */
typedef struct wr_session_policy {
    /*
     ...
     */
    wr_time_us keepalive_period_us;
     ...
     * 0 selects 6 s.
     */
    wr_time_us calibration_raise_limit_us;
     ...
     * way back except wr_calibration_abort().  Which of the two waits expired is
     * in `previous_phase` on the WR_EV_CALIBRATION_PHASE event.
     */
    wr_time_us calibration_result_timeout_us;

    /* Bring-up watchdog: if the device has said nothing by then, give up and
     * classify.  0 → 10 s. */
    wr_time_us bringup_timeout_us;
     ...
     * WR_WARN_STREAM_START_TIMEOUT is emitted — the session does NOT silently
     * retry, because a start that failed once will fail again and the consumer
     * needs to see it.  0 → 3 s.
     */
    wr_time_us stream_start_timeout_us;

    /*
     * ⚠ How long without a host→device write before WR_WARN_KEEPALIVE_LATE.
     ...
     */
    wr_time_us keepalive_alarm_us;

    /*
     * How long without a live frame, WHILE STREAMING AND OUTSIDE A RETRIEVAL,
     * before WR_WARN_LIVE_GAP.  ⚠ Suppressed during a history bracket, where
     ...
     */
    wr_time_us live_gap_alarm_us;

    /*
     * How often to emit WR_EV_PINNED_SAMPLES, and only when the count has
     ...
     */
    wr_time_us pinned_report_period_us;

    /* Defaults for wr_history_request_around().  0 → §7.6's 3 s / 1.5 s. */
    wr_time_us history_pre_roll_us;
    wr_time_us history_post_roll_us;

    /*
     * The systematic carried in every clock snapshot, microseconds of error per
     * second of session.  ⚠ 0 here means "use the default"
     * (WR_ACCURACY_DRIFT_US_PER_S_DEFAULT, 2200 µs/s — §10's measured figure),
     * consistent with every other field in this struct.
     *
     * ⚠ That is the OPPOSITE of wr_clock_correction.residual_drift_us_per_s,
    ...
    /* Report WR_EV_CLOCK_DEGRADED when residual p90 exceeds this.  0 → 20 ms. */
    uint32_t residual_alarm_us;

    /* Emit WR_EV_CLOCK_UPDATED at most this often.  0 → 1 s. */
    wr_time_us clock_event_period_us;
...
WR_API wr_session_policy wr_session_policy_default(void);
 ...
 *
 * ⚠ wr_session_create() makes EXACTLY ONE allocation and wr_session_destroy()
 ...
 */
typedef struct wr_live_digest {
    uint32_t sample_index;
    uint32_t reserved;
    uint64_t digest;      /* over the RAW counts of both units */
} wr_live_digest;

typedef struct wr_session_memory {
    wr_sample     *live_ring;         size_t live_ring_capacity;    /* 0 → recommended */
    wr_event      *event_ring;        size_t event_ring_capacity;   /* 0 → recommended */
    wr_wire_chunk *wire_ring;         size_t wire_ring_capacity;    /* ⚠ 0 → OFF */
    wr_sample     *history_gather;    size_t history_gather_capacity; /* 0 → recommended */
    wr_index_range *coverage_storage; size_t coverage_capacity;     /* 0 → recommended */
     ...
     */
    wr_live_digest *digest_ring;      size_t digest_ring_capacity;  /* ⚠ 0 → OFF */
} wr_session_memory;
 ...
 */
#define WR_LIVE_RING_RECOMMENDED       2048u
#define WR_EVENT_RING_RECOMMENDED       256u
#define WR_WIRE_RING_RECOMMENDED       1024u
#define WR_HISTORY_GATHER_RECOMMENDED 12288u   /* > the ~6000-sample estimate */
#define WR_COVERAGE_RECOMMENDED         512u
/*
 ...
 * the ring covers is compared over fewer indices, `wr_overlap.evicted` counts
 ...
 */
#define WR_DIGEST_RING_RECOMMENDED     1024u

/* ------------------------------------------------------------------------ */
/* Configuration                                                             */
/* ------------------------------------------------------------------------ */
typedef struct wr_session_config {
    wr_stream_config  stream_config;   /* wr_stream_config_default() unless you mean it */
    wr_session_policy policy;
    wr_session_memory memory;          /* zeroed → allocate from `allocator` */
    wr_allocator      allocator;       /* zeroed → malloc/free */
...
WR_API wr_session_config wr_session_config_default(void);

/* ------------------------------------------------------------------------ */
/* Lifecycle                                                                 */
/* ------------------------------------------------------------------------ */
WR_API wr_status wr_session_create(const wr_session_config *config, wr_session **out_session);
 ...
 * the `83` produced by wr_session_stop_stream() first if it wants a clean stop.
 *
 * ⚠ CLOSE, THEN DRAIN ONCE MORE — closing FINISHES work, it does not discard it.
 * Every outstanding reservation materialises here with WR_HIST_CANCELLED and
 * whatever had arrived, so after this returns:
 *
 *   - wr_session_poll_events() still yields the WR_EV_HISTORY_READY (and any
 *     warning) for each of them;
 *   - wr_history_collect() still hands the blocks over, until destroy().
 *
 ...
 */
WR_API void wr_session_close(wr_session *session);

/* Implies close().  Any wr_history_block already collected stays valid. */
WR_API void wr_session_destroy(wr_session *session);
...
/*
 * ⚠ Fails loudly if `negotiated_mtu` < WR_MIN_ATT_MTU: it emits
 * WR_EV_MTU_REJECTED carrying the negotiated value and refuses to run, because
 ...
 */
WR_API void wr_session_on_link_up(wr_session *session, int32_t negotiated_mtu, wr_time_us now_us);
 ...
 * expressible: link-down drives every subsequent sample to WR_CAL_UNCALIBRATED
 * and the routine must be re-run.
 */
WR_API void wr_session_on_link_down(wr_session *session, wr_link_down_cause cause,
                                    wr_time_us now_us);
 ...
 * didUpdateValueForCharacteristic on CoreBluetooth — and WR_MIN_ATT_MTU exists
 ...
 * instead it raises WR_WARN_TRAILING_BYTES on the first frame that is not a
 * whole number of records, loudly and immediately.
 */
WR_API void wr_session_on_bytes(wr_session *session, const uint8_t *data, size_t length,
...
WR_API void wr_session_on_advertising_seen(wr_session *session, wr_time_us now_us);
 ...
 * WR_TIME_NEVER.  Re-read after every call into the session. */
WR_API wr_time_us wr_session_next_due_us(const wr_session *session);

/* Run everything now due.  Cheap and idempotent when nothing is. */
WR_API void wr_session_tick(wr_session *session, wr_time_us now_us);
 ...
 */
WR_API size_t wr_session_poll_writes(wr_session *session, wr_write_request *out, size_t max);
WR_API size_t wr_session_poll_events(wr_session *session, wr_event *out, size_t max);
WR_API size_t wr_session_poll_live(wr_session *session, wr_sample *out, size_t max);
WR_API size_t wr_session_poll_wire(wr_session *session, wr_wire_chunk *out, size_t max);
...
WR_API uint64_t wr_session_dropped_live(const wr_session *session);
WR_API uint64_t wr_session_dropped_events(const wr_session *session);
WR_API uint64_t wr_session_dropped_wire(const wr_session *session);
 ...
 * WR_EV_READY and leave it running for the whole session: calibration needs it
 ...
 * any routine in progress, and a second start drops WR_CAL_CALIBRATED to
 * WR_CAL_UNKNOWN — because whether a restart costs the device's transform is
 ...
 */
WR_API wr_status wr_session_start_stream(wr_session *session);
WR_API wr_status wr_session_stop_stream(wr_session *session);
WR_API bool      wr_session_is_streaming(const wr_session *session);
WR_API uint64_t  wr_session_stream_id(const wr_session *session);
 ...
 */
WR_API wr_status wr_session_power_off(wr_session *session);

/*
 * ⚠ TEARDOWN AND OUTSTANDING RESERVATIONS.  wr_session_stop_stream(),
 * wr_session_power_off() and wr_session_close() each CANCEL any queued or
 * in-flight retrieval FIRST, materialising a block with WR_HIST_CANCELLED and
 ...
 * initiates yields WR_HIST_CANCELLED; anything that HAPPENS TO US — a link drop
 * — yields WR_HIST_LINK_LOST, immediately rather than at the deadline.
 *
 * ⚠ ONE VISIBLE CONSEQUENCE.  wr_session_stop_stream() called DURING a
 ...
 *
 * ⚠ THERE IS DELIBERATELY NO wr_calibration_save() / _load() / "reuse last
 ...
 *
 *   - Every wr_calibration_* call returns WR_ERR_BUSY while a history bracket
 ...
 *
 * ⚠ ONE EXEMPTION: wr_calibration_abort() ALWAYS works, bracket or no bracket.
...
/*
 * ⚠ Returns WR_ERR_NO_STREAM if no stream is running.  §8.2: the device
 ...
 * It refuses rather than waiting — see wr_calibration_phase for why there is no
 * AWAIT_STREAM state.
 */
WR_API wr_status wr_calibration_begin(wr_session *session);

/* The user is in pose 0 — forearm horizontal, wrist straight.  Sends `a2 00`.
 * Refuses with WR_ERR_NO_STREAM if the stream is not already running: the
 * device cannot observe a raise from two static samples (§8.2). */
WR_API wr_status wr_calibration_confirm_horizontal(wr_session *session);

/* The user has completed the raise — forearm raised ~30° across the chest.
 * Sends `a2 01`. */
WR_API wr_status wr_calibration_confirm_raise(wr_session *session);
 ...
 * averages a short run of live samples and emits WR_EV_CALIBRATION_PRESENCE.
 *
 * ⚠ SKIPPING IT LEAVES wr_sample.calibration AT WR_CAL_UNKNOWN, not
 ...
 *
 * ⚠ Returns WR_ERR_BUSY while a history bracket is open: the measurement is
 * taken from live samples and a retrieval suspends them.  Retry once the pull
 * completes — the user is standing still either way.
 *
 * ⚠ IT RETURNS BEFORE THE MEASUREMENT EXISTS.  WR_OK means the run has started;
 * the library then collects the next live samples at that pose and reports
 * WR_EV_CALIBRATION_PRESENCE when it has enough, or WR_WARN_PRESENCE_NOT_MEASURED
 * when too few arrived to average.  The phase reaches WR_CALP_COMPLETE either
 * way, so a UI waits on the event and not on this return value.
 */
WR_API wr_status wr_calibration_confirm_reference_pose(wr_session *session);

/*
 * ⚠ TWO OUTCOMES, BECAUSE THE DEVICE CANNOT BE UNDONE.
 *
 * Before `0x94`, nothing has been applied and this ends the routine at
 * WR_CALP_ABORTED with WR_CAL_ABORT_CALLER.
 *
 * At WR_CALP_VERIFYING the transform is ALREADY APPLIED — the device
 ...
 * check, not to undo a calibration, and the phase goes to WR_CALP_COMPLETE
 * carrying WR_CAL_ABORT_CALLER: the routine finished, the check was declined,
 * the angle stays NaN and wr_sample.calibration stays WR_CAL_UNKNOWN.  Reporting
 * ABORTED there would tell a consumer nothing happened to a stream whose frame
 * had just changed underneath it.
 *
 * Returns WR_ERR_INVALID_STATE when no routine is running.
 */
WR_API wr_status wr_calibration_abort(wr_session *session);

WR_API wr_calibration_phase wr_calibration_current_phase(const wr_session *session);
WR_API wr_calibration_state wr_session_calibration_state(const wr_session *session);

/*
 * The reference-pose relative angle, degrees.  ⚠ A PRESENCE CHECK, NOT A
 * QUALITY SCORE — see wr_calibration_presence_event for the measurement that
 * shows it inverts.  NaN before any measurement.
 */
WR_API float wr_calibration_presence_angle_deg(const wr_session *session);

/*
 * The reference-pose anchor from the most recent presence measurement — the
 * same value WR_EV_CALIBRATION_PRESENCE carried.
 *
 ...
 * one measured.  Returns WR_ERR_INVALID_STATE if no measurement has been taken
 * on the current calibration.
 */
WR_API wr_status wr_calibration_reference_anchor(const wr_session *session,
                                                 wr_calibration_presence_event *out);

/* ------------------------------------------------------------------------ */
/* Queries — synchronous, no radio, no blocking                              */
/* ------------------------------------------------------------------------ */
WR_API wr_status wr_session_clock(const wr_session *session, wr_clock_snapshot *out);
WR_API wr_status wr_session_device_info(const wr_session *session, wr_device_info *out);
WR_API wr_stream_config wr_session_stream_config(const wr_session *session);
 ...
 *
 * ⚠ BOTH KNOBS MOVE TOGETHER — see wr_clock_correction in clock.h for why, and
 ...
 * WR_ERR_INVALID_ARG.  Set WR_CORRECTION_RATE, WR_CORRECTION_DRIFT, or both;
 ...
 */
WR_API wr_status wr_session_set_clock_correction(wr_session *session,
