...
/*
 * wrist/coverage.h — what actually arrived, as intervals.
 *
 ...
 */
#ifndef WRIST_COVERAGE_H
#define WRIST_COVERAGE_H
 ...
 */
typedef struct wr_coverage {
    wr_index_range *ranges;    /* caller-owned storage */
    size_t          capacity;
    size_t          count;
    ...
    bool            has_bounds;
    wr_index_range  bounds;    /* min..max seen, exact even after overflow */
} wr_coverage;

WR_API void wr_coverage_init(wr_coverage *cov, wr_index_range *storage, size_t capacity);
WR_API void wr_coverage_reset(wr_coverage *cov);

/* WR_OK, or WR_ERR_BUFFER_TOO_SMALL once `overflowed` is set.  On overflow the
...
WR_API wr_status wr_coverage_add(wr_coverage *cov, uint32_t index);
WR_API wr_status wr_coverage_add_range(wr_coverage *cov, wr_index_range range);

WR_API bool wr_coverage_contains(const wr_coverage *cov, uint32_t index);

/* Distinct indices held within `window`, and the fraction of `window` they
 * cover, in [0,1].  A window of zero width yields 0.0. */
WR_API uint64_t wr_coverage_indices_in(const wr_coverage *cov, wr_index_range window);
WR_API double   wr_coverage_fraction(const wr_coverage *cov, wr_index_range window);

/*
 * ⚠ ACHIEVED DENSITY IS NOT HERE, AND THAT IS DELIBERATE.  C4's other half is
 * wr_sample_step_density() in history.h, measured over the DELIVERED SAMPLES.
 *
...
WR_API size_t wr_coverage_gaps(const wr_coverage *cov,
                               wr_index_range window,
                               wr_index_range *out,
                               size_t max);
...
WR_API uint32_t wr_coverage_largest_gap(const wr_coverage *cov, wr_index_range window);

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* WRIST_COVERAGE_H */
