...
/*
 * wrist/event.h — everything the session has to say, as drained POD.
 *
 ...
 */
#ifndef WRIST_EVENT_H
#define WRIST_EVENT_H
...
typedef enum wr_event_type {
    WR_EV_NONE = 0,

    /* Link -------------------------------------------------------------- */
    WR_EV_LINK_UP,            /* bring-up has started                         */
    WR_EV_LINK_DOWN,          /* with a classification — see wr_link_down     */
    WR_EV_MTU_REJECTED,       /* ⚠ negotiated MTU < 96; the session refuses to run */
    WR_EV_READY,              /* bring-up complete; safe to start a stream    */

    /* Device information ------------------------------------------------ */
    WR_EV_DEVICE_INFO,        /* 0x80 versions + 0x84 sensor map              */
    WR_EV_BATTERY,            /* 0x81 — arrives every 30 s as a side effect   */
    WR_EV_IDENTITY,           /* ⚠ 0x85 MAC / 0x86 serial — personal data     */

    /* Stream ------------------------------------------------------------ */
    WR_EV_STREAM_STARTED,     /* first 0x90 frame seen, NOT the a0 01 ack     */
    WR_EV_STREAM_STOPPED,
    WR_EV_STREAM_RESTARTED,   /* ⚠ reported, never silent — api-request B8    */

    /* Calibration ------------------------------------------------------- */
    WR_EV_CALIBRATION_PHASE,  /* the observable state machine, for a UI       */
    WR_EV_CALIBRATION_PRESENCE, /* the reference-pose angle — presence, NOT quality */

    /* History ----------------------------------------------------------- */
    WR_EV_HISTORY_STARTED,    /* `a1 02` accepted — the acceptance test       */
    WR_EV_HISTORY_PROGRESS,
    WR_EV_HISTORY_READY,      /* a block is collectable                       */
    WR_EV_HISTORY_BLIND_SPAN, /* live delivery was suspended for a retrieval  */
    WR_EV_HISTORY_EVICTION_RISK, /* a queued request may not survive the wait */

    /* Clock ------------------------------------------------------------- */
    WR_EV_CLOCK_UPDATED,      /* periodic; carries the current snapshot       */
    WR_EV_CLOCK_DEGRADED,     /* ⚠ residual spread widened, or fit degenerate */

    /* Device -> host oddments ------------------------------------------- */
    WR_EV_BUTTON,             /* 0xfb — an edge HINT; do not build a counter  */
    WR_EV_DEVICE_ERROR,       /* 0xd0 — one byte, and it means nothing specific */
    WR_EV_UNKNOWN_MESSAGE,    /* logged and ignored, per §5.1                 */
    WR_EV_PINNED_SAMPLES,     /* ⚠ silent int16 saturation was observed       */
    WR_EV_WARNING,
 ...
 */
typedef enum wr_link_down_cause {
    WR_LINK_DOWN_UNKNOWN = 0,
    WR_LINK_DOWN_LOCAL_REQUEST,     /* we asked for it                         */
    WR_LINK_DOWN_SUPERVISION_TIMEOUT,
    WR_LINK_DOWN_REMOTE_CLOSED,
    WR_LINK_DOWN_TRANSPORT_ERROR,
    WR_LINK_DOWN_ADAPTER_GONE,
    /* Not a transport cause; the transport may report it if it knows. */
    WR_LINK_DOWN_CONNECTION_TAKEN   /* ⚠ the device accepts ONE connection and the
...
typedef enum wr_recovery_advice {
    WR_RECOVER_UNKNOWN = 0,
    WR_RECOVER_RECONNECT_WITH_BACKOFF, /* ordinary link loss                    */
    WR_RECOVER_NEEDS_BUTTON_PRESS,     /* ⚠ do NOT retry — only the user can fix */
    WR_RECOVER_NEEDS_OTHER_APP_CLOSED,
    WR_RECOVER_DO_NOT_RETRY            /* we powered it off on purpose           */
} wr_recovery_advice;

typedef struct wr_link_down_event {
    uint8_t    cause;               /* wr_link_down_cause   */
    uint8_t    advice;              /* wr_recovery_advice   */
    uint8_t    calibration_invalidated; /* ⚠ always 1 — §8.3 */
    uint8_t    reserved;
    wr_time_us connected_for_us;
    wr_time_us since_last_device_byte_us;
...
/* Calibration                                                               */
/* ------------------------------------------------------------------------ */
typedef enum wr_calibration_phase {
    WR_CALP_IDLE = 0,
     ...
     * the markers — so wr_calibration_begin() REFUSES with WR_ERR_NO_STREAM
     * rather than entering a waiting state.  Under the one-stream cycle the
     * stream is open from just after WR_EV_READY and stays open (AR B8), so a
     * wait would never be entered anyway, and an unreachable phase is worse
     * than no phase. */
    WR_CALP_AWAIT_HORIZONTAL,  /* forearm horizontal, wrist straight          */
    WR_CALP_MARKING_POSE0,     /* `a2 00` written, awaiting `a2 01` reply     */
    WR_CALP_OBSERVING_RAISE,   /* the device is watching a CONTINUOUS raise   */
    WR_CALP_MARKING_POSE1,     /* `a2 01` written, awaiting `a2 01` reply     */
    WR_CALP_APPLYING,          /* awaiting `0x94`                             */
    WR_CALP_VERIFYING,         /* ⚠ transform APPLIED; presence not yet measured */
    /*
     ...
     * verdict (§8.2).  Read wr_session_calibration_state() for what is actually
     * known — WR_CAL_UNKNOWN when the presence check was skipped or could not be
     * taken, and only WR_CAL_CALIBRATED when one was measured and passed.
     */
    WR_CALP_COMPLETE,
    WR_CALP_ABORTED,           /* a real state, not an error string           */
    WR_CALIBRATION_PHASE_COUNT
} wr_calibration_phase;

typedef enum wr_calibration_abort_reason {
    WR_CAL_ABORT_NONE = 0,
    /*
     * wr_calibration_abort().  ⚠ Also carried on a transition to COMPLETE, not
     * only to ABORTED: aborting at WR_CALP_VERIFYING declines the presence check
     * on a transform the device has ALREADY applied, and reporting that as
     * ABORTED would claim nothing happened.  See wr_calibration_abort().
     */
    WR_CAL_ABORT_CALLER,
    WR_CAL_ABORT_RAISE_TOO_SLOW,   /* CLIENT policy — the device imposes no deadline */
    WR_CAL_ABORT_STREAM_LOST,
    WR_CAL_ABORT_LINK_LOST,
    /*
     ...
     */
    WR_CAL_ABORT_NO_RESULT
} wr_calibration_abort_reason;

typedef struct wr_calibration_phase_event {
    uint8_t    phase;          /* wr_calibration_phase */
    uint8_t    previous_phase;
    uint8_t    abort_reason;   /* wr_calibration_abort_reason */
    uint8_t    reserved;
    wr_time_us elapsed_us;     /* since the routine began */
} wr_calibration_phase_event;
...
#define WR_PRESENCE_CALIBRATED_MAX_DEG  6.0f
#define WR_PRESENCE_ABSENT_MIN_DEG     10.0f

/* Longest run of live samples the presence check will consider. */
#define WR_PRESENCE_MAX_SAMPLES 64u

typedef struct wr_calibration_presence_event {
     ...
     * The distinction is load-bearing and wr_presence.c explains it: near zero,
     ...
     * wr_relative_angle_deg() of this pair reproduces `relative_angle_deg`
     ...
     * run of at most WR_PRESENCE_MAX_SAMPLES the heading drift of §6.2
     ...
     * ([WR_UNIT_LOWER_ARM], [WR_UNIT_PALM]).
     *
    ...
    uint8_t state;                /* wr_calibration_state */
    uint8_t samples_used;
    uint8_t reserved[2];
} wr_calibration_presence_event;

/* ------------------------------------------------------------------------ */
/* History                                                                   */
/* ------------------------------------------------------------------------ */
typedef struct wr_history_progress_event {
    uint64_t request_id;
    uint32_t records_delivered;
    float    fraction;            /* of the requested index span, [0,1] */
    wr_time_us elapsed_us;
} wr_history_progress_event;

typedef struct wr_history_blind_span_event {
    uint64_t   request_id;
    wr_time_range span;           /* host time over which live delivery was suspended */
} wr_history_blind_span_event;

typedef struct wr_history_eviction_risk_event {
    uint64_t   request_id;
    wr_time_us queued_for_us;
    wr_time_us estimated_eviction_in_us;  /* may be negative: already at risk */
} wr_history_eviction_risk_event;

/* ------------------------------------------------------------------------ */
/* Misc payloads                                                             */
/* ------------------------------------------------------------------------ */
typedef struct wr_stream_event {
    uint64_t stream_id;
    uint8_t  config_bits;
    uint8_t  reserved[7];
} wr_stream_event;

typedef struct wr_battery_event {
    uint8_t  percent;
    uint8_t  reserved;
    uint16_t status_undecoded;    /* observed 2226-2235.  NOT millivolts. */
} wr_battery_event;

typedef struct wr_device_error_event {
...
} wr_device_error_event;

typedef struct wr_unknown_message_event {
    uint8_t message_id;
    uint8_t length;
    uint8_t first_bytes[6];       /* for a log line; the wire log has the rest */
} wr_unknown_message_event;

typedef struct wr_pinned_event {
    wr_pinned_counts counts;
    uint64_t         over_samples; /* how many samples the counts cover */
} wr_pinned_event;

typedef enum wr_warning_code {
    WR_WARN_NONE = 0,
    WR_WARN_SHORT_FRAME,
     ...
     */
    WR_WARN_UNEXPECTED_RECORD_COUNT,
    WR_WARN_QUAT_NORM,             /* structural check failed — decode may be misaligned */
    WR_WARN_INDEX_REGRESSION,
    WR_WARN_TICK_PREDICTION_MARGIN,/* wrap resolution getting close to ambiguous */
     ...
     * ever: start_stream() refused with WR_ERR_INVALID_STATE, every calibration
     * call refused with WR_ERR_NO_STREAM, history never issued, and nothing said
     * why.  The session now stops locally and reports this instead.
     */
    WR_WARN_STREAM_STOP_TIMEOUT,
    /*
     ...
     * wr_history_resident_range() falls back to the narrower of the two, which
     ...
     * answers through.
     */
    WR_WARN_HISTORY_DEPTH_CONFLICT,
    ...
    WR_WARN_HOST_CLOCK_REGRESSION,
    WR_WARN_NONSTANDARD_CONFIG,
    WR_WARN_LEGACY_STREAM,         /* 0x7f frames: no device clock, no history */
     ...
     * stays WR_CAL_UNKNOWN and `wr_calibration_presence_angle_deg()` stays NaN.
     */
    WR_WARN_PRESENCE_NOT_MEASURED,
    /*
     * ⚠ The reference-pose angle landed BETWEEN the two populations §8.2
     * measured — above WR_PRESENCE_CALIBRATED_MAX_DEG and below
     * WR_PRESENCE_ABSENT_MIN_DEG.  `detail_f64` carries the angle.
     *
     ...
     */
    WR_WARN_CALIBRATION_INDETERMINATE,
    /*
     ...
     * state is driven to WR_CAL_UNCALIBRATED.
     */
    WR_WARN_CALIBRATION_ABSENT,
    /*
     ...
     * `detail_i32` is the wr_calibration_phase it arrived in.
     *
     ...
     * per-sample flag drops to WR_CAL_UNKNOWN with it.
     */
    WR_WARN_CALIBRATION_UNSOLICITED,
    /*
     ...
     * read it.
     */
    WR_WARN_CALIBRATION_STATUS_FORM,
    /*
     ...
     */
    WR_WARN_SENSOR_COUNT_UNSUPPORTED,
    /*
     * ⚠ The device reports a product id other than WR_PRODUCT_ID_MEASURED — it
     ...
     */
    WR_WARN_UNVERIFIED_PRODUCT,
    WR_WARN_CODE_COUNT
} wr_warning_code;

typedef struct wr_warning_event {
    uint16_t code;                 /* wr_warning_code */
    uint16_t reserved;
    int32_t  detail_i32;
    double   detail_f64;
} wr_warning_event;
...
/* The event                                                                 */
/* ------------------------------------------------------------------------ */
typedef struct wr_event {
    uint16_t   type;               /* wr_event_type */
    uint16_t   reserved;
    uint32_t   sequence;           /* monotonic per session; gaps mean the queue overflowed */
    wr_time_us host_time_us;       /* when the session learned this, on the caller's clock */
    uint64_t   stream_id;          /* 0 outside a stream */

    union {
        wr_link_down_event             link_down;
        wr_device_info                 device_info;
        wr_battery_event               battery;
        wr_stream_event                stream;
        wr_calibration_phase_event     calibration_phase;
        wr_calibration_presence_event  calibration_presence;
        wr_history_progress_event      history_progress;
        wr_history_blind_span_event    history_blind_span;
        wr_history_eviction_risk_event history_eviction_risk;
        wr_clock_snapshot              clock;
        wr_device_error_event          device_error;
        wr_unknown_message_event       unknown_message;
        wr_pinned_event                pinned;
        wr_warning_event               warning;
        int32_t                        mtu;         /* WR_EV_MTU_REJECTED: negotiated value */
        /*
         * Deliberate ABI reservation, not a debug escape hatch: it fixes
         * sizeof(wr_event) so a future payload can be added without moving it
         * and without breaking a binding compiled against an older header.
         * 256 leaves ~39 % headroom over the largest real member
         * (wr_clock_snapshot).  Nothing writes through it.
         */
        uint8_t                        reserved_abi[256];
    } u;
} wr_event;

WR_API const char *wr_event_type_name(wr_event_type type);
WR_API const char *wr_warning_code_name(wr_warning_code code);
WR_API const char *wr_calibration_phase_name(wr_calibration_phase phase);
WR_API const char *wr_link_down_cause_name(wr_link_down_cause cause);
WR_API const char *wr_recovery_advice_name(wr_recovery_advice advice);

/*
 * ⚠ SAFE TO LOG?  api-request §2.13: the MAC and serial identify a specific
 * unit and a specific owner.  wr_event_is_sensitive() is true for exactly the
 ...
 */
WR_API bool wr_event_is_sensitive(const wr_event *ev);
...
WR_API int wr_event_format(const wr_event *ev, char *out, size_t size,
