...
/*
 * hackmotion/history.h — the primary data path.
 *
 ...
 * and if one is ever restarted that is a reported event (HM_EV_STREAM_RESTARTED),
 * not an implementation detail.
 */
#ifndef HACKMOTION_HISTORY_H
#define HACKMOTION_HISTORY_H
...
struct hm_session;
struct hm_session_policy; /* session.h — pointer only, so the headers stay acyclic */

/* ------------------------------------------------------------------------ */
/* Request                                                                   */
/* ------------------------------------------------------------------------ */
typedef struct hm_history_request {
     ...
     * usable margin.  hm_history_request_around() applies that default. */
    hm_time_range window;
     ...
     * PRECISION at the window exceeds this (hm_clock_error.precision_us — not
     * the total, see clock.h), the request completes with
     * HM_HIST_REFUSED_ALIGNMENT and no radio traffic at all, so the refusal
     ...
     * HM_HIST_REFUSED_ALIGNMENT — is still refused, because there the `a1`
 ...
 */
HM_API hm_history_request hm_history_request_around(const struct hm_session_policy *policy,
                                                    hm_time_us event_host_us);

/* ------------------------------------------------------------------------ */
/* Result status                                                             */
/* ------------------------------------------------------------------------ */
typedef enum hm_history_status {
    HM_HIST_COMPLETE = 0,  /* every requested index arrived, step 1           */
    HM_HIST_SHORT,         /* contiguous but narrower than requested          */
    HM_HIST_HOLED,         /* ⚠ the device's main failure mode — gaps INSIDE  */
    HM_HIST_TIMED_OUT,     /* deadline hit; partial content is still valid    */
    HM_HIST_CANCELLED,
    /*
     ...
     */
    HM_HIST_REFUSED_ALIGNMENT,
    HM_HIST_EVICTED,       /* the range had already left the buffer           */
    ...
    HM_HIST_LINK_LOST,
    HM_HIST_NOT_ALIGNABLE, /* legacy 0x7f stream: `a1` addresses a header that
     ...
     */
    HM_HIST_ERROR,
    HM_HISTORY_STATUS_COUNT
} hm_history_status;

HM_API const char *hm_history_status_name(hm_history_status status);

/* ------------------------------------------------------------------------ */
/* Gaps — and there are two kinds that must not be conflated (C7)            */
/* ------------------------------------------------------------------------ */
typedef enum hm_gap_kind {
    /*
     ...
     * uncertainty says so.
     *
     * ⚠ Across a RETRIEVAL this overlaps HM_GAP_NOT_RECORDED and the stronger
...
typedef struct hm_gap {
    hm_time_range  span;
    hm_index_range indices;
    uint8_t        kind;     /* hm_gap_kind */
    uint8_t        reserved[7];
} hm_gap;

HM_API const char *hm_gap_kind_name(hm_gap_kind kind);

/* ------------------------------------------------------------------------ */
/* Calibration span (api-request C10)                                        */
/* ------------------------------------------------------------------------ */
/*
 * ⚠ THIS IS THE ONE PLACE HM_CAL_LOST IS EVER WRITTEN.  No hm_sample carries it
 ...
 */
typedef struct hm_calibration_span {
    uint8_t state_at_start;  /* hm_calibration_state */
 ...
 * is immutable from the moment hm_history_collect() hands it over.
 *
 * Release it with hm_history_block_release().  That is the only cleanup.
 *
 ...
 */
typedef struct hm_history_block {
    uint32_t layout_version;   /* HM_SAMPLE_LAYOUT_VERSION when written */
    uint32_t sample_stride;    /* sizeof(hm_sample) in the producing build */

    const hm_sample *samples;  /* owned by the block; POD; stride as above */
    size_t           sample_count;

    uint8_t          status;   /* hm_history_status */
    uint8_t          attempts; /* how many `a1` requests were issued */
    /*
     * ⚠ THE INTERVAL LIST BELOW IS A SUPERSET, NOT THE TRUTH, WHEN THIS IS SET.
     *
     * `hm_coverage` uses fixed caller-provided storage (hm_session_memory.
     ...
     *
     * ⚠ AND THE TWO RANGE TYPES DIFFER: hm_time_range is HALF-OPEN
     * [start_us, end_us); hm_index_range is INCLUSIVE [first, last].  That is
     * where an off-by-one is born, so the conversion exists exactly once, in
     * hm_clock_index_range_for_time().  Do not derive one from the other by
     * hand.
     */
    hm_time_range    requested;         /* half-open */
    hm_index_range   requested_indices; /* inclusive */
    const hm_time_range  *delivered;        /* owned; ascending, disjoint */
    size_t                delivered_count;
    const hm_index_range *delivered_indices;/* owned; same order */

    /*
     * Owned; ascending by first index, and ⚠ the three kinds MAY OVERLAP — see
     * hm_gap_kind.  A span the device never recorded is also a span it never
     ...
     * consecutive delivered indices (hm_sample_step_density below).  §7.3's two
     ...
     * MEASURABLE, never "empty": see hm_sample_step_density.
     *
     ...
     * supplied (see hm_session_memory).  Never read a zero mismatch count
     ...
     */
    hm_time_range self_recording_gap;

    /* Provenance — everything a consumer writes into swing.json (C13). */
    hm_clock_snapshot   fit;            /* immutable, carried WITH the data (C9) */
    hm_calibration_span calibration;    /* covering the block's span (C10)       */
    hm_stream_config    config;         /* the byte that produced these samples  */
    uint8_t             reserved2[7];
    hm_pinned_counts    pinned;         /* ⚠ silent int16 saturation             */

    hm_time_us   requested_at_us;
    hm_time_us   completed_at_us;
} hm_history_block;
...
/* The widest typical spacing hm_sample_step_density() will characterise.  Well
 * past §7.3's floor of 8, which no step exceeded across 17,739 measured ones. */
#define HM_DENSITY_STEP_MAX 256u
 ...
 * index or that carry HM_SAMPLE_INDEX_MISSING; or a median gap wider than
 * HM_DENSITY_STEP_MAX, which is fewer than one sample in 256 indices.  Read it
 * beside `sample_count` and `delivered[]`, which say which.
 *
 * Samples must be ascending, strictly monotonic in `sample_index` and
 * deduplicated — which is exactly what hm_history_block guarantees (C3).
 */
HM_API double hm_sample_step_density(const hm_sample *samples, size_t count);
 ...
 *
 * Returns HM_OK and an id.  Reserving does not block and issues no radio
 * traffic before the window's last sample can exist.
 *
 * ⚠ Validated at RESERVE time, not discovered at deadline time.  Returns
 * HM_ERR_INVALID_ARG for a request that cannot possibly succeed:
 *
 ...
 *
 * ⚠ Also returns HM_ERR_NO_FIT if no live frame has EVER arrived on the current
 ...
 * yields HM_HIST_REFUSED_ALIGNMENT on the block instead.
 *
 ...
 *
 *   - HM_ERR_BUFFER_TOO_SMALL — the window is wider than the gather area
 *     (hm_session_memory.history_gather).  ⚠ Refused, never truncated: a
 *     silently clipped window returns a block that looks complete and is not.
 *   - HM_ERR_BUSY — every reservation slot is taken.  Requests are serialised
 *     (a second `a1` cannot be issued until the first completes), so the queue
 *     is deliberately short.
 */
HM_API hm_status hm_history_reserve(struct hm_session *session,
                                    const hm_history_request *request,
                                    uint64_t *out_request_id);

/*
 * HM_PENDING while in flight; HM_OK with `*out_block` set once complete,
 ...
 */
HM_API hm_status hm_history_collect(struct hm_session *session,
                                    uint64_t request_id,
                                    hm_history_block **out_block);

/* Cancel a reservation.  The block still materialises, with whatever arrived
 * and HM_HIST_CANCELLED, so a capture records what it got.
 *
 * ⚠ hm_session_close() does the same to every outstanding reservation:
 * each one materialises with HM_HIST_CANCELLED and whatever had arrived, and
 * stays collectable until hm_session_destroy(), which releases any block the
 * caller never took.  A capture should record what it got even when the answer
 * is nothing. */
HM_API hm_status hm_history_cancel(struct hm_session *session, uint64_t request_id);
 ...
 *
 *  1. hm_history_block_release() may be called from ANY thread, at any time,
 *     including after hm_session_destroy().
 ...
 */
HM_API void hm_history_block_release(hm_history_block *block);
 ...
 * warns about via HM_EV_HISTORY_EVICTION_RISK rather than silently returning a
 * holed set for the second shot. */
HM_API size_t hm_history_pending(const struct hm_session *session);
 ...
 *
 *   HM_OK              — the width is what the device has ACTUALLY SERVED this
 ...
 *   HM_PENDING         — nothing has been SERVED yet, so no residency has been
 *                        verified.  `*out_range` carries the best estimate
 *                        available: HM_HISTORY_DEPTH_SEED_US, narrowed by any
 ...
 *                        HM_OK.
 *   HM_ERR_NO_STREAM   — §7.4: the device only buffers while streaming.
 *   HM_ERR_NO_FIT      — no mapping from an index to a host time.  ⚠ Expected
 ...
 * HM_WARN_HISTORY_DEPTH_CONFLICT fires.  That is §7.3's open question answering
 ...
 * HM_HIST_EVICTED.
 */
HM_API hm_status hm_history_resident_range(const struct hm_session *session,
                                           hm_time_range *out_range);

/*
 * True if the whole of [start,end) is inside the resident estimate.
 *
 * ⚠ ANSWERS ONLY FROM THE MEASURED CASE — hm_history_resident_range()'s HM_OK.
 ...
 */
HM_API bool hm_history_coverage_available(const struct hm_session *session,
                                          hm_time_us start_us,
                                          hm_time_us end_us);
