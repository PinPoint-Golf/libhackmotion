...
/*
 * hackmotion/clock.h — the device→host clock mapping, and an honest account of
 * how wrong it is.
 *
 ...
 * recorded.
 *
 * A hm_clock_snapshot is a self-contained immutable value (api-request C9).  It
 ...
 */
#ifndef HACKMOTION_CLOCK_H
#define HACKMOTION_CLOCK_H
 ...
 */
#define HM_NOMINAL_SAMPLE_RATE_HZ   799.2
#define HM_RATE_PLAUSIBLE_MIN_HZ    795.0
#define HM_RATE_PLAUSIBLE_MAX_HZ    803.0
...
#define HM_NOMINAL_TICKS_PER_SAMPLE 80.166
#define HM_TICK_MODULUS             65536u
 ...
 *
 * So the library reports it SEPARATELY from precision — see hm_clock_error —
 * and a consumer that measures its own rig feeds both halves back through
 * hm_session_set_clock_correction().
 */
#define HM_ACCURACY_DRIFT_US_PER_S_DEFAULT 2200.0   /* = 2.2 ms per second */

/* ------------------------------------------------------------------------ */
/* Snapshot                                                                  */
/* ------------------------------------------------------------------------ */
#define HM_PROVENANCE_MAX 64
 ...
 *
 * HM_CLOCK_HAS_FIT is set from the very first live frame.  With one observation
 ...
 * nominal 799.2 Hz — fits only the offset, and raises HM_CLOCK_SHORT_BASELINE.
 ...
 * regimes is visible in hm_clock_error_at() — SHORT_BASELINE adds a
 ...
 */
#define HM_CLOCK_RATE_FIT_MIN_SPAN_US ((hm_time_us)2 * 1000 * 1000)

typedef enum hm_clock_flag {
    HM_CLOCK_HAS_FIT       = 1u << 0, /* a usable fit exists                        */
    /*
     ...
     */
    HM_CLOCK_RATE_POOLED   = 1u << 1,
    HM_CLOCK_DEGENERATE    = 1u << 2, /* ⚠ envelope support collapsed to one end     */
    HM_CLOCK_RATE_IMPLAUSIBLE = 1u << 3, /* fitted rate outside the plausible band   */
    HM_CLOCK_STALE         = 1u << 4, /* no live observation for a long time         */
    HM_CLOCK_BLIND         = 1u << 5, /* a retrieval is in flight; live is suspended */
    HM_CLOCK_EXTERNAL_CORRECTION = 1u << 6, /* a consumer-supplied ppm is in force   */
    HM_CLOCK_SHORT_BASELINE = 1u << 7 /* span too short to separate rate from offset */
} hm_clock_flag;
 ...
 * host clock's epoch.
 */
typedef struct hm_clock_snapshot {
    uint64_t   stream_id;
    uint32_t   flags;              /* hm_clock_flag */
    int32_t    observations;       /* live frames contributing to this fit */

    uint32_t   anchor_index;
    uint32_t   reserved0;
    hm_time_us anchor_host_us;
    ...
    hm_time_us offset_us;          /* host_us(0); the lower-envelope estimate, reported */
    hm_time_us span_us;            /* how long a baseline the fit rests on */
    hm_time_us last_observation_us;/* stale mappings must be visible as stale */
 ...
 * Returns HM_TIME_UNKNOWN if the snapshot carries no fit.
 */
HM_API hm_time_us hm_clock_to_host_us(const hm_clock_snapshot *fit, uint32_t sample_index);

/*
 * The inverse, for sizing an `a1` request from a host-time window.
 * Returns HM_ERR_NO_FIT with no fit, HM_ERR_INVALID_ARG if the result would
 * fall outside the unwrapped index space.
 */
HM_API hm_status hm_clock_index_for_host_us(const hm_clock_snapshot *fit,
                                            hm_time_us host_us,
                                            uint32_t *out_index);
 ...
 *                  hm_clock_meets_budget() accept any budget at all.  Design
 ...
 * than informing it.  hm_clock_meets_budget() gates on precision for exactly
...
HM_API hm_clock_error hm_clock_error_at(const hm_clock_snapshot *fit, uint32_t sample_index);

/* Convenience accessors for the two halves.  Same numbers, one at a time. */
HM_API uint32_t hm_clock_precision_us(const hm_clock_snapshot *fit, uint32_t sample_index);
HM_API uint32_t hm_clock_uncertainty_us(const hm_clock_snapshot *fit, uint32_t sample_index);
...
HM_API bool hm_clock_meets_budget(const hm_clock_snapshot *fit,
 ...
 *
 * ⚠ Note the deliberate asymmetry with hm_session_policy, where 0 means "use
 ...
 *
 * A consumer that has measured only the rate sets only HM_CORRECTION_RATE, and
 * the systematic keeps whatever value the policy gave it.
 *
 * Everything set here is carried in every subsequent hm_clock_snapshot, and
 * therefore into every history block, so a re-analysis a year later reproduces
 * the same alignment (api-request C9).
 */
typedef enum hm_correction_field {
    HM_CORRECTION_RATE  = 1u << 0, /* apply rate_ppm                */
    HM_CORRECTION_DRIFT = 1u << 1  /* apply residual_drift_us_per_s */
} hm_correction_field;

typedef struct hm_clock_correction {
    uint32_t fields;                 /* bitmask; 0 → HM_ERR_INVALID_ARG              */
    uint32_t reserved;
 ...
 * OFF-BY-ONE IS BORN.  hm_time_range is HALF-OPEN [start, end); hm_index_range
 ...
 * nothing else.
 *
 * Use it rather than calling hm_clock_index_for_host_us() twice.
 *
 * Returns HM_ERR_NO_FIT with no fit, HM_ERR_INVALID_ARG if the window is empty,
 * inverted, or maps outside the unwrapped index space.
 */
HM_API hm_status hm_clock_index_range_for_time(const hm_clock_snapshot *fit,
                                               hm_time_range window,
                                               hm_index_range *out);

HM_API const char *hm_clock_flag_name(hm_clock_flag flag);

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* HACKMOTION_CLOCK_H */
