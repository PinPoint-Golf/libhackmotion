...
/*
 * hackmotion/sample.h — the one sample type, live and history alike.
 *
 ...
 * struct grows.
 */
#ifndef HACKMOTION_SAMPLE_H
#define HACKMOTION_SAMPLE_H
 ...
 * every hm_sample_block so a persisted capture stays readable. */
#define HM_SAMPLE_LAYOUT_VERSION 1u
...
/* Scales (spec §6.4)                                                        */
/* ------------------------------------------------------------------------ */
#define HM_QUAT_SCALE            16384.0f  /* Q14 */
#define HM_QUAT_NORM_NOMINAL     16384.7f  /* measured over 6,064 records, ±0.41 */
/*
 ...
 */
#define HM_QUAT_NORM_TOLERANCE      64.0f
#define HM_ACCEL_LSB_MPS2         0.0098f  /* 1 LSB = 1 mg */
#define HM_ACCEL_FULL_SCALE_MPS2   321.0f  /* ±32.8 g */
#define HM_PIN_THRESHOLD           32767   /* |count| >= this is a pinned sample */

/* ------------------------------------------------------------------------ */
/* One unit's reading                                                        */
/* ------------------------------------------------------------------------ */
typedef struct hm_unit_sample {
    /*
     ...
     * Order is w, x, y, z.
     *
     * ⚠⚠ THE HAZARD IS HERE, NOT IN hm_quat_relative().  Reading these two
     ...
     *
     *   - hm_quat_relative() is the blessed path.  Use it.
     ...
     *     two are related by
     *
     *         q_rel_inverse_first = q_arm* ⊗ q_rel_hackmotion ⊗ q_arm
     *
    ...
    uint8_t  has_ticks;
    uint8_t  pinned_mask;         /* bit per hm_channel that hit HM_PIN_THRESHOLD */
     ...
     * history).  HM_TIME_UNKNOWN only when the configuration carries no ticks.
     *
     ...
     */
    hm_time_us device_time_us;
} hm_unit_sample;

/* ------------------------------------------------------------------------ */
/* Sample flags and provenance                                               */
/* ------------------------------------------------------------------------ */
typedef enum hm_sample_source {
    HM_SOURCE_LIVE    = 0,  /* arrival is real time; these pairs ARE the clock fit */
    HM_SOURCE_HISTORY = 1,  /* arrival is meaningless; host time comes from the fit */
    HM_SOURCE_REPLAY  = 2   /* decoded from a wire recording */
} hm_sample_source;
 ...
 * hm_calibration_confirm_reference_pose() leaves every later sample here.
 *
 * ⚠ NO SAMPLE EVER CARRIES HM_CAL_LOST, and that is a decision rather than an
 ...
 *
 * The value exists for `hm_calibration_span` on a history block (history.h),
 ...
 */
typedef enum hm_calibration_state {
    HM_CAL_UNKNOWN      = 0,
    HM_CAL_UNCALIBRATED = 1,
    HM_CAL_CALIBRATED   = 2,
    HM_CAL_LOST         = 3
} hm_calibration_state;

typedef enum hm_sample_flag {
    HM_SAMPLE_PINNED               = 1u << 0, /* some channel saturated — silently */
    HM_SAMPLE_NOT_TIME_ALIGNABLE   = 1u << 1, /* stream carries no device clock (§6.3.1) */
    HM_SAMPLE_NONSTANDARD_CONFIG   = 1u << 2, /* not 0x7e — relative-angle cancellation unmeasured */
    HM_SAMPLE_HOST_TIME_EXTRAPOLATED = 1u << 3, /* mapped beyond the fit's observations */
    HM_SAMPLE_NO_FIT               = 1u << 4, /* host_time_us is HM_TIME_UNKNOWN */
    HM_SAMPLE_QUAT_NORM_SUSPECT    = 1u << 5, /* |q| outside tolerance — decode may be misaligned */
    HM_SAMPLE_TICKS_MISSING        = 1u << 6, /* no per-unit device time available */
    HM_SAMPLE_INDEX_MISSING        = 1u << 7  /* legacy 0x7f layout: no record header */
} hm_sample_flag;
...
typedef struct hm_sample {
    ...
    uint8_t    source;           /* hm_sample_source */
    uint8_t    calibration;      /* hm_calibration_state, at the instant of capture */

    uint16_t   flags;            /* hm_sample_flag */
    ...
    /* Arrival instant as the transport stamped it.  HM_TIME_UNKNOWN for
     * history records: bulk arrival timestamps carry no information (§10). */
    hm_time_us host_recv_us;

    /*
     * ⚠ TWO NUMBERS, NOT ONE (api-request B2).  See hm_clock_error.
...
HM_API const hm_unit_sample *hm_sample_unit(const hm_sample *s, hm_unit unit);

/* Human-readable unit name: "lower_arm" / "palm".  Never NULL. */
HM_API const char *hm_unit_name(hm_unit unit);
 ...
 */
typedef struct hm_pinned_counts {
    uint32_t n[HM_UNIT_COUNT][HM_CHANNEL_COUNT];
    uint32_t total;
} hm_pinned_counts;

HM_API const char *hm_channel_name(hm_channel ch);
HM_API void hm_pinned_counts_reset(hm_pinned_counts *counts);
HM_API void hm_pinned_counts_add(hm_pinned_counts *counts, const hm_sample *s);

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* HACKMOTION_SAMPLE_H */
