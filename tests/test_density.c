...
static size_t fill_step(hm_sample *out, uint32_t first, uint32_t last, uint32_t step)
{
    size_t n = 0;
 ...
 */
HM_TEST(density_reads_step_one_as_the_full_internal_rate)
{
    hm_sample s[MAX_SAMPLES];
    size_t    n = fill_step(s, 1000u, 1399u, 1u);

    HM_ASSERT_EQ(n, 400);
    HM_ASSERT_NEAR(hm_sample_step_density(s, n), 1.0, 1e-12);
}

HM_TEST(density_reads_the_at_rest_floor_as_one_eighth)
{
    hm_sample s[MAX_SAMPLES];
    ...
    HM_ASSERT_NEAR(hm_sample_step_density(s, n), 0.125, 1e-12);
}
 ...
 */
HM_TEST(density_separates_a_short_reply_from_one_at_the_hundred_hertz_floor)
{
    hm_sample short_reply[MAX_SAMPLES];
    hm_sample at_rest[MAX_SAMPLES];
    ...
    HM_ASSERT_EQ(n_short, 50);
    HM_ASSERT_EQ(n_rest, 50);

    HM_ASSERT_NEAR(hm_sample_step_density(short_reply, n_short), 1.0, 1e-12);
    HM_ASSERT_NEAR(hm_sample_step_density(at_rest, n_rest), 0.125, 1e-12);
}
 ...
 */
HM_TEST(density_is_the_spacing_and_not_the_average_rate)
{
    hm_sample two_runs[MAX_SAMPLES];
    hm_sample uniform[MAX_SAMPLES];
    size_t    a = fill_step(two_runs, 7000u, 7115u, 1u);
    size_t    b;

    a += fill_step(&two_runs[a], 7284u, 7399u, 1u);
    HM_ASSERT_EQ(a, 232);
    HM_ASSERT_NEAR(hm_sample_step_density(two_runs, a), 1.0, 1e-12);

    b = fill_step(uniform, 7000u, 7399u, 2u);
    HM_ASSERT_EQ(b, 200);
    HM_ASSERT_NEAR(hm_sample_step_density(uniform, b), 0.5, 1e-12);
}
 ...
 */
HM_TEST(density_takes_the_sparser_of_two_middle_gaps)
{
    hm_sample s[4];
    memset(s, 0, sizeof(s));
    ...
    HM_ASSERT_NEAR(hm_sample_step_density(s, 3), 0.125, 1e-12);
}
 ...
 */
HM_TEST(density_refuses_rather_than_guessing)
{
    hm_sample s[MAX_SAMPLES];
    size_t    n;
    ...
    HM_ASSERT_NEAR(hm_sample_step_density(NULL, 10), 0.0, 1e-12);
    HM_ASSERT_NEAR(hm_sample_step_density(s, 0), 0.0, 1e-12);
    /* One sample cannot show a spacing. */
    HM_ASSERT_NEAR(hm_sample_step_density(s, 1), 0.0, 1e-12);

    /* Not strictly ascending in device index (api-request C3). */
    s[1].sample_index = 100u;
    HM_ASSERT_NEAR(hm_sample_step_density(s, 2), 0.0, 1e-12);
    s[1].sample_index = 99u;
    HM_ASSERT_NEAR(hm_sample_step_density(s, 2), 0.0, 1e-12);

    /* An index the frame never carried is not a spacing of zero. */
    s[1].sample_index = 101u;
    s[1].flags = (uint16_t)HM_SAMPLE_INDEX_MISSING;
    HM_ASSERT_NEAR(hm_sample_step_density(s, 2), 0.0, 1e-12);

    /* Wider than the widest spacing this can characterise: fewer than one
     * sample in HM_DENSITY_STEP_MAX indices. */
    n = fill_step(s, 0u, 100000u, HM_DENSITY_STEP_MAX + 1u);
    HM_ASSERT(n > 2);
    HM_ASSERT_NEAR(hm_sample_step_density(s, n), 0.0, 1e-12);

    /* ...and exactly at the limit it still answers. */
    n = fill_step(s, 0u, 100000u, HM_DENSITY_STEP_MAX);
    HM_ASSERT(n > 2);
    HM_ASSERT_NEAR(hm_sample_step_density(s, n), 1.0 / (double)HM_DENSITY_STEP_MAX, 1e-12);
}

/*
 * ⚠ One wide hole does not move the spacing, and that is the point: the swing
 * pulls in `swings.hmwire` read 1.000 with 34-43 gaps apiece because the
 ...
 */
HM_TEST(density_is_unmoved_by_a_single_wide_hole_which_the_gap_list_carries)
{
    hm_sample s[MAX_SAMPLES];
