 ...
 */
static hm_sample sample_wobbled(float rel_deg, float wobble_deg, uint32_t index,
                                uint16_t arm_ticks, uint16_t palm_ticks)
{
    uint8_t buf[64];
    hm_wire_block arm = hm_wire_identity_block(arm_ticks);
    hm_wire_block palm = hm_wire_identity_block(palm_ticks);
    ...
    hm_quat_multiply(q_base, q_wob, q_arm);
    hm_quat_normalise(q_arm);

    q_rel[0] = cosf(hr);
    q_rel[1] = sinf(hr);
    q_rel[2] = 0.0f;
    q_rel[3] = 0.0f;
    hm_quat_multiply(q_rel, q_arm, q_palm);
    hm_quat_normalise(q_palm);
...
static hm_sample sample_about(float deg, char axis, uint32_t index, uint16_t arm_ticks,
                              uint16_t palm_ticks)
{
    uint8_t buf[64];
    hm_wire_block arm = hm_wire_identity_block(arm_ticks);
...
static hm_sample sample_at(float deg, uint32_t index, uint16_t arm_ticks, uint16_t palm_ticks)
{
    uint8_t buf[64];
    hm_wire_block arm = hm_wire_identity_block(arm_ticks);
    hm_wire_block palm = hm_wire_identity_block(palm_ticks);
...
HM_TEST(presence_classifies_the_measured_populations)
{
    /* Calibration applied: 0.36° / 0.79°, and 3.73-3.80° holding the pose. */
    HM_ASSERT_EQ(hm_presence_classify(0.36f), HM_CAL_CALIBRATED);
    HM_ASSERT_EQ(hm_presence_classify(0.79f), HM_CAL_CALIBRATED);
    HM_ASSERT_EQ(hm_presence_classify(3.80f), HM_CAL_CALIBRATED);

    /* Uncalibrated, or after a power cycle: 14.36-15.76°.
     * After a plain disconnect: 18.80°. */
    HM_ASSERT_EQ(hm_presence_classify(15.01f), HM_CAL_UNCALIBRATED);
    HM_ASSERT_EQ(hm_presence_classify(18.80f), HM_CAL_UNCALIBRATED);
    ...
    HM_ASSERT_EQ(hm_presence_classify(8.0f), HM_CAL_UNKNOWN);
    HM_ASSERT_EQ(hm_presence_classify(NAN), HM_CAL_UNKNOWN);

    /* The thresholds sit inside the gap, with margin on both sides. */
    HM_ASSERT(HM_PRESENCE_CALIBRATED_MAX_DEG > 3.80f);
    HM_ASSERT(HM_PRESENCE_ABSENT_MIN_DEG < 14.36f);
}
 ...
 */
HM_TEST(presence_keeps_the_reference_pose_anchor)
{
    hm_sample run[5];
    hm_calibration_presence_event ev;
    ...
    HM_ASSERT_EQ(hm_presence_select_reference(run, 5, &ev), HM_OK);
    HM_ASSERT_EQ(ev.samples_used, 5);
    HM_ASSERT_NEAR(ev.relative_angle_deg, 12.76f, 0.3);
    ...
    HM_ASSERT_MSG(chosen != (size_t)-1, "the anchor must be one of the measured records");
    HM_ASSERT_EQ(ev.skew_us, 921);
     ...
     * the angle was computed from — an averaged pair could not promise this. */
    {
        ...
        HM_ASSERT_NEAR(hm_relative_angle_deg(&rebuilt), ev.relative_angle_deg, 1e-4);
    }

    /* The quaternions are the ones that were measured, verbatim. */
    HM_ASSERT_NEAR(ev.q_lower_arm[0], run[chosen].lower_arm.q_world_to_body[0], 1e-9);
    HM_ASSERT_NEAR(ev.q_palm[1], run[chosen].palm.q_world_to_body[1], 1e-9);

    /* And it is the record nearest the run's centre, not an arbitrary one. */
    HM_ASSERT_MSG(chosen != 3 && chosen != 4, "the medoid must not be an extreme of the run");
}
 ...
 * calibrations on it.
 */
HM_TEST(presence_survives_the_sub_degree_quantisation_floor)
{
    hm_sample run[16];
    hm_calibration_presence_event ev;
    ...
    HM_ASSERT_EQ(hm_presence_select_reference(run, 16, &ev), HM_OK);
    ...
    HM_ASSERT_MSG(ev.state == HM_CAL_CALIBRATED,
                  "a sub-degree pose must still classify as calibrated");
    HM_ASSERT(ev.relative_angle_deg < HM_PRESENCE_CALIBRATED_MAX_DEG);

    /* And the anchor is still a real, usable pair regardless of the noise. */
    HM_ASSERT(ev.sample_index >= 100u);
    {
        float n2 = 0.0f;
        for (int k = 0; k < 4; ++k) {
            n2 += ev.q_palm[k] * ev.q_palm[k];
        }
        HM_ASSERT_NEAR(sqrtf(n2), 1.0f, 1e-3);
    }
}
 ...
 * ANGLES implementation, which is the biased estimator hm_presence.c exists to
 ...
 */
HM_TEST(presence_uses_the_rotation_medoid_not_the_angle_median)
{
    hm_sample run[5];
    hm_calibration_presence_event ev;
    float angles[5];
    ...
    for (size_t i = 0; i < 5; ++i) {
        angles[i] = hm_relative_angle_deg(&run[i]);
    }
    for (size_t i = 0; i < 5; ++i) {
        size_t below = 0;
    ...
    HM_ASSERT_MSG(angle_median_i == 2, "the fixture must make the angle median the outlier");

    HM_ASSERT_EQ(hm_presence_select_reference(run, 5, &ev), HM_OK);
    HM_ASSERT_MSG(ev.sample_index != run[2].sample_index,
     ...
     */
    {
        ...
        hm_quat_relative(&chosen, rel);
        HM_ASSERT_MSG(fabsf(rel[1]) > 0.05f, "the medoid must be one of the x-axis cluster");
        HM_ASSERT_MSG(fabsf(rel[3]) < 0.01f, "and not the z-axis outlier");
    }
}
 ...
 */
HM_TEST(presence_averages_the_absolute_pose_and_reports_the_wobble)
{
    hm_sample run[9];
    hm_calibration_presence_event ev;
    ...
    HM_ASSERT_EQ(hm_presence_select_reference(run, 9, &ev), HM_OK);
    ...
    first_angle = hm_relative_angle_deg(&run[0]);
    for (size_t i = 1; i < 9; ++i) {
        float d = fabsf(hm_relative_angle_deg(&run[i]) - first_angle);
        if (d > rel_spread) {
            rel_spread = d;
        }
    }
    HM_ASSERT_MSG(rel_spread < 0.1f, "the fixture's wobble must be common-mode");

    /* The spread names the wobble the medoid is blind to. */
    HM_ASSERT_NEAR(ev.pose_spread_deg[HM_UNIT_LOWER_ARM], 1.5, 0.2);
    HM_ASSERT_NEAR(ev.pose_spread_deg[HM_UNIT_PALM], 1.5, 0.2);
     ...
     * to it than the medoid record is. */
    {
        ...
        HM_ASSERT_NEAR(sqrtf(n_arm), 1.0f, 1e-5);
...
HM_TEST(presence_resolves_the_uncalibrated_population_precisely)
{
    hm_sample run[8];
    hm_calibration_presence_event ev;
    ...
    HM_ASSERT_EQ(hm_presence_select_reference(run, 8, &ev), HM_OK);
    HM_ASSERT_NEAR(ev.relative_angle_deg, 15.01f, 0.2);
    HM_ASSERT_EQ(ev.state, HM_CAL_UNCALIBRATED);
}
 ...
 */
HM_TEST(presence_reports_an_uncalibrated_run_with_its_anchor)
{
    hm_sample run[3];
    hm_calibration_presence_event ev;
    ...
    HM_ASSERT_EQ(hm_presence_select_reference(run, 3, &ev), HM_OK);
    HM_ASSERT_NEAR(ev.relative_angle_deg, 15.0f, 0.5);
    HM_ASSERT_EQ(ev.state, HM_CAL_UNCALIBRATED);
    HM_ASSERT_EQ(ev.sample_index, 18);
    HM_ASSERT(ev.q_palm[1] != 0.0f);
}

HM_TEST(presence_selection_is_defensive)
{
    hm_sample one = sample_at(1.0f, 5, 100, 159);
    hm_calibration_presence_event ev;

    /* A single sample is a legitimate run of one. */
    HM_ASSERT_EQ(hm_presence_select_reference(&one, 1, &ev), HM_OK);
    HM_ASSERT_EQ(ev.samples_used, 1);
    HM_ASSERT_EQ(ev.sample_index, 5);

    HM_ASSERT_EQ(hm_presence_select_reference(NULL, 3, &ev), HM_ERR_INVALID_ARG);
    HM_ASSERT_EQ(hm_presence_select_reference(&one, 0, &ev), HM_ERR_INVALID_ARG);
    HM_ASSERT_EQ(hm_presence_select_reference(&one, 1, NULL), HM_ERR_INVALID_ARG);
}

/* An over-long run is capped rather than overrunning the working arrays. */
HM_TEST(presence_caps_an_overlong_run)
{
    hm_sample run[HM_PRESENCE_MAX_SAMPLES + 16u];
    hm_calibration_presence_event ev;

    for (uint32_t i = 0; i < HM_PRESENCE_MAX_SAMPLES + 16u; ++i) {
        run[i] = sample_at(1.0f + (float)i * 0.01f, i, (uint16_t)(1000u + i),
                           (uint16_t)(1059u + i));
    }
    HM_ASSERT_EQ(hm_presence_select_reference(run, HM_PRESENCE_MAX_SAMPLES + 16u, &ev), HM_OK);
    HM_ASSERT_EQ(ev.samples_used, HM_PRESENCE_MAX_SAMPLES);
    HM_ASSERT(ev.sample_index < HM_PRESENCE_MAX_SAMPLES);
}
