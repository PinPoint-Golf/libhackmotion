...
|---|---|---|---|---|
| `swings.wrwire` | 204 K | 46 s | five golf swings, **six mid-stream retrievals** | the whole of history retrieval — §7.3's step-1 runs and step-8 floor, §7.5's stall, §8.8's live-vs-history agreement |
| `session1.wrwire` | 368 K | 174 s | a calibration choreography, one retrieval **past two index wraps** | §8.2's `0x94` payload; §7.3's **100 Hz floor** over a still wrist; and §7.4's re-wrap above 131,072 |
| `smoke.wrwire` | 95 K | 32 s | live only, hand motion, **no retrieval at all** | §7.5's pull-free tick ratio (80.138), and the "nothing was checked" path |
...
specific owner (api-request §2.13). `tools/wr_capture.py` redacts them by
...
```sh
./build/dev/wrwire info FILE.wrwire | grep identifiers    # says "redacted"
./build/dev/wrwire dump FILE.wrwire --hex | grep -A1 'REDACTED'
strings -n 6 FILE.wrwire | grep -E '([0-9A-F]{2}:){5}[0-9A-F]{2}|^WG3[0-9]'
```
...
`.gitignore` still excludes `*.wrwire` everywhere else and un-excludes only this
...
- `wrwire verify` — the container: framing, monotonic host time, no unmarked
...
- `wr_gather_replay` — ⚠ the **implementation**, driven by the device's own
  ...
  these captures, deliberately: `swings.wrwire` is the 657–728 Hz regime and
  `session1.wrwire` is the 100.5 Hz one, and a check that only ever sees one of
  them has seen half the device. ⚠ `session1.wrwire`'s first frame arrived
...
- `wrwire reconcile` — the **specification**, claim by claim. Deliberately *not*
