...
/*
 * wr_record.c — the `.wrwire` container.  Chunks in, a versioned file out, and
...
/* Defaults                                                                  */
/* ------------------------------------------------------------------------ */
wr_recording_info wr_recording_info_default(void)
{
    wr_recording_info info;
    memset(&info, 0, sizeof(info));
    info.config_bits = WR_CONFIG_OBSERVED_DEFAULT;
    info.layout_version = WR_SAMPLE_LAYOUT_VERSION;
    ...
    memcpy(info.clock, WR_RECORD_CLOCK_MONOTONIC, sizeof(WR_RECORD_CLOCK_MONOTONIC));
...
wr_status wr_recorder_open(const char *path, const wr_recording_info *info,
                           wr_recorder **out_recorder)
{
    wr_recorder *r;
    wr_recording_info local;
    ...
    *out_recorder = NULL;
    ...
    local = (info != NULL) ? *info : wr_recording_info_default();
    if (local.clock[0] == '\0') {
        memcpy(local.clock, WR_RECORD_CLOCK_MONOTONIC, sizeof(WR_RECORD_CLOCK_MONOTONIC));
    }
    if (local.layout_version == 0u) {
        local.layout_version = WR_SAMPLE_LAYOUT_VERSION;
    }

    r = (wr_recorder *)calloc(1, sizeof(*r));
    if (r == NULL) {
...
wr_status wr_recorder_write(wr_recorder *recorder, const wr_wire_chunk *chunks, size_t count)
{
    uint8_t header[WR_RECORD_ENTRY_HEADER];

    if (recorder == NULL || (chunks == NULL && count > 0u)) {
        return WR_ERR_INVALID_ARG;
    }
    for (size_t i = 0; i < count; ++i) {
        if (chunks[i].length > WR_WIRE_CHUNK_MAX) {
            return WR_ERR_INVALID_ARG;
        }
    }

    for (size_t i = 0; i < count; ++i) {
        const wr_wire_chunk *c = &chunks[i];
        ...
        if (fwrite(header, 1, sizeof(header), recorder->fp) != sizeof(header) ||
            ...
            if (recorder->first_error == WR_OK) {
                recorder->first_error = WR_ERR_INVALID_STATE;
            }
...
uint64_t wr_recorder_chunks(const wr_recorder *recorder)
{
    return (recorder != NULL) ? recorder->chunks : 0u;
}

uint64_t wr_recorder_bytes(const wr_recorder *recorder)
{
    return (recorder != NULL) ? recorder->bytes : 0u;
}

wr_status wr_recorder_close(wr_recorder *recorder)
{
    wr_status st;
    ...
    if (fflush(recorder->fp) != 0 || ferror(recorder->fp)) {
        if (st == WR_OK) {
            st = WR_ERR_INVALID_STATE;
        }
    }
    if (fclose(recorder->fp) != 0 && st == WR_OK) {
...
wr_status wr_replay_open(const char *path, wr_replay **out_replay)
{
    wr_replay *r;
    char line[WR_RECORD_HEADER_MAX];
    ...
    *out_replay = NULL;
    ...
    r = (wr_replay *)calloc(1, sizeof(*r));
    if (r == NULL) {
        return WR_ERR_NO_MEMORY;
    }
    r->info = wr_recording_info_default();
    ...
    for (;;) {
        ...
        if (len == -1) {
            fclose(r->fp);
            free(r);
            return WR_ERR_TRUNCATED; /* header never terminated */
        }
        if (len == -2) {
            fclose(r->fp);
            free(r);
        ...
        header_bytes += (size_t)len + 1u;
        if (header_bytes > WR_RECORD_HEADER_MAX) {
            fclose(r->fp);
            free(r);
            return WR_ERR_BUFFER_TOO_SMALL;
        }
...
const wr_recording_info *wr_replay_info(const wr_replay *replay)
{
    return (replay != NULL) ? &replay->info : NULL;
}

wr_status wr_replay_next(wr_replay *replay, wr_wire_chunk *out_chunk)
{
    uint8_t header[WR_RECORD_ENTRY_HEADER];
    size_t got;
    uint32_t length;
    ...
    if (got == 0u) {
        return feof(replay->fp) ? WR_DONE : WR_ERR_INVALID_STATE;
    }
...
uint64_t wr_replay_chunks_read(const wr_replay *replay)
{
    return (replay != NULL) ? replay->chunks_read : 0u;
}

void wr_replay_close(wr_replay *replay)
{
    if (replay == NULL) {
        return;
    }
