...
/*
 * hm_report.c — printing a reconciliation report.
 *
...
static void claim(FILE *out, hm_check_verdict v, const char *section, const char *name)
{
    fputs("  [", out);
    pad_to(out, hm_check_verdict_name(v), 11u);
    fputs("] ", out);
...
static void print_stat(FILE *out, const hm_stat *s, const char *units)
{
    if (s->n == 0u) {
    ...
    fprintf(out, "%.6g ± %.3g %s  [n=%llu, %.6g … %.6g]", s->mean, hm_stat_sd(s), units,
            (unsigned long long)s->n, s->min, s->max);
}

static void print_flags(FILE *out, uint32_t flags)
{
    static const hm_clock_flag all[] = {
        HM_CLOCK_HAS_FIT,          HM_CLOCK_RATE_POOLED, HM_CLOCK_DEGENERATE,
        HM_CLOCK_RATE_IMPLAUSIBLE, HM_CLOCK_STALE,       HM_CLOCK_BLIND,
        HM_CLOCK_EXTERNAL_CORRECTION, HM_CLOCK_SHORT_BASELINE};
    bool any = false;
    for (size_t i = 0; i < sizeof(all) / sizeof(all[0]); ++i) {
        if ((flags & (uint32_t)all[i]) != 0u) {
            fprintf(out, "%s%s", any ? "," : "", hm_clock_flag_name(all[i]));
            any = true;
        }
    }
    if (!any) {
...
{
    bool any = false;
    for (int c = 1; c < HM_WARN_CODE_COUNT; ++c) {
        if ((bits & (1u << (unsigned)c)) != 0u) {
            fprintf(out, "%s%s", any ? ", " : "", hm_warning_code_name((hm_warning_code)c));
            any = true;
        }
    }
    if (!any) {
...
static double seconds(hm_time_us us)
{
    return (double)us / 1e6;
}

void hm_reconcile_print(const hm_reconcile_report *rep, FILE *out)
{
    char cfgtext[HM_CONFIG_DESCRIBE_SIZE];

    if (rep == NULL || out == NULL) {
        return;
    }

    fprintf(out, "hmwire reconciliation — a capture read back against docs/specification.md\n");
    ...
    fprintf(out, "  identifiers          %s\n",
            rep->info.identifiers_recorded ? "RECORDED (0x85/0x86 in the clear)"
                                           : "redacted");
    (void)hm_stream_config_describe(rep->config, cfgtext, sizeof(cfgtext));
    ...
    if (rep->quat_norm[HM_UNIT_LOWER_ARM].n == 0u) {
        ...
        for (int u = 0; u < HM_UNIT_COUNT; ++u) {
            cont(out);
            fprintf(out, "%-9s ", hm_unit_name((hm_unit)u));
    ...
    if (rep->verdict_rate == HM_CHECK_NO_EVIDENCE && rep->fitted_rate_hz == 0.0) {
    ...
    if (!rep->tick_ratio_fitted[HM_UNIT_LOWER_ARM] || !rep->tick_ratio_fitted[HM_UNIT_PALM]) {
        ...
        fprintf(out, "lower_arm %.4f, palm %.4f  [least squares, n=%llu]\n",
                rep->ticks_per_index[HM_UNIT_LOWER_ARM], rep->ticks_per_index[HM_UNIT_PALM],
        ...
        if (!rep->rel_rate_measured) {
            ...
            fprintf(out, "relative rate palm−arm: %+.2f ± %.2f ppm  (§6.5 claims ≤%.0f)\n",
                    rep->rel_rate_ppm, rep->rel_rate_ppm_sigma, HM_ONE_RATE_CLAIM_PPM);
            if (rep->rel_rate_ppm_sigma > HM_ONE_RATE_CLAIM_PPM) {
                cont(out);
                fprintf(out, "⚠ this capture resolves only ±%.2f ppm, so it CANNOT test a "
                             "%.0f ppm claim — stream for longer\n",
                        rep->rel_rate_ppm_sigma, HM_ONE_RATE_CLAIM_PPM);
            }
        }
    }
    cont(out);
    ...
    for (int u = 0; u < HM_UNIT_COUNT; ++u) {
        fprintf(out, "%s%s ", (u == 0) ? "" : ", ", hm_unit_name((hm_unit)u));
    ...
    if (rep->steps_total == 0u) {
        ...
        for (int b = 0; b < HM_STEP_BUCKET_COUNT; ++b) {
            cont(out);
            fprintf(out, "%-12s %8llu\n", hm_step_bucket_name((hm_step_bucket)b), (unsigned long long)rep->step[b]);
        }
        cont(out);
    ...
    /* --- §6.4 headroom ---------------------------------------------------------- */
    claim(out, HM_CHECK_NOTE, "§6.4", "gyro headroom (clipping is silent)");
    fprintf(out, "peak |ω| %.0f °/s = %.0f%% of full scale\n",
            (rep->gyro_peak_dps[HM_UNIT_PALM] > rep->gyro_peak_dps[HM_UNIT_LOWER_ARM])
                ? rep->gyro_peak_dps[HM_UNIT_PALM]
                : rep->gyro_peak_dps[HM_UNIT_LOWER_ARM],
    ...
    if (rep->pinned.total > 0u) {
        fputs(" —", out);
        for (int u = 0; u < HM_UNIT_COUNT; ++u) {
            for (int ch = 0; ch < HM_CHANNEL_COUNT; ++ch) {
                if (rep->pinned.n[u][ch] > 0u) {
                    fprintf(out, " %s.%s=%u", hm_unit_name((hm_unit)u),
                            hm_channel_name((hm_channel)ch), rep->pinned.n[u][ch]);
                }
            }
        }
    }
    ...
    for (int u = 0; u < HM_UNIT_COUNT; ++u) {
        cont(out);
        fprintf(out, "%-9s |a| ", hm_unit_name((hm_unit)u));
    ...
    if (rep->host_writes < 2u) {
        ...
        if (rep->max_host_write_gap_us > (hm_time_us)300 * 1000 * 1000) {
            cont(out);
    ...
    if (rep->stream_start_latency_us == HM_TIME_UNKNOWN) {
        ...
        if (rep->stream_start_latency_us > (hm_time_us)1000 * 1000 ||
