...
/*
 * wr_reconcile.c — replay a `.wrwire` recording against the specification and
 ...
 *   verdict has a distinct WR_CHECK_NO_EVIDENCE value, and the printer refuses
 ...
 * §5 specifies for wr_session.c; getting it wrong here would feed ~4,000 bulk
 ...
 */
#define WR_FULL_RATE_GYRO_DPS 300.0
...
/* ------------------------------------------------------------------------ */
/* wr_stat — Welford, so a long capture does not lose the spread             */
/* ------------------------------------------------------------------------ */
void wr_stat_reset(wr_stat *stat)
{
    if (stat == NULL) {
        return;
    }
...
void wr_stat_add(wr_stat *stat, double value)
{
    double delta;
...
double wr_stat_sd(const wr_stat *stat)
{
...
const char *wr_check_verdict_name(wr_check_verdict verdict)
{
    switch (verdict) {
        case WR_CHECK_NO_EVIDENCE: return "no evidence";
        case WR_CHECK_MATCH:       return "match";
        case WR_CHECK_DIFFERS:     return "DIFFERS";
        case WR_CHECK_NOTE:        return "note";
        default:                   return "?";
    }
}

const char *wr_step_bucket_name(wr_step_bucket bucket)
{
    switch (bucket) {
        case WR_STEP_NONPOSITIVE:   return "<=0";
        case WR_STEP_DENSE:         return "1-7 (dense)";
        case WR_STEP_8:             return "8 (~100 Hz)";
        case WR_STEP_9_31:          return "9-31";
        case WR_STEP_32:            return "32 (~25 Hz)";
        case WR_STEP_ABOVE_32:      return ">32 (gap)";
        case WR_STEP_BUCKET_COUNT:  return "?";
        default:                    return "?";
    }
}
...
/* Lifecycle                                                                 */
/* ------------------------------------------------------------------------ */
static void begin_stream(wr_reconciler *r)
{
    wr_index_unwrapper_reset(&r->idx);
    wr_tick_unwrapper_reset(&r->tick[WR_UNIT_LOWER_ARM]);
    wr_tick_unwrapper_reset(&r->tick[WR_UNIT_PALM]);
     ...
     * keeps its pooled rate across streams and drops everything else (wr_fit.h). */
    wr_fit_begin_stream(&r->fit, r->stream_id);
}

wr_status wr_reconcile_begin(const wr_recording_info *info, wr_reconciler **out_reconciler)
{
    wr_reconciler *r;
    ...
    *out_reconciler = NULL;
    ...
    r = (wr_reconciler *)calloc(1, sizeof(*r));
    if (r == NULL) {
        return WR_ERR_NO_MEMORY;
    }
    r->skew = (int16_t *)calloc(WR_RECONCILE_SKEW_MAX, sizeof(int16_t));
    if (r->skew == NULL) {
        free(r);
        return WR_ERR_NO_MEMORY;
    }

    r->rep.info = (info != NULL) ? *info : wr_recording_info_default();
    r->rep.first_us = WR_TIME_UNKNOWN;
    r->rep.last_us = WR_TIME_UNKNOWN;
    r->rep.stream_start_latency_us = WR_TIME_UNKNOWN;
    r->rep.fast_gyro_threshold_dps = WR_FAST_GYRO_DPS;
    ...
    if (r->rep.info.config_legacy) {
        r->cfg = wr_stream_config_legacy();
    } else if (r->rep.info.config_bits == WR_CONFIG_OBSERVED_DEFAULT) {
        r->cfg = wr_stream_config_default();
    } else {
        r->cfg = wr_stream_config_nonstandard(r->rep.info.config_bits,
                                              "from the recording's file header");
    }
    r->rep.config_from_stream = false;

    wr_fit_init(&r->fit, WR_ACCURACY_DRIFT_US_PER_S_DEFAULT);
    begin_stream(r);
...
void wr_reconcile_free(wr_reconciler *reconciler)
{
    if (reconciler == NULL) {
        return;
    }
...
/* Host → device                                                             */
/* ------------------------------------------------------------------------ */
static void observe_host_write(wr_reconciler *r, const wr_wire_chunk *c)
{
    uint8_t b0;
    ...
    if (r->have_last_write && c->host_time_us >= r->last_write_us) {
        wr_time_us gap = c->host_time_us - r->last_write_us;
...
static void bucket_step(wr_reconciler *r, int64_t step)
{
    wr_step_bucket b;

    if (step <= 0) {
        b = WR_STEP_NONPOSITIVE;
    } else if (step <= 7) {
        b = WR_STEP_DENSE;
    } else if (step == 8) {
        b = WR_STEP_8;
...
static void observe_record(wr_reconciler *r, const wr_sample *s, wr_time_us recv_us)
{
    const wr_unit_sample *unit[WR_UNIT_COUNT];
    double gyro_mag[WR_UNIT_COUNT];
    double accel_mag[WR_UNIT_COUNT];
    uint32_t unwrapped = 0u;
    bool suspect = false;
    int32_t skew = 0;
    bool have_skew = false;

    unit[WR_UNIT_LOWER_ARM] = &s->lower_arm;
    unit[WR_UNIT_PALM] = &s->palm;

    for (int u = 0; u < WR_UNIT_COUNT; ++u) {
        double norm = quat_norm_raw(unit[u]->q_world_to_body_raw);
        wr_stat_add(&r->rep.quat_norm[u], norm);
        if (fabs(norm - (double)WR_QUAT_NORM_NOMINAL) > (double)WR_QUAT_NORM_TOLERANCE) {
            r->rep.quat_norm_suspect++;
        }
        accel_mag[u] = vec3_mag(unit[u]->linear_accel_mps2);
        gyro_mag[u] = vec3_mag(unit[u]->gyro_dps);
        wr_stat_add(&r->rep.accel_mag_mps2[u], accel_mag[u]);
        wr_stat_add(&r->rep.gyro_mag_dps[u], gyro_mag[u]);
        if (gyro_mag[u] > r->rep.gyro_peak_dps[u]) {
            r->rep.gyro_peak_dps[u] = gyro_mag[u];
        }
    }

    wr_pinned_counts_add(&r->rep.pinned, s);
     ...
     */
    if (gyro_mag[WR_UNIT_LOWER_ARM] > WR_FAST_GYRO_DPS ||
        gyro_mag[WR_UNIT_PALM] > WR_FAST_GYRO_DPS) {
        wr_stat_add(&r->rep.accel_palm_minus_arm_fast,
                    accel_mag[WR_UNIT_PALM] - accel_mag[WR_UNIT_LOWER_ARM]);
    }
    ...
    if (s->lower_arm.has_ticks && s->palm.has_ticks) {
        skew = wr_tick_skew(s->palm.ticks_raw, s->lower_arm.ticks_raw);
        have_skew = true;
        wr_stat_add(&r->rep.skew_ticks, (double)skew);
        r->rep.skew_total++;
        if (r->rep.skew_stored < WR_RECONCILE_SKEW_MAX) {
    ...
    if (r->bracket_open) {
        ...
        {
            double omega = (gyro_mag[WR_UNIT_LOWER_ARM] > gyro_mag[WR_UNIT_PALM])
                               ? gyro_mag[WR_UNIT_LOWER_ARM]
                               : gyro_mag[WR_UNIT_PALM];
            wr_stat_add(&r->rep.history_gyro_dps, omega);
            ...
            if (r->have_hist_prev) {
                ...
                if (step <= 2u && omega > WR_FULL_RATE_GYRO_DPS) {
    ...
    if ((s->flags & WR_SAMPLE_INDEX_MISSING) != 0u) {
    ...
    unwrapped = wr_index_unwrap(&r->idx, s->sample_index_raw, &suspect);
    if (suspect) {
    ...
    for (int u = 0; u < WR_UNIT_COUNT; ++u) {
        if (unit[u]->has_ticks) {
            uint32_t margin = 0u;
            int64_t ticks = wr_tick_unwrap(&r->tick[u], unwrapped, unit[u]->ticks_raw, &margin);
    ...
    if (r->awaiting_post_bracket && r->have_live_mark) {
        ...
        double expected = (double)d_ticks / WR_NOMINAL_TICKS_PER_SAMPLE;
        double lost_samples = expected - (double)d_index;
        double lost_ms = lost_samples / WR_NOMINAL_SAMPLE_RATE_HZ * 1000.0;
        ...
        if (lost_samples > 0.0 && fraction >= 0.5) {
            ...
            wr_stat_add(&r->rep.retrieval_stall_ms, lost_ms);
            wr_stat_add(&r->rep.retrieval_indices_lost, lost_samples);
            wr_stat_add(&r->rep.retrieval_stall_fraction, fraction);
            r->rep.stall_total_ms += lost_ms;
            r->rep.stall_total_ticks += lost_samples * WR_NOMINAL_TICKS_PER_SAMPLE;
            if (lost_samples * WR_NOMINAL_TICKS_PER_SAMPLE > r->rep.stall_worst_ticks) {
                r->rep.stall_worst_ticks = lost_samples * WR_NOMINAL_TICKS_PER_SAMPLE;
            }
        }
    ...
    wr_fit_observe(&r->fit, unwrapped, recv_us);
...
/* Device → host                                                             */
/* ------------------------------------------------------------------------ */
static void observe_notification(wr_reconciler *r, const wr_wire_chunk *c)
{
    wr_decoded dec;
    wr_status st;
    ...
    st = wr_codec_decode(c->data, c->length, r->cfg, &dec);
    ...
    switch (dec.kind) {
        ...
        case WR_MSGK_LEGACY_FRAME: {
            size_t one = 1u + wr_stream_config_record_size(r->cfg);
            size_t two = 1u + 2u * wr_stream_config_record_size(r->cfg);
...
/* ------------------------------------------------------------------------ */
void wr_reconcile_observe(wr_reconciler *r, const wr_wire_chunk *c)
{
    ...
    switch ((wr_wire_direction)c->direction) {
        case WR_WIRE_HOST_TO_DEVICE:
            observe_host_write(r, c);
            break;
        case WR_WIRE_DEVICE_TO_HOST:
...
static void finish_skew(wr_reconciler *r)
{
...
static void decide_verdicts(wr_reconcile_report *rep)
{
    /* --- §6.3 framing ---------------------------------------------------- */
    if (rep->notif_one_record + rep->notif_two_records + rep->notif_other_len == 0u) {
        rep->verdict_frame_length = WR_CHECK_NO_EVIDENCE;
    } else if (rep->notif_other_len == 0u) {
        rep->verdict_frame_length = WR_CHECK_MATCH;
    } else {
        rep->verdict_frame_length = WR_CHECK_DIFFERS;
    }

    /* --- §6.4 the quaternion norm ----------------------------------------- */
    if (rep->quat_norm[WR_UNIT_LOWER_ARM].n == 0u) {
        rep->verdict_quat_norm = WR_CHECK_NO_EVIDENCE;
    } else {
        ...
        for (int u = 0; u < WR_UNIT_COUNT; ++u) {
            ...
            if (fabs(rep->quat_norm[u].mean - (double)WR_QUAT_NORM_NOMINAL) > 4.0) {
                ok = false;
            }
        }
        rep->verdict_quat_norm = ok ? WR_CHECK_MATCH : WR_CHECK_DIFFERS;
    }

    /* --- §6.5 the rate ---------------------------------------------------- */
    if (rep->fit.observations <= 0 || (rep->fit.flags & WR_CLOCK_HAS_FIT) == 0u) {
        rep->verdict_rate = WR_CHECK_NO_EVIDENCE;
    } else if ((rep->fit.flags & WR_CLOCK_SHORT_BASELINE) != 0u) {
        /* ⚠ Below WR_FIT_MIN_SPAN_US the rate cannot be separated from the
         * offset at all.  That is not a small answer, it is no answer. */
        rep->verdict_rate = WR_CHECK_NO_EVIDENCE;
    } else if ((rep->fit.flags & WR_CLOCK_DEGENERATE) != 0u) {
        rep->verdict_rate = WR_CHECK_DIFFERS;
    } else if (rep->fitted_rate_hz >= WR_RATE_PLAUSIBLE_MIN_HZ &&
               rep->fitted_rate_hz <= WR_RATE_PLAUSIBLE_MAX_HZ) {
        rep->verdict_rate = WR_CHECK_MATCH;
    } else {
        rep->verdict_rate = WR_CHECK_DIFFERS;
    }
    ...
    if (!rep->tick_ratio_fitted[WR_UNIT_LOWER_ARM] || !rep->tick_ratio_fitted[WR_UNIT_PALM] ||
        ...
        rep->verdict_tick_ratio = WR_CHECK_NO_EVIDENCE;
    } else if (rep->rel_rate_ppm_sigma > WR_ONE_RATE_CLAIM_PPM) {
        /*
        ...
        for (int u = 0; u < WR_UNIT_COUNT; ++u) {
            ...
            if (fabs(rep->ticks_per_index[u] - WR_NOMINAL_TICKS_PER_SAMPLE) >
                0.005 * WR_NOMINAL_TICKS_PER_SAMPLE) {
                ok = false;
            }
        }
        /*
         ...
         */
        if (fabs(rep->rel_rate_ppm) - 2.0 * rep->rel_rate_ppm_sigma > WR_ONE_RATE_CLAIM_PPM) {
            ok = false;
        }
        rep->verdict_tick_ratio = ok ? WR_CHECK_MATCH : WR_CHECK_DIFFERS;
    }

    /* --- §10.3 the skew ---------------------------------------------------- */
    if (rep->skew_stored < 2u) {
        rep->verdict_skew = WR_CHECK_NO_EVIDENCE;
    } else {
        /*
        ...
        rep->verdict_skew = (stable && small) ? WR_CHECK_MATCH : WR_CHECK_DIFFERS;
    }

    /* --- §6.6 the live rate is a continuum -------------------------------- */
    if (rep->steps_total == 0u) {
        rep->verdict_bursts = WR_CHECK_NO_EVIDENCE;
    } else if (rep->step[WR_STEP_DENSE] > 0u) {
        rep->verdict_bursts = WR_CHECK_MATCH;
    } else if (rep->step[WR_STEP_8] == 0u && rep->step[WR_STEP_9_31] == 0u) {
        ...
        rep->verdict_bursts = WR_CHECK_NOTE;
    } else {
        /* Motion happened and produced no dense step.  §6.6 says dense steps
         * appear in EVERY session containing motion. */
        rep->verdict_bursts = WR_CHECK_DIFFERS;
    }

    /* --- §6.3 which block is the palm -------------------------------------- */
    if (rep->accel_palm_minus_arm_fast.n == 0u) {
        rep->verdict_palm_is_second_block = WR_CHECK_NO_EVIDENCE;
    } else if (rep->accel_palm_minus_arm_fast.mean > 0.0) {
        rep->verdict_palm_is_second_block = WR_CHECK_MATCH;
    } else {
        rep->verdict_palm_is_second_block = WR_CHECK_DIFFERS;
    }

    /* --- §9.1 bring-up ------------------------------------------------------ */
    if (rep->bringup_len == 0u) {
        rep->verdict_bringup = WR_CHECK_NO_EVIDENCE;
    } else if (rep->bringup_matches_vendor) {
        rep->verdict_bringup = WR_CHECK_MATCH;
    } else {
         ...
         * §7.1) was never read. */
        rep->verdict_bringup = asked_versions ? WR_CHECK_NOTE : WR_CHECK_DIFFERS;
    }

    /* --- §9.2 the keepalive -------------------------------------------------- */
    if (rep->host_writes < 2u) {
        rep->verdict_keepalive = WR_CHECK_NO_EVIDENCE;
    } else if (rep->max_host_write_gap_us <= (wr_time_us)33 * 1000 * 1000) {
        rep->verdict_keepalive = WR_CHECK_MATCH;
    } else {
        rep->verdict_keepalive = WR_CHECK_DIFFERS;
    }

    /* --- §7.5 does a mid-stream retrieval cost a recording gap? ---------------- */
    if (rep->retrievals_measured == 0u) {
        rep->verdict_retrieval_continuity = WR_CHECK_NO_EVIDENCE;
    } else if (rep->retrievals_stalled == 0u) {
        rep->verdict_retrieval_continuity = WR_CHECK_MATCH;
    } else {
        rep->verdict_retrieval_continuity = WR_CHECK_DIFFERS;
    }

    /* --- §7.3 the motion-adaptive buffer -------------------------------------- */
    if (rep->records_history < 2u) {
        rep->verdict_history_rate = WR_CHECK_NO_EVIDENCE;
         ...
         */
        rep->verdict_history_rate = WR_CHECK_DIFFERS;
        ...
        rep->verdict_history_rate = WR_CHECK_MATCH;
    }
}

void wr_reconcile_finish(wr_reconciler *r, wr_reconcile_report *out_report)
{
    if (r == NULL) {
        return;
    }
    if (!r->finished) {
        ...
        r->rep.config = r->cfg;
        r->rep.expected_len_one_record = 1u + wr_stream_config_record_size(r->cfg);
        r->rep.expected_len_two_records = 1u + 2u * wr_stream_config_record_size(r->cfg);

        if (r->have_first_time && r->rep.last_us >= r->rep.first_us) {
            r->rep.duration_us = r->rep.last_us - r->rep.first_us;
        }

        wr_fit_snapshot(&r->fit, r->rep.last_us, &r->rep.fit);
        if (r->rep.fit.observations > 0 && r->rep.fit.fitted_rate_hz > 0.0) {
            r->rep.fitted_rate_hz = r->rep.fit.fitted_rate_hz;
            r->rep.ppm_vs_nominal = (r->rep.fitted_rate_hz - WR_NOMINAL_SAMPLE_RATE_HZ) /
                                    WR_NOMINAL_SAMPLE_RATE_HZ * 1e6;
        ...
        r->rep.tick_fit_n = r->ratio_fit[WR_UNIT_LOWER_ARM].n;
        if (r->rep.tick_ratio_fitted[WR_UNIT_LOWER_ARM] &&
            r->rep.tick_ratio_fitted[WR_UNIT_PALM]) {
            double mean = 0.5 * (r->rep.ticks_per_index[WR_UNIT_LOWER_ARM] +
                                 r->rep.ticks_per_index[WR_UNIT_PALM]);
            double slope = 0.0, sigma = 0.0;
        ...
        if (r->rep.steps_total > 0u) {
            r->rep.dense_fraction =
                (double)r->rep.step[WR_STEP_DENSE] / (double)r->rep.steps_total;
        }

        {
            double full_scale = wr_stream_config_gyro_full_scale_dps(r->cfg);
            double peak = r->rep.gyro_peak_dps[WR_UNIT_LOWER_ARM];
            if (r->rep.gyro_peak_dps[WR_UNIT_PALM] > peak) {
                peak = r->rep.gyro_peak_dps[WR_UNIT_PALM];
            }
            if (full_scale > 0.0) {
...
int wr_reconcile_disagreements(const wr_reconcile_report *rep)
{
    int n = 0;
    const wr_check_verdict v[] = {
...
int wr_reconcile_unmeasured(const wr_reconcile_report *rep)
{
    int n = 0;
    const wr_check_verdict v[] = {
        rep->verdict_frame_length, rep->verdict_quat_norm,
