...
uint32_t wr_index_unwrap(wr_index_unwrapper *u, uint16_t raw, bool *out_suspect)
{
    ...
    if (step >= WR_INDEX_REGRESSION_LIMIT) {
        u->regressions++;
        if (out_suspect != NULL) {
            *out_suspect = true;
        }
...
{
    memset(u, 0, sizeof(*u));
    u->ratio = WR_NOMINAL_TICKS_PER_SAMPLE;
    u->worst_margin = (uint32_t)(WR_TICK_MODULUS / 2u);
}

int64_t wr_tick_unwrap(wr_tick_unwrapper *u, uint32_t index, uint16_t raw, uint32_t *out_margin)
{
    int64_t predicted;
    int64_t residue;
    int64_t wraps;
    ...
    if (!u->have_anchor) {
        ...
        u->fit_last_ticks = 0;
        if (out_margin != NULL) {
            *out_margin = (uint32_t)(WR_TICK_MODULUS / 2u);
        }
        return 0;
    }
    ...
    wraps = (int64_t)llround(((double)(predicted - residue)) / (double)WR_TICK_MODULUS);
    unwrapped = residue + wraps * (int64_t)WR_TICK_MODULUS;

    error = unwrapped - predicted;
    if (error < 0) {
        error = -error;
    }
    margin = (error >= (int64_t)(WR_TICK_MODULUS / 2u))
                 ? 0u
                 : (uint32_t)((int64_t)(WR_TICK_MODULUS / 2u) - error);
    ...
    if (index > u->fit_last_index) {
...

        {
            ...
            uint64_t needed = u->ratio_fitted ? (uint64_t)u->fit_span_at_refit * 2u
                                              : (uint64_t)WR_TICK_RATIO_FIT_INDICES;
            if (span >= WR_TICK_RATIO_FIT_INDICES && (uint64_t)span >= needed) {
...
wr_time_us wr_ticks_to_us(int64_t ticks, double tick_rate_hz)
{
    ...
    return (wr_time_us)llround(((double)ticks / tick_rate_hz) * 1e6);
}

int32_t wr_tick_skew(uint16_t palm_raw, uint16_t lower_arm_raw)
{
    uint16_t d = (uint16_t)(palm_raw - lower_arm_raw);
    if (d >= (uint16_t)(WR_TICK_MODULUS / 2u)) {
        return (int32_t)d - (int32_t)WR_TICK_MODULUS;
    }
    return (int32_t)d;
}
