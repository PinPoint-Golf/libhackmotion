...
/*
 * wr_fit.h — internal.  The device→host clock fit.
 *
 ...
 * wr_fit detects that and raises WR_CLOCK_DEGENERATE.
 *
 * ── The estimator ──────────────────────────────────────────────────────────
 ...
 * WHOLE CONNECTION.  wr_fit_begin_stream() folds the finished stream's slope
...
#define WR_FIT_HULL_MAX   96u
#define WR_FIT_RESID_MAX 256u
...
#define WR_FIT_MIN_SPAN_US ((wr_time_us)2 * 1000 * 1000)
...
typedef struct wr_fit {
    ...
    wr_time_us origin_host_us;
    uint64_t   stream_id;

    /* Lower convex hull of the observations. */
    wr_fit_point hull[WR_FIT_HULL_MAX];
    size_t       hull_n;
    ...
    wr_fit_point resid[WR_FIT_RESID_MAX];
    size_t       resid_n;
    size_t       resid_head;

    int64_t    first_i, last_i;
    wr_time_us last_host_us;
     ...
     * publishing that zero made wr_clock_meets_budget() accept any budget at all
    ...
    /* Test hook — see wr_fit_force_slope(). */
    bool   slope_forced;
    double forced_slope;
} wr_fit;
 ...
 */
wr_status wr_fit_set_correction(wr_fit *f, const wr_clock_correction *correction);
 ...
 * The fit then reports WR_CLOCK_DEGENERATE, because the pinned rate disagrees
