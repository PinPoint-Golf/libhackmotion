...
wr_time_us wr_clock_to_host_us(const wr_clock_snapshot *fit, uint32_t sample_index)
{
    double di;
    ...
    return fit->anchor_host_us + (wr_time_us)llround(di * fit->slope_us_per_index);
}

wr_status wr_clock_index_for_host_us(const wr_clock_snapshot *fit, wr_time_us host_us,
                                     uint32_t *out_index)
{
    double di;
    double idx;
    ...
    if ((fit->flags & (uint32_t)WR_CLOCK_HAS_FIT) == 0u ||
...
wr_clock_error wr_clock_error_at(const wr_clock_snapshot *fit, uint32_t sample_index)
{
    wr_clock_error e;
    double precision;
    double systematic;
    ...
    if ((fit->flags & (uint32_t)WR_CLOCK_DEGENERATE) != 0u) {
    ...
    if ((fit->flags & (uint32_t)WR_CLOCK_SHORT_BASELINE) != 0u) {
        precision += (double)fit->residual_max_us;
    }
    /*
     ...
     * return precision_us = 0, and wr_clock_meets_budget() accepted ANY budget.
     * AR B2's refuse-rather-than-misalign gate, silently off.
     *
     * The connection's own measured jitter stands in where there is one
     * (wr_fit.carry_*); this covers the first fit of a connection, where there is
...
uint32_t wr_clock_precision_us(const wr_clock_snapshot *fit, uint32_t sample_index)
{
    return wr_clock_error_at(fit, sample_index).precision_us;
}

uint32_t wr_clock_uncertainty_us(const wr_clock_snapshot *fit, uint32_t sample_index)
{
    return wr_clock_error_at(fit, sample_index).total_us;
}

bool wr_clock_meets_budget(const wr_clock_snapshot *fit, uint32_t sample_index,
                           uint32_t budget_us)
{
    /* ⚠ precision, not total — see wr_clock_error. */
    return wr_clock_error_at(fit, sample_index).precision_us <= budget_us;
}

wr_status wr_clock_index_range_for_time(const wr_clock_snapshot *fit, wr_time_range window,
                                        wr_index_range *out)
{
    double first_exact;
    double last_exact;
    ...
    if (window.start_us >= window.end_us) {
        return WR_ERR_INVALID_ARG; /* half-open and empty */
    }
    if ((fit->flags & (uint32_t)WR_CLOCK_HAS_FIT) == 0u ||
        !(fit->slope_us_per_index > 0.0)) {
        return WR_ERR_NO_FIT;
    ...
    /*
     * wr_time_range is half-open, wr_index_range is inclusive.  The conversion
    ...
    if (last_i < first_i) {
        return WR_ERR_INVALID_ARG; /* the window falls between two samples */
    }
...
{
    return 1e6 / WR_NOMINAL_SAMPLE_RATE_HZ;
}

void wr_fit_init(wr_fit *f, double accuracy_drift_us_per_s)
{
    memset(f, 0, sizeof(*f));
    f->slope = nominal_slope();
    f->accuracy_drift_us_per_s =
        (accuracy_drift_us_per_s > 0.0) ? accuracy_drift_us_per_s
                                        : WR_ACCURACY_DRIFT_US_PER_S_DEFAULT;
    f->dirty = true;
}

void wr_fit_begin_stream(wr_fit *f, uint64_t stream_id)
{
     ...
     * earns a vote. */
    if (f->have_fit && f->n >= 64 && (f->flags & (uint32_t)WR_CLOCK_DEGENERATE) == 0u &&
        (f->flags & (uint32_t)WR_CLOCK_SHORT_BASELINE) == 0u) {
...
static int64_t cross_of(wr_fit_point o, wr_fit_point a, wr_fit_point b)
{
...
static int64_t chord_deviation(const wr_fit_point *H, size_t j)
{
...
static void hull_make_room(wr_fit *f)
{
    size_t best = 1;
    int64_t best_dev = INT64_MAX;

    if (f->hull_n < WR_FIT_HULL_MAX) {
        return;
    }
    /*
...
void wr_fit_observe(wr_fit *f, uint32_t sample_index, wr_time_us host_recv_us)
{
    wr_fit_point p;
    ...
    f->resid_head = (f->resid_head + 1u) % WR_FIT_RESID_MAX;
    if (f->resid_n < WR_FIT_RESID_MAX) {
        f->resid_n++;
    }
...
void wr_fit_set_blind(wr_fit *f, bool blind)
{
    if (blind) {
        f->flags |= (uint32_t)WR_CLOCK_BLIND;
    } else {
        f->flags &= ~(uint32_t)WR_CLOCK_BLIND;
    }
}

wr_status wr_fit_set_correction(wr_fit *f, const wr_clock_correction *c)
{
    if (f == NULL || c == NULL) {
        return WR_ERR_INVALID_ARG;
    }
     ...
     * mislead.
     */
    if ((c->fields & (uint32_t)(WR_CORRECTION_RATE | WR_CORRECTION_DRIFT)) == 0u) {
        return WR_ERR_INVALID_ARG;
    }
    if ((c->fields & (uint32_t)WR_CORRECTION_RATE) != 0u &&
        !(c->rate_ppm > -100000.0 && c->rate_ppm < 100000.0)) {
        return WR_ERR_INVALID_ARG;
    }
    if ((c->fields & (uint32_t)WR_CORRECTION_DRIFT) != 0u &&
...
static void residual_stats(wr_fit *f)
{
    int64_t buf[WR_FIT_RESID_MAX];
    size_t n = f->resid_n;
    ...
    for (size_t k = 0; k < n; ++k) {
        size_t idx = (f->resid_head + WR_FIT_RESID_MAX - n + k) % WR_FIT_RESID_MAX;
 ...
 * the lower hull. */
static void anchor_at_slope(wr_fit *f, double slope, size_t *out_vertex)
{
    size_t best = 0;
    double best_v = 0.0;
 ...
 */
#define WR_FIT_PINNED_RATE_TOLERANCE_PPM 200.0

static void check_pinned_slope(wr_fit *f, double slope)
{
    double di;
    double empirical;
    double err_ppm;
    ...
    if (err_ppm > WR_FIT_PINNED_RATE_TOLERANCE_PPM) {
        f->flags |= (uint32_t)WR_CLOCK_DEGENERATE;
    }
}

void wr_fit_force_slope(wr_fit *f, double slope_us_per_index)
{
    if (f == NULL) {
        return;
    }
...
/*
 * ⚠ WR_CLOCK_RATE_POOLED SAYS THE PUBLISHED SLOPE *IS* THE CARRIED-OVER ONE, so
 ...
 * measured here.
 *
 * ⚠ And design §6.1.1 reuses wr_fit_begin_stream() as the per-PULL re-anchor, so
 ...
 */
static void flag_seeded_rate(wr_fit *f)
{
    if (f->pooled_weight > 0.0) {
        f->flags |= (uint32_t)WR_CLOCK_RATE_POOLED;
    }
}

static void refit(wr_fit *f)
{
    double span_us;
    double seed = (f->pooled_weight > 0.0) ? f->pooled_slope : nominal_slope();

    f->dirty = false;
    f->flags &= (uint32_t)WR_CLOCK_BLIND; /* everything else is recomputed */
    ...
    if (f->slope_forced) {
        ...
        f->have_fit = true;
        f->flags |= (uint32_t)WR_CLOCK_HAS_FIT;
    ...
    if (f->n < 2 || span_us < (double)WR_FIT_MIN_SPAN_US) {
        /*
        ...
        anchor_at_slope(f, f->slope, NULL);
        f->have_fit = true;
        f->flags |= (uint32_t)(WR_CLOCK_HAS_FIT | WR_CLOCK_SHORT_BASELINE);
        flag_seeded_rate(f);
        residual_stats(f);
        return;
    }
     ...
     */
    {
        ...
        if (!have_best) {
            f->slope = seed;
            anchor_at_slope(f, f->slope, NULL);
            f->flags |= (uint32_t)WR_CLOCK_SHORT_BASELINE;
            flag_seeded_rate(f);
        } else {
            double rate = 1e6 / best_slope;
            if (!(rate > WR_RATE_PLAUSIBLE_MIN_HZ && rate < WR_RATE_PLAUSIBLE_MAX_HZ)) {
                /* Outside anything this crystal could do.  Keep the seeded rate
                 * and say so — an implausible rate is a symptom, not a value to
                 * publish. */
                f->flags |= (uint32_t)WR_CLOCK_RATE_IMPLAUSIBLE;
                f->slope = seed;
                anchor_at_slope(f, f->slope, NULL);
                check_pinned_slope(f, f->slope);
                flag_seeded_rate(f);
...
void wr_fit_snapshot(wr_fit *f, wr_time_us now_us, wr_clock_snapshot *out)
{
    ...
    if (!f->have_fit) {
        out->flags = f->flags & ~(uint32_t)WR_CLOCK_HAS_FIT;
    ...
    if (f->external_ppm != 0.0) {
        out->flags |= (uint32_t)WR_CLOCK_EXTERNAL_CORRECTION;
    }
    if (now_us != WR_TIME_UNKNOWN && f->last_host_us != 0 &&
        (now_us - f->last_host_us) > WR_FIT_STALE_AFTER_US) {
        out->flags |= (uint32_t)WR_CLOCK_STALE;
    }
    ...
    out->offset_us =
        out->anchor_host_us - (wr_time_us)llround((double)out->anchor_index * corrected_slope);
