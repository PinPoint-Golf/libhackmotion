...
/*
 * hm_session.c — the sans-I/O session core.
 *
 ...
 * the reference-pose anchor it keeps, and the HM_ERR_BUSY interlocks against a
 * retrieval.
 *
 ...
 * real reply HOLED, so a rule keyed on HM_HIST_COMPLETE would learn nothing on
 ...
 *     one hm_fit_observe() call site.                              §5 of notes
...
#define BRINGUP_REQUIRED_INFO ((uint32_t)HM_INFO_VERSIONS)
#define BRINGUP_ALL_INFO                                                                     \
    ((uint32_t)(HM_INFO_VERSIONS | HM_INFO_SENSOR_MAP | HM_INFO_BATTERY | HM_INFO_MAC |      \
                HM_INFO_SERIAL))
 ...
 * for this reason.
 */
#define BRACKET_LIMIT_US (2 * HM_HISTORY_DEPTH_SEED_US)
 ...
 * hm_history_block_release() — the one call that leaves the session thread. */
 ...
 */
#define SLEEP_SUSPECT_IDLE_US ((hm_time_us)270 * 1000 * 1000) /* 4.5 min of 5.0 */
#define SLEEP_SUSPECT_QUIET_US ((hm_time_us)10 * 1000 * 1000)
#define ADVERTISING_FRESH_US ((hm_time_us)5 * 1000 * 1000)

/* Exponential backoff for HM_RECOVER_RECONNECT_WITH_BACKOFF.  The host may use
 * it or ignore it — it owns the adapter (design §5.4). */
#define RETRY_BACKOFF_FIRST_US ((hm_time_us)500 * 1000)
#define RETRY_BACKOFF_MAX_US ((hm_time_us)30 * 1000 * 1000)
...
#define RETRY_ADVERTISING_US ((hm_time_us)250 * 1000)
...
#define TICK_MARGIN_ALARM (HM_TICK_MODULUS / 10u) /* a fifth of ±32768 */
...
#define WARN_THROTTLE_US ((hm_time_us)1000 * 1000)

/*
 * ⚠ THE PRESENCE RUN IS BOUNDED BOTH WAYS, and the two bounds answer different
 * failures.
 *
 * The count: HM_PRESENCE_MAX_SAMPLES (64) is what hm_presence_select_reference()
 ...
 * possible evidence.
 */
#define PRESENCE_RUN_WINDOW_US ((hm_time_us)2 * 1000 * 1000)
 ...
 * one `a1`.  Overflow is preceded by minutes of HM_WARN_KEEPALIVE_LATE. */
...
static void ring_init(hm_ring *r, void *base, size_t capacity, size_t elem)
{
    r->base = (uint8_t *)base;
    r->elem = elem;
...
static void *ring_slot(const hm_ring *r, size_t logical)
{
...
static void *ring_push(hm_ring *r, const void *value)
{
    void *slot;
...
static size_t ring_pop(hm_ring *r, void *out, size_t max)
{
...
static void ring_clear(hm_ring *r)
{
    r->head = 0u;
    r->count = 0u;
}
 ...
 */
typedef enum hm_due_id {
    HM_DUE_KEEPALIVE = 0,
    HM_DUE_BRINGUP,
    HM_DUE_STREAM_START,
    HM_DUE_STREAM_STOP, /* ⚠ the device's `0x83`; see due_stream_stop()        */
    HM_DUE_CAL_DEVICE,   /* a marker's `a2 01` reply, or the `0x94` result   */
    HM_DUE_CAL_RAISE,    /* ⚠ CLIENT policy — the device imposes no deadline */
    HM_DUE_CAL_PRESENCE, /* the presence run's window over live samples      */
    HM_DUE_CLOCK_EVENT,
    HM_DUE_KEEPALIVE_ALARM,
    HM_DUE_LIVE_GAP,
    HM_DUE_PINNED_REPORT,
    HM_DUE_POWER_OFF_LINGER,
    HM_DUE_BRACKET_LIMIT,
     ...
     * row without the `fire` beside it fails hm_session_create(), which is the
     * point. */
    HM_DUE_HISTORY_START,    /* the earliest reservation whose window can close */
    HM_DUE_HISTORY_DEADLINE, /* the earliest outstanding request's own deadline */
    HM_DUE_HISTORY_EVICTION, /* §8.6: the earliest queued window at risk of it  */
    HM_DUE_COUNT
} hm_due_id;
...
typedef enum hm_stream_state {
    HM_STREAM_STOPPED = 0,
    HM_STREAM_STARTING, /* ⚠ waiting for the first 0x90, NOT for the a0 01 ack */
    HM_STREAM_RUNNING,
    HM_STREAM_STOPPING
} hm_stream_state;
...
/* ------------------------------------------------------------------------ */
/*
 * The public hm_history_block is the FIRST member, so hm_history_block_release()
 ...
 */
typedef struct hm_history_record {
    hm_history_block pub; /* ⚠ MUST BE FIRST */
    uint32_t         magic;
    uint32_t         reserved;
    hm_allocator     alloc; /* ⚠ a COPY — see above */
    size_t           bytes;
} hm_history_record;

typedef enum hm_gather_state {
    HM_GATHER_FREE = 0,
    HM_GATHER_QUEUED,    /* reserved; waiting for the window to close, or for a turn */
    HM_GATHER_REQUESTED, /* `a1` queued; ⚠ waiting for `a1 02`, the ACCEPTANCE TEST */
    HM_GATHER_BRACKET,   /* records arriving                                        */
    HM_GATHER_REFILL,    /* an attempt ended holed; waiting for a live frame        */
    HM_GATHER_READY      /* block materialised, waiting to be collected             */
} hm_gather_state;
...
typedef struct hm_gather_run {
    size_t start;
    size_t count;
} hm_gather_run;

typedef struct hm_request {
    hm_gather_state    state;
    uint64_t           id;
    hm_history_request req; /* the caller's, copied and clamped */
    hm_time_us         reserved_us;
    ...
    hm_index_range indices;  /* what the window maps to — unwrapped, inclusive */
    hm_index_range recorded; /* ...clamped to what the device can have taken   */
    hm_index_range ask;      /* ...and the sub-range of the attempt in flight  */
    bool           have_indices;
    /* ⚠ An EXPLICIT flag, because a width cannot be the sentinel: hm_index_range
    ...
    uint8_t       attempts;
    hm_gather_run run[HISTORY_MAX_ATTEMPTS];
    size_t        run_count;
    bool          run_open;
    bool          run_have_index;
    ...
    hm_history_record *block;
    bool               alloc_failed;
} hm_request;

struct hm_session {
    /* --- Config ---------------------------------------------------------- */
    hm_session_policy policy; /* resolved and clamped at create */
    hm_stream_config  requested_cfg;
    hm_stream_config  active_cfg;
    ...
    hm_link_state  link;
    bool           mtu_rejected;
    hm_time_us     link_up_us;
    hm_time_us     last_device_byte_us;
    hm_time_us     last_host_write_us;
    hm_time_us     last_advertising_us;
    ...
    /* --- Stream ---------------------------------------------------------- */
    hm_stream_state stream;
    uint64_t        stream_id;
    uint64_t        next_stream_id;
    uint32_t        stream_starts;
    hm_time_us      stream_started_us; /* phase 4 reads it for the resident range */
    hm_time_us      last_live_us;
    bool            have_live;
    ...
    /* --- Per-stream decode ------------------------------------------------ */
    hm_index_unwrapper idx; /* ⚠ LIVE ONLY.  History never touches it (§10.1). */
    hm_tick_unwrapper  tick[HM_UNIT_COUNT];
    hm_fit             fit;
    hm_clock_snapshot  snap; /* refreshed once per notification, not per record */
    bool               snap_valid;
    hm_overlap         overlap;
    hm_pinned_counts   pinned;      /* since the last HM_EV_PINNED_SAMPLES */
     ...
     * hm_calibration_span.spans_transition on a block. */
    hm_calibration_state cal_state;
    hm_calibration_state cal_prev_state;
    hm_time_us           cal_changed_us;

    hm_calibration_phase        cal_phase;
    hm_calibration_phase        cal_phase_prev;
    hm_calibration_abort_reason cal_abort_reason;
    hm_time_us                  cal_started_us;
     ...
     * hm_calibration_* call refuses while a bracket is open.
     */
    bool                          cal_run_active;
    size_t                        cal_run_count;
    hm_sample                     cal_run[HM_PRESENCE_MAX_SAMPLES];
    ...
    hm_calibration_presence_event cal_anchor;
     ...
     * reachable: hm_history_resident_range() answers HM_OK once anything has
     * been measured and HM_PENDING while it has not.  A counter nothing can
     * read is the same shape as a check that has silently stopped running.
     */
    hm_time_us depth_lo_us;
    hm_time_us depth_hi_us;
    bool       have_depth_lo;
    bool       have_depth_hi;
    /* Both bounds measured and crossing — §7.3's open question answering itself, once
     * per stream, through HM_WARN_HISTORY_DEPTH_CONFLICT. */
    bool depth_conflict_warned;

    hm_request      reqs[HISTORY_MAX_PENDING];
    hm_sample      *gather; /* shared: only the request in flight writes here */
    size_t          gather_cap;
    hm_index_range *cov_storage;
    size_t          cov_cap;

    /* --- Deadlines -------------------------------------------------------- */
    hm_time_us due[HM_DUE_COUNT];
    /* ⚠ The seatbelt's own record.  The OBSERVABLE guarantee is that
     * hm_session_next_due_us() strictly increases when a host steps to exactly
    ...
    /* --- Warning throttle -------------------------------------------------- */
    uint32_t   warn_pending[HM_WARN_CODE_COUNT];
    hm_time_us warn_last_us[HM_WARN_CODE_COUNT];
    bool       warn_seen[HM_WARN_CODE_COUNT];
 ...
 */
static void history_arm(hm_session *s);
static void history_service(hm_session *s);
static void history_abandon(hm_session *s, hm_request *r, hm_history_status status);
static void history_bracket_opened(hm_session *s);
static void history_bracket_closed(hm_session *s);
static void history_abandon_all(hm_session *s, hm_history_status status);
static bool history_outstanding(const hm_session *s);
 ...
 * hm_session_destroy() is one free and a caller can account for the library's
 * footprint with a single number.
 */
typedef struct hm_layout {
    uint8_t *base;
    size_t   used;
} hm_layout;

static void *layout_take(hm_layout *l, size_t bytes, size_t align)
{
...
/* ------------------------------------------------------------------------ */
static void event_push(hm_session *s, hm_event *ev)
{
    if (s->closed) {
     ...
     * means the queue overflowed — which hm_session_dropped_events() also
...
static void event_emit(hm_session *s, hm_event_type type)
{
    hm_event ev;
...
static void warn_now(hm_session *s, hm_warning_code code, int32_t detail_i32, double detail_f64)
{
    hm_event ev;
    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_WARNING;
 ...
 */
static void warn_rate(hm_session *s, hm_warning_code code)
{
    s->warn_pending[code]++;
...
static void warn_flush(hm_session *s)
{
    for (int i = 0; i < (int)HM_WARN_CODE_COUNT; ++i) {
        ...
        }
        {
            ...
            s->warn_pending[i] = 0u;
            warn_now(s, (hm_warning_code)i, n, 0.0);
        }
    }
}
 ...
 */
static void wire_log(hm_session *s, hm_wire_direction dir, const uint8_t *data, size_t length,
                     bool redact)
{
    hm_wire_chunk chunk;
    void         *slot;
    ...
    /* ⚠ `sequence` is CARRIED, not renumbered by the reader: HM_WIRE_LOST says
    ...
    if (length > HM_WIRE_CHUNK_MAX) {
        length = HM_WIRE_CHUNK_MAX; /* unreachable: the longest message is 93 B */
    }
    ...
    if (redact) {
        ...
        chunk.flags |= (uint8_t)HM_WIRE_REDACTED;
        if (length > 0u) {
            chunk.data[0] = data[0];
        }
    } else if (length > 0u) {
    ...
    if (s->wire_lost_pending) {
        chunk.flags |= (uint8_t)HM_WIRE_LOST;
...

    {
        ...
        if (slot != NULL && s->wire.dropped != before) {
             ...
             * encounters next. */
            hm_wire_chunk *head = (hm_wire_chunk *)ring_slot(&s->wire, 0u);
            head->flags |= (uint8_t)HM_WIRE_LOST;
        }
    }
}

/* Link, MTU and stream boundaries, as short ASCII.  tools/hm_capture.py writes
 * the same vocabulary, so one reader handles both. */
static void wire_meta(hm_session *s, const char *text)
{
    wire_log(s, HM_WIRE_META, (const uint8_t *)text, strlen(text), false);
}
 ...
 */
static void note_now(hm_session *s, hm_time_us now_us)
{
    if (!s->have_now) {
        s->have_now = true;
        s->now_us = now_us;
    ...
    if (now_us < s->now_us) {
        warn_now(s, HM_WARN_HOST_CLOCK_REGRESSION, 0, (double)(s->now_us - now_us));
...
/* ------------------------------------------------------------------------ */
static void arm(hm_session *s, hm_due_id id, hm_time_us when)
{
    /*
...
static void disarm(hm_session *s, hm_due_id id)
{
    s->due[id] = HM_TIME_NEVER;
}

static void disarm_all(hm_session *s)
{
    for (int i = 0; i < (int)HM_DUE_COUNT; ++i) {
        s->due[i] = HM_TIME_NEVER;
    }
}
 ...
 */
static bool writes_allowed(const hm_session *s)
{
    if (s->closed || s->link == HM_LINK_CLOSED) {
        return false;
    }
    ...
    if (s->link == HM_LINK_DOWN) {
        return false;
    }
    return !s->bracket_open;
}

static void queue_write(hm_session *s, const hm_write_request *req)
{
    if (!writes_allowed(s)) {
        return;
    }
...
static void queue_command(hm_session *s, hm_status (*encode)(hm_write_request *))
{
    hm_write_request req;
    if (encode(&req) == HM_OK) {
        queue_write(s, &req);
    }
}
 ...
 * goes through hm_command_emit()'s allowlist; there is no second path out.
 *
 ...
 */
static hm_status queue_command_arg(hm_session *s,
                                   hm_status (*encode)(hm_write_request *, uint8_t), uint8_t arg)
{
    hm_write_request req;
    hm_status        st = encode(&req, arg);
    if (st < HM_OK) {
        return st;
    }
    queue_write(s, &req);
...
/* Link                                                                      */
/* ------------------------------------------------------------------------ */
static void stream_reset_decode(hm_session *s)
{
    hm_index_unwrapper_reset(&s->idx);
    hm_tick_unwrapper_reset(&s->tick[HM_UNIT_LOWER_ARM]);
    hm_tick_unwrapper_reset(&s->tick[HM_UNIT_PALM]);
    hm_overlap_reset(&s->overlap);
    s->have_index = false;
    s->first_index = 0u;
    s->head_index = 0u;
    s->have_live = false;
    ...
    hm_pinned_counts_reset(&s->pinned);
    s->pinned_span = 0u;
    disarm(s, HM_DUE_PINNED_REPORT);
...
static void set_calibration_state(hm_session *s, hm_calibration_state state)
{
 ...
 * transition at all.  The block's `hm_calibration_span` has the same limit and
 * says so; nothing on the wire offers more.
 *
 * ⚠ Never returns HM_CAL_LOST.  It cannot: set_calibration_state() is only ever
 * called with UNKNOWN, UNCALIBRATED or CALIBRATED.  LOST belongs to
 * hm_calibration_span alone (sample.h, history.h) and no sample carries it.
 */
static hm_calibration_state cal_state_at(const hm_session *s, hm_time_us at_us)
{
    if (s->cal_prev_state == s->cal_state || s->cal_changed_us == 0 ||
        at_us == HM_TIME_UNKNOWN || at_us >= s->cal_changed_us) {
        return s->cal_state;
    }
 ...
 *    HM_CALP_COMPLETE is not evidence that calibration took.
 ...
 * 3. SKIPPING THE CHECK LEAVES THE FLAG AT HM_CAL_UNKNOWN (§7.5, design review
 ...
 * such.
 */
static bool cal_in_progress(const hm_session *s)
{
    switch (s->cal_phase) {
        case HM_CALP_AWAIT_HORIZONTAL:
        case HM_CALP_MARKING_POSE0:
        case HM_CALP_OBSERVING_RAISE:
        case HM_CALP_MARKING_POSE1:
        case HM_CALP_APPLYING:
        case HM_CALP_VERIFYING:
            return true;
        case HM_CALP_IDLE:
        case HM_CALP_COMPLETE:
        case HM_CALP_ABORTED:
        case HM_CALIBRATION_PHASE_COUNT:
            break;
    }
    return false;
}
 ...
 */
static void cal_arm_for_phase(hm_session *s)
{
    disarm(s, HM_DUE_CAL_DEVICE);
    disarm(s, HM_DUE_CAL_RAISE);
    disarm(s, HM_DUE_CAL_PRESENCE);
    ...
    switch (s->cal_phase) {
        ...
        case HM_CALP_APPLYING:
             ...
             * given — which is the failure it has, and HM_WARN_KEEPALIVE_LATE
             * names it directly an order of magnitude before this matters. */
            arm(s, HM_DUE_CAL_DEVICE, s->now_us + s->policy.calibration_result_timeout_us);
            break;
        case HM_CALP_OBSERVING_RAISE:
            ...
            arm(s, HM_DUE_CAL_RAISE, s->now_us + s->policy.calibration_raise_limit_us);
            break;
        /* HM_CALP_VERIFYING has no bound of its own: the presence run is
...
static void cal_set_phase(hm_session *s, hm_calibration_phase phase,
                          hm_calibration_abort_reason reason)
{
    hm_event ev;
    ...
    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_CALIBRATION_PHASE;
...
static void cal_run_stop(hm_session *s)
{
...
static void cal_abort(hm_session *s, hm_calibration_abort_reason reason)
{
     ...
     * HM_CAL_UNKNOWN and the device has re-referenced its stream — an abort
     * cannot undo that, and claiming UNCALIBRATED would label anatomically
     * referenced samples as raw ones.
     */
    cal_set_phase(s, HM_CALP_ABORTED, reason);
}
 ...
 */
static void cal_finish_presence(hm_session *s)
{
    hm_event ev;
    ...
    if (n < PRESENCE_RUN_MIN ||
        hm_presence_select_reference(s->cal_run, n, &s->cal_anchor) != HM_OK) {
        s->cal_run_count = 0u;
        warn_now(s, HM_WARN_PRESENCE_NOT_MEASURED, (int32_t)n, 0.0);
        /* The transform IS applied, so the routine completed; what did not
         * happen is the check, and the state stays UNKNOWN to say so. */
        cal_set_phase(s, HM_CALP_COMPLETE, HM_CAL_ABORT_NONE);
        return;
    }
    s->cal_run_count = 0u;
    ...
    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_CALIBRATION_PRESENCE;
    ev.u.calibration_presence = s->cal_anchor;
    event_push(s, &ev);

    /* §7.5's three bands.  hm_presence_classify() drew them from §8.2's two
     * populations and returns HM_CAL_UNKNOWN for the gap between them. */
    switch ((hm_calibration_state)s->cal_anchor.state) {
        case HM_CAL_CALIBRATED:
            set_calibration_state(s, HM_CAL_CALIBRATED);
            break;
        case HM_CAL_UNCALIBRATED:
            warn_now(s, HM_WARN_CALIBRATION_ABSENT, 0,
                     (double)s->cal_anchor.relative_angle_deg);
            set_calibration_state(s, HM_CAL_UNCALIBRATED);
            break;
        case HM_CAL_UNKNOWN:
        case HM_CAL_LOST:
        default:
            ...
            warn_now(s, HM_WARN_CALIBRATION_INDETERMINATE, 0,
                     (double)s->cal_anchor.relative_angle_deg);
            break;
    }
    cal_set_phase(s, HM_CALP_COMPLETE, HM_CAL_ABORT_NONE);
...
static void cal_run_note_sample(hm_session *s, const hm_sample *sample)
{
    if (!s->cal_run_active) {
        return;
    }
    if (s->cal_run_count < HM_PRESENCE_MAX_SAMPLES) {
...
static void stop_stream_locally(hm_session *s, bool emit_event)
{
    if (s->stream == HM_STREAM_STOPPED) {
        return;
    }
    ...
    cal_abort(s, HM_CAL_ABORT_STREAM_LOST);
    s->stream = HM_STREAM_STOPPED;
     ...
     */
    history_abandon_all(s, HM_HIST_NO_STREAM);
    disarm(s, HM_DUE_STREAM_START);
    disarm(s, HM_DUE_STREAM_STOP);
    disarm(s, HM_DUE_LIVE_GAP);
    disarm(s, HM_DUE_CLOCK_EVENT);
    if (emit_event) {
        hm_event ev;
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_STREAM_STOPPED;
...
static void close_bracket(hm_session *s)
{
    bool owned;
     ...
     */
    {
        hm_event ev;
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_HISTORY_BLIND_SPAN;
     ...
     * hm_fit_begin_stream() is exactly that operation: it folds this stretch's
     ...
     */
    hm_fit_set_blind(&s->fit, false);
    hm_fit_begin_stream(&s->fit, s->stream_id);
    s->snap_valid = false;
    ...
    if (s->link == HM_LINK_READY) {
        arm(s, HM_DUE_KEEPALIVE, s->now_us + s->policy.keepalive_period_us);
    }
    /*
     ...
     */
    if (s->stream == HM_STREAM_RUNNING) {
        arm(s, HM_DUE_LIVE_GAP, s->now_us + s->policy.live_gap_alarm_us);
    }
...
static hm_recovery_advice classify_link_down(const hm_session *s, hm_link_down_cause cause,
                                             hm_time_us *out_delay)
{
    hm_time_us idle_write = s->now_us - s->last_host_write_us;
    hm_time_us quiet = s->now_us - s->last_device_byte_us;
    ...
    *out_delay = 0;
    ...
    if (s->powering_off || s->local_teardown) {
        return HM_RECOVER_DO_NOT_RETRY;
    }
    ...
    if (cause == HM_LINK_DOWN_CONNECTION_TAKEN) {
        return HM_RECOVER_NEEDS_OTHER_APP_CLOSED;
    }
    /*
    ...
    if (s->last_host_write_us != 0 && idle_write >= SLEEP_SUSPECT_IDLE_US) {
        return HM_RECOVER_NEEDS_BUTTON_PRESS;
    }
    if (cause == HM_LINK_DOWN_REMOTE_CLOSED && quiet >= SLEEP_SUSPECT_QUIET_US) {
        return HM_RECOVER_NEEDS_BUTTON_PRESS;
    }
    ...
    if (s->have_advertising && (s->now_us - s->last_advertising_us) <= ADVERTISING_FRESH_US) {
        *out_delay = RETRY_ADVERTISING_US;
        return HM_RECOVER_RECONNECT_WITH_BACKOFF;
    }

    {
        hm_time_us delay = RETRY_BACKOFF_FIRST_US;
...
static void go_link_down(hm_session *s, hm_link_down_cause cause)
{
    hm_event   ev;
    hm_time_us delay = 0;

    if (s->link == HM_LINK_DOWN || s->link == HM_LINK_CLOSED) {
        return;
    }
    ...
    cal_abort(s, HM_CAL_ABORT_LINK_LOST);
    /*
     * ⚠ THE CALIBRATION STATE MOVES BEFORE THE RESERVATIONS ARE ANSWERED, and
     * the ordering is the whole of HM_CAL_LOST.  A block gathered under a
     ...
     * stream that ended.
     */
    history_abandon_all(s, HM_HIST_LINK_LOST);
    stop_stream_locally(s, s->stream != HM_STREAM_STOPPED);
    close_bracket(s);

    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_LINK_DOWN;
    ...
    ev.u.link_down.suggested_retry_delay_us =
        (ev.u.link_down.advice == (uint8_t)HM_RECOVER_RECONNECT_WITH_BACKOFF) ? delay : 0;
    event_push(s, &ev);

    if (ev.u.link_down.advice == (uint8_t)HM_RECOVER_RECONNECT_WITH_BACKOFF) {
        s->consecutive_failures++;
    }

    /* ⚠ The anchor belongs to the calibration that just died with the link, and
     * hm_calibration_reference_anchor() promises "the CURRENT calibration".
...
static void enter_ready(hm_session *s)
{
    hm_event ev;

    if (s->link != HM_LINK_BRINGUP) {
        return;
    }
    s->link = HM_LINK_READY;
    s->consecutive_failures = 0u;
    disarm(s, HM_DUE_BRINGUP);
    ...
    event_emit(s, HM_EV_READY);

    /* §9.2 — the keepalive runs from READY onward, always, for the whole
     * connection.  There is no way to switch it off. */
    arm(s, HM_DUE_KEEPALIVE, s->now_us + s->policy.keepalive_period_us);
    arm(s, HM_DUE_KEEPALIVE_ALARM, s->now_us + s->policy.keepalive_alarm_us);
}

static void maybe_enter_ready(hm_session *s)
{
    if (s->link != HM_LINK_BRINGUP) {
        return;
    }
...
/* Deadline handlers                                                         */
/* ------------------------------------------------------------------------ */
static void due_keepalive(hm_session *s)
{
    /*
...
static void due_bringup(hm_session *s)
{
    if (s->link != HM_LINK_BRINGUP) {
        return;
    }
     ...
     *                missing; `hm_device_info.valid` records which did.
    ...
    go_link_down(s, HM_LINK_DOWN_UNKNOWN);
}

static void due_stream_start(hm_session *s)
{
    if (s->stream != HM_STREAM_STARTING) {
        return;
    }
    /*
    ...
    warn_now(s, HM_WARN_STREAM_START_TIMEOUT, 0, 0.0);
}

/*
 * ⚠ THE ONE DEVICE-FACING WAIT §5.7 LEFT UNBOUNDED (implementation-review I9).
 *
 * stop_stream() sets HM_STREAM_STOPPING and queues `83`; the only way out was
 ...
 * there for ever: start_stream() returned HM_ERR_INVALID_STATE, cal_guard()
 * returned HM_ERR_NO_STREAM, history_service() never issued, and
 * hm_history_resident_range() refused, with no event or warning explaining any
 ...
 */
static void due_stream_stop(hm_session *s)
{
    if (s->stream != HM_STREAM_STOPPING) {
        return;
    }
    stop_stream_locally(s, true);
    warn_now(s, HM_WARN_STREAM_STOP_TIMEOUT, 0, 0.0);
}

static void due_cal_device(hm_session *s)
{
    /*
     ...
     */
    cal_abort(s, HM_CAL_ABORT_NO_RESULT);
}

static void due_cal_raise(hm_session *s)
{
    if (s->cal_phase != HM_CALP_OBSERVING_RAISE) {
        return;
    }
    ...
    cal_abort(s, HM_CAL_ABORT_RAISE_TOO_SLOW);
}

static void due_cal_presence(hm_session *s)
{
    if (!s->cal_run_active) {
        return;
    }
...
static void due_clock_event(hm_session *s)
{
    hm_event ev;
    bool     degraded;
    ...
    hm_fit_snapshot(&s->fit, s->now_us, &s->snap);
    s->snap_valid = true;

    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_CLOCK_UPDATED;
    ev.u.clock = s->snap;
    event_push(s, &ev);
    ...
    degraded = (s->snap.residual_p90_us > s->policy.residual_alarm_us) ||
               ((s->snap.flags & (uint32_t)(HM_CLOCK_DEGENERATE | HM_CLOCK_RATE_IMPLAUSIBLE)) !=
                0u);
    if (degraded && !s->clock_degraded) {
        hm_event dev;
        memset(&dev, 0, sizeof(dev));
        dev.type = (uint16_t)HM_EV_CLOCK_DEGRADED;
        dev.u.clock = s->snap;
        event_push(s, &dev);
    }
    s->clock_degraded = degraded;

    arm(s, HM_DUE_CLOCK_EVENT, s->now_us + s->policy.clock_event_period_us);
}

static void due_keepalive_alarm(hm_session *s)
{
    hm_time_us since = s->now_us - s->last_host_write_us;
    ...
    if (since >= s->policy.keepalive_alarm_us) {
        warn_now(s, HM_WARN_KEEPALIVE_LATE, 0, (double)since);
        arm(s, HM_DUE_KEEPALIVE_ALARM, s->now_us + s->policy.keepalive_alarm_us);
        return;
    }
    arm(s, HM_DUE_KEEPALIVE_ALARM, s->last_host_write_us + s->policy.keepalive_alarm_us);
}

static void due_live_gap(hm_session *s)
{
    hm_time_us since;
    ...
    if (s->stream != HM_STREAM_RUNNING || s->bracket_open || !s->have_live) {
        return;
    }
    since = s->now_us - s->last_live_us;
    if (since >= s->policy.live_gap_alarm_us) {
        warn_now(s, HM_WARN_LIVE_GAP, 0, (double)since);
        arm(s, HM_DUE_LIVE_GAP, s->now_us + s->policy.live_gap_alarm_us);
        return;
    }
    arm(s, HM_DUE_LIVE_GAP, s->last_live_us + s->policy.live_gap_alarm_us);
}

static void due_pinned_report(hm_session *s)
{
    hm_event ev;
    ...
    memset(&ev, 0, sizeof(ev));
    ev.type = (uint16_t)HM_EV_PINNED_SAMPLES;
...
static void due_power_off_linger(hm_session *s)
{
    /*
     ...
     * has still not reported the disconnect by HM_POWER_OFF_LINGER_US, the
     * session declares the link gone itself — with DO_NOT_RETRY, because the
     * device now needs a physical button press.
     */
    go_link_down(s, HM_LINK_DOWN_LOCAL_REQUEST);
}

static void due_bracket_limit(hm_session *s)
{
    /*
...
static void due_history_start(hm_session *s)
{
...
static void due_history_deadline(hm_session *s)
{
    /*
    ...
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        hm_request *r = &s->reqs[i];
        if (r->state == HM_GATHER_FREE || r->state == HM_GATHER_READY) {
            continue;
        }
        if (r->req.deadline_us <= s->now_us) {
            history_abandon(s, r, HM_HIST_TIMED_OUT);
        }
    }
    history_arm(s);
    history_service(s);
...
static void due_history_eviction(hm_session *s)
{
    /*
...
typedef struct hm_due_row {
    const char *name;
    void (*fire)(hm_session *);
} hm_due_row;

static const hm_due_row k_due[HM_DUE_COUNT] = {
...
_Static_assert(sizeof(k_due) / sizeof(k_due[0]) == (size_t)HM_DUE_COUNT,
               "every hm_due_id needs a row: a missing one must not be a silent no-op");
 ...
 */
static void stamp_with(hm_session *s, hm_sample *sample, const hm_clock_snapshot *fit,
                       hm_time_us host_recv_us, hm_time_us captured_at_us,
                       hm_sample_source source)
{
    ...
    if (fit != NULL && (fit->flags & (uint32_t)HM_CLOCK_HAS_FIT) != 0u &&
        (sample->flags & (uint16_t)HM_SAMPLE_INDEX_MISSING) == 0u) {
        hm_clock_error err = hm_clock_error_at(fit, sample->sample_index);
        sample->host_time_us = hm_clock_to_host_us(fit, sample->sample_index);
        ...
        if (sample->sample_index > fit->last_index || sample->sample_index < fit->first_index) {
            sample->flags |= (uint16_t)HM_SAMPLE_HOST_TIME_EXTRAPOLATED;
        }
    } else {
        sample->host_time_us = HM_TIME_UNKNOWN;
        sample->precision_us = UINT32_MAX;
        sample->uncertainty_us = UINT32_MAX;
        sample->flags |= (uint16_t)HM_SAMPLE_NO_FIT;
    }
...
static void stamp(hm_session *s, hm_sample *sample, hm_time_us host_recv_us,
                  hm_sample_source source)
{
 ...
 */
static void derive_from_frame(hm_session *s, hm_sample *sample)
{
    bool     suspect = false;
    uint32_t unwrapped;

    if ((sample->flags & (uint16_t)HM_SAMPLE_INDEX_MISSING) != 0u) {
    ...
    unwrapped = hm_index_unwrap(&s->idx, sample->sample_index_raw, &suspect);
    sample->sample_index = unwrapped;
    if (suspect) {
        /* A step above HM_INDEX_REGRESSION_LIMIT is almost certainly a
        ...
        warn_rate(s, HM_WARN_INDEX_REGRESSION);
    }
    if (!s->have_index) {
        s->have_index = true;
    ...
    {
        hm_unit_sample *unit[HM_UNIT_COUNT];
        unit[HM_UNIT_LOWER_ARM] = &sample->lower_arm;
        unit[HM_UNIT_PALM] = &sample->palm;

        for (int u = 0; u < (int)HM_UNIT_COUNT; ++u) {
            uint32_t margin = 0u;
            int64_t  ticks;
            if (!unit[u]->has_ticks) {
                unit[u]->device_time_us = HM_TIME_UNKNOWN;
                continue;
            }
            ticks = hm_tick_unwrap(&s->tick[u], unwrapped, unit[u]->ticks_raw, &margin);
            /*
            ...
            unit[u]->device_time_us =
                hm_ticks_to_us(ticks, hm_tick_rate_hz(&s->tick[u], HM_NOMINAL_SAMPLE_RATE_HZ));
            if (margin < TICK_MARGIN_ALARM && !s->tick_margin_warned) {
                ...
                warn_now(s, HM_WARN_TICK_PREDICTION_MARGIN, (int32_t)margin, 0.0);
            }
        }
        ...
        if (sample->lower_arm.has_ticks && sample->palm.has_ticks) {
            ...
            double  rate = hm_tick_rate_hz(&s->tick[HM_UNIT_LOWER_ARM], HM_NOMINAL_SAMPLE_RATE_HZ);
            sample->skew_us = (int32_t)hm_ticks_to_us(skew_ticks, rate);
        }
    }
}

static void live_deliver(hm_session *s, hm_sample *sample, hm_time_us host_recv_us)
{
    stamp(s, sample, host_recv_us, HM_SOURCE_LIVE);

    if ((sample->flags & (uint16_t)HM_SAMPLE_INDEX_MISSING) == 0u) {
        hm_overlap_note_live(&s->overlap, sample->sample_index, hm_sample_raw_digest(sample));
    }
    ...
    if ((sample->flags & (uint16_t)HM_SAMPLE_PINNED) != 0u) {
        hm_pinned_counts_add(&s->pinned, sample);
        if (s->due[HM_DUE_PINNED_REPORT] == HM_TIME_NEVER) {
            arm(s, HM_DUE_PINNED_REPORT, s->now_us + s->policy.pinned_report_period_us);
        }
    }
    s->pinned_span++;
 ...
 */
static int slot_of(const hm_session *s, const hm_request *r)
{
    return (int)(r - s->reqs);
}

static uint64_t index_width(hm_index_range r)
{
...
{
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        if (s->reqs[i].state != HM_GATHER_FREE && s->reqs[i].state != HM_GATHER_READY) {
            return true;
        }
    }
    return false;
 ...
 */
static bool split_at_wrap(hm_index_range want, hm_index_range *head, hm_index_range *rest)
{
    uint32_t boundary;
 ...
 */
static void derive_history(hm_session *s, hm_request *r, hm_sample *sample)
{
    hm_unit_sample *unit[HM_UNIT_COUNT];
    ...
    for (int u = 0; u < (int)HM_UNIT_COUNT; ++u) {
        ...
        ticks = hm_tick_unwrap(&r->tick[u], sample->sample_index, unit[u]->ticks_raw, &margin);
        unit[u]->device_time_us =
            hm_ticks_to_us(ticks, hm_tick_rate_hz(&r->tick[u], HM_NOMINAL_SAMPLE_RATE_HZ));
        if (margin < TICK_MARGIN_ALARM) {
            ...
            warn_rate(s, HM_WARN_TICK_PREDICTION_MARGIN);
        }
    }

    if (sample->lower_arm.has_ticks && sample->palm.has_ticks) {
        int32_t skew_ticks = hm_tick_skew(sample->palm.ticks_raw, sample->lower_arm.ticks_raw);
        double  rate = hm_tick_rate_hz(&r->tick[HM_UNIT_LOWER_ARM], HM_NOMINAL_SAMPLE_RATE_HZ);
        sample->skew_us = (int32_t)hm_ticks_to_us(skew_ticks, rate);
    }
}
 ...
 */
static void stamp_history(hm_session *s, hm_request *r, hm_sample *sample)
{
    bool       datable = (r->fit.flags & (uint32_t)HM_CLOCK_HAS_FIT) != 0u &&
     ...
     * the block's own `hm_calibration_span`.
     */
    hm_time_us captured_at = datable ? hm_clock_to_host_us(&r->fit, sample->sample_index)
                                     : r->req.window.start_us;
    stamp_with(s, sample, datable ? &r->fit : NULL, HM_TIME_UNKNOWN, captured_at,
               HM_SOURCE_HISTORY);
}

static void history_progress_event(hm_session *s, const hm_request *r, hm_event_type type)
{
    hm_event ev;
    ...
    ev.u.history_progress.fraction =
        r->have_indices ? (float)hm_coverage_fraction(&r->cov, r->indices) : 0.0f;
 ...
 */
static void gather_record(hm_session *s, hm_sample *sample)
{
    hm_request *r = NULL;
    uint32_t    idx;

    if (s->bracket_owned && s->active >= 0 &&
        s->reqs[s->active].state == HM_GATHER_BRACKET) {
        r = &s->reqs[s->active];
    }
    if (r == NULL || (sample->flags & (uint16_t)HM_SAMPLE_INDEX_MISSING) != 0u) {
        /*
         ...
         * and discarded either way.  They must not reach hm_fit_observe() and
    ...
    if ((sample->flags & (uint16_t)HM_SAMPLE_PINNED) != 0u) {
        hm_pinned_counts_add(&r->pinned, sample);
    }
    /*
     ...
     * mismatch count.
     */
    switch (hm_overlap_check(&s->overlap, idx, hm_sample_raw_digest(sample))) {
        case HM_OVERLAP_AGREES:
            r->overlap_samples++;
            break;
        case HM_OVERLAP_DIFFERS:
            r->overlap_samples++;
            r->overlap_mismatches++;
            break;
        case HM_OVERLAP_ABSENT:
        default:
            break;
    }

    (void)hm_coverage_add(&r->cov, idx);
    ...
    if ((r->sample_count % HISTORY_PROGRESS_EVERY) == 0u) {
        history_progress_event(s, r, HM_EV_HISTORY_PROGRESS);
    }
}

static void gather_close_run(hm_request *r)
{
    if (!r->run_open) {
        return;
    }
...
static size_t merge_runs(const hm_sample *src, const hm_gather_run *run, size_t run_count,
                         hm_sample *dst, size_t dst_cap)
{
...
/* Host time of one index under the block's own fit, or HM_TIME_UNKNOWN where
 * that fit cannot speak for it (§6.1.1). */
static hm_time_us block_time_at(const hm_request *r, uint32_t index)
{
    if ((r->fit.flags & (uint32_t)HM_CLOCK_HAS_FIT) == 0u || index < r->fit.first_index) {
        return HM_TIME_UNKNOWN;
    }
    return hm_clock_to_host_us(&r->fit, index);
}

/* ⚠ hm_index_range is INCLUSIVE and hm_time_range is HALF-OPEN, so the end
 * carries one sample period.  This is where that off-by-one would be born. */
static hm_time_range block_span(const hm_request *r, uint32_t first, uint32_t last)
{
    hm_time_range t;

    t.start_us = block_time_at(r, first);
    t.end_us = block_time_at(r, last);
    if (t.start_us == HM_TIME_UNKNOWN || t.end_us == HM_TIME_UNKNOWN) {
        t.start_us = HM_TIME_UNKNOWN;
        t.end_us = HM_TIME_UNKNOWN;
        return t;
    }
    t.end_us += (hm_time_us)llround(r->fit.slope_us_per_index);
    return t;
}
 ...
 */
static hm_history_status gather_status(const hm_request *r)
{
    if (!r->have_indices) {
        return HM_HIST_SHORT;
    }
    if (hm_coverage_indices_in(&r->cov, r->indices) >= index_width(r->indices)) {
        return HM_HIST_COMPLETE;
    }
    return (r->cov.count <= 1u) ? HM_HIST_SHORT : HM_HIST_HOLED;
}

/*
 * ⚠ THE ONE PLACE HM_CAL_LOST IS EVER WRITTEN (sample.h, history.h, design
 ...
 */
static void fill_calibration_span(const hm_session *s, hm_time_range span,
                                  hm_calibration_span *out)
{
    ...
    if (!have_transition || span.start_us == HM_TIME_UNKNOWN) {
        return;
    }
    ...
    if (s->cal_changed_us >= span.end_us) {
        ...
        if (s->cal_prev_state == HM_CAL_CALIBRATED && s->cal_state != HM_CAL_CALIBRATED) {
            out->state_at_end = (uint8_t)HM_CAL_LOST;
            out->spans_transition = 1u;
        }
    }
}
 ...
 * returns an even one-in-eight.  Replaying `swings.hmwire` through this gather
 ...
 * reporting HM_HISTORY_DEPTH_SEED_US — a figure measured once, on somebody
 ...
 * what a refill chases and when HM_WARN_HISTORY_HOLED fires:
 *
 ...
 */
static void history_learn_depth(hm_session *s, const hm_request *r, hm_history_status status)
{
     ...
     * rules out six of them, which is what HM_HIST_EVICTED already asserts — so
     ...
     */
    if (status == HM_HIST_EVICTED && !r->cov.has_bounds) {
        bound = (hm_time_us)llround(
    ...
    bound = (hm_time_us)llround((double)(r->head_at_ask - oldest) * slope);
     ...
     * cancelled a pull.
     */
    if (status != HM_HIST_COMPLETE && status != HM_HIST_HOLED && status != HM_HIST_SHORT) {
        return;
    }
    /*
     ...
     * stream — silently shrinking hm_history_resident_range() and able to fire
     * HM_WARN_HISTORY_DEPTH_CONFLICT.  §10.1 measured 4,182 records with every
    ...
    bound = (hm_time_us)llround((double)(r->head_at_ask - r->recorded.first) * slope);
    if (!s->have_depth_hi || bound < s->depth_hi_us) {
        s->depth_hi_us = bound;
        s->have_depth_hi = true;
    }
    ...
    if (s->have_depth_lo && s->depth_hi_us <= s->depth_lo_us && !s->depth_conflict_warned) {
        ...
        warn_now(s, HM_WARN_HISTORY_DEPTH_CONFLICT,
 ...
 * nothing (design §8.2).
 */
static void history_finish(hm_session *s, hm_request *r, hm_history_status status)
{
    hm_history_record *rec;
    hm_layout          layout;
    hm_sample         *samples;
    hm_time_range     *delivered;
    hm_index_range    *delivered_idx;
    hm_gap            *gaps;
    ...
    size_t             gap_count = 0u;
    hm_index_range     window = r->have_indices ? r->indices : r->recorded;
    hm_index_range     served = r->have_indices ? r->recorded : r->indices;
    void              *mem;
    ...
    if (!r->have_indices) {
         ...
         * handed back a HM_GAP_NOT_DELIVERED over [0,0] and a
         * HM_GAP_NOT_RECORDED over [1, window.last], anchored below the request
    ...
    layout.base = NULL;
    layout.used = 0u;
    (void)layout_take(&layout, sizeof(hm_history_record), 16u);
    (void)layout_take(&layout, r->sample_count * sizeof(hm_sample), 16u);
    (void)layout_take(&layout, n_delivered * sizeof(hm_time_range), 16u);
    (void)layout_take(&layout, n_delivered * sizeof(hm_index_range), 16u);
    (void)layout_take(&layout, gap_cap * sizeof(hm_gap), 16u);

    mem = s->alloc.alloc(s->alloc.ctx, layout.used);
    if (mem == NULL) {
        /* ⚠ Still terminal, and still reported.  collect() returns
         * HM_ERR_NO_MEMORY rather than HM_PENDING forever, because a request
         * that goes quiet is the one failure a consumer cannot act on. */
        r->alloc_failed = true;
        r->block = NULL;
        r->state = HM_GATHER_READY;
        hm_coverage_init(&r->cov, NULL, 0u);
        history_progress_event(s, r, HM_EV_HISTORY_READY);
        history_arm(s);
        history_service(s);
        return;
    }
    ...
    layout.used = 0u;
    rec = layout_take(&layout, sizeof(hm_history_record), 16u);
    samples = layout_take(&layout, r->sample_count * sizeof(hm_sample), 16u);
    delivered = layout_take(&layout, n_delivered * sizeof(hm_time_range), 16u);
    delivered_idx = layout_take(&layout, n_delivered * sizeof(hm_index_range), 16u);
    gaps = layout_take(&layout, gap_cap * sizeof(hm_gap), 16u);

    rec->magic = HISTORY_BLOCK_MAGIC;
    rec->alloc = s->alloc; /* ⚠ a COPY: this is what lets the block outlive us */
    rec->bytes = layout.used;

    rec->pub.layout_version = HM_SAMPLE_LAYOUT_VERSION;
    rec->pub.sample_stride = (uint32_t)sizeof(hm_sample);
     ...
     */
    if (r->have_indices && (r->fit.flags & (uint32_t)HM_CLOCK_HAS_FIT) != 0u &&
        ...
        gaps[gap_count].span.start_us = HM_TIME_UNKNOWN;
        gaps[gap_count].span.end_us = HM_TIME_UNKNOWN;
        gaps[gap_count].kind = (uint8_t)HM_GAP_FIT_BLIND;
        gap_count++;
    }
    if (served.first > window.first) {
        ...
        gaps[gap_count].kind = (uint8_t)HM_GAP_NOT_RECORDED;
        gap_count++;
    }
    if (r->have_indices && index_width(served) > 0u) {
        ...
        for (size_t i = 0; i < r->cov.count && gap_count + 1u < gap_cap; ++i) {
            ...
            if ((uint64_t)lo > cursor) {
                ...
                gaps[gap_count].kind = (uint8_t)HM_GAP_NOT_DELIVERED;
                gap_count++;
            }
    ...
    rec->pub.coverage_fraction = r->have_indices ? hm_coverage_fraction(&r->cov, window) : 0.0;
    ...
    rec->pub.density = hm_sample_step_density(samples, rec->pub.sample_count);
    {
        uint32_t largest = r->have_indices ? hm_coverage_largest_gap(&r->cov, window) : 0u;
    ...
    if (status == HM_HIST_HOLED) {
         ...
         * TAKEN, which is a different fact and already an HM_GAP_NOT_RECORDED —
        ...
        uint32_t undelivered = hm_coverage_largest_gap(&r->cov, served);
        if (undelivered >= HISTORY_ADAPTIVE_STEP_MAX) {
            warn_now(s, HM_WARN_HISTORY_HOLED, (int32_t)undelivered,
                     rec->pub.coverage_fraction);
        }
    }
    if (status == HM_HIST_SHORT && r->have_indices) {
        uint64_t missing = index_width(window) - hm_coverage_indices_in(&r->cov, window);
        if (missing > 0u) {
            warn_now(s, HM_WARN_HISTORY_SHORT,
    ...
    r->block = rec;
    r->state = HM_GATHER_READY;
    hm_coverage_init(&r->cov, NULL, 0u); /* the shared storage belongs to the next request */
...
{
    if (s->active == slot_of(s, r)) {
        if (r->state == HM_GATHER_REQUESTED) {
            /*
            ...
            arm(s, HM_DUE_BRACKET_LIMIT, s->now_us + BRACKET_LIMIT_US);
        }
        s->bracket_owned = false;
        s->active = -1;
    }
...
static void history_attempt(hm_session *s, hm_request *r)
{
    hm_write_request w;

    if (hm_cmd_history(&w, (uint16_t)(r->ask.first & 0xffffu),
                       (uint16_t)(r->ask.last & 0xffffu)) < HM_OK) {
        /* §7.1 requires first < last, and hm_cmd_history() refuses anything
         * else — so a mis-split fails loudly here rather than returning a
         * dense, complete and entirely wrong block. */
        history_finish(s, r, HM_HIST_ERROR);
        return;
    }
    queue_write(s, &w);
    /*
    ...
    r->attempts++;
    r->state = HM_GATHER_REQUESTED;
    s->active = slot_of(s, r);
    history_arm(s);
}

static void history_issue(hm_session *s, hm_request *r)
{
    hm_index_range range;
    hm_index_range recorded;
    ...
    if (s->stream != HM_STREAM_RUNNING || !s->have_index) {
        history_finish(s, r, HM_HIST_NO_STREAM);
        return;
    }
    if (hm_stream_config_is_legacy(s->active_cfg)) {
        /* §6.3.1: a 0x7f record has no header, so `a1` addresses a counter that
         * does not exist. */
        history_finish(s, r, HM_HIST_NOT_ALIGNABLE);
        return;
    }
    if (hm_clock_index_range_for_time(&r->fit, r->req.window, &range) < HM_OK) {
        history_finish(s, r, HM_HIST_ERROR);
        return;
    }
    r->indices = range;
    ...
    if (r->req.alignment_budget_us != 0u) {
        ...
        if (!hm_clock_meets_budget(&r->fit, range.first, r->req.alignment_budget_us) ||
            !hm_clock_meets_budget(&r->fit, range.last, r->req.alignment_budget_us)) {
            history_finish(s, r, HM_HIST_REFUSED_ALIGNMENT);
            return;
        }
    }
     ...
     * than its tail.  What falls outside becomes HM_GAP_NOT_RECORDED on the
    ...
    if (recorded.first == recorded.last) {
        ...
        history_finish(s, r, HM_HIST_SHORT);
        return;
    }
     ...
     * the error is not confined to the timestamps: `hm_clock_index_range_for_time`
     ...
     * such a window cannot be served whatever we do, and HM_HIST_ERROR says we
     * will not guess rather than returning a third of it as though it were all.
     * hm_history_reserve()'s gather-area check refuses it first in every default
    ...
    hm_coverage_init(&r->cov, s->cov_storage, s->cov_cap);
    {
        hm_index_range head;
        hm_index_range rest;
...
static void history_capture_fits(hm_session *s)
{
    ...
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        hm_request *r = &s->reqs[i];
        if (r->state != HM_GATHER_QUEUED || r->have_fit) {
            continue;
        }
        ...
        if (!have) {
            hm_fit_snapshot(&s->fit, s->now_us, &snap);
            have = true;
        }
        if ((snap.flags & (uint32_t)HM_CLOCK_HAS_FIT) == 0u) {
 ...
 * nothing.  hm_history_resident_range() below is a CLAIM OF RESIDENCY, so it
 ...
 */
static hm_time_us history_depth_estimate_us(const hm_session *s)
{
    hm_time_us depth = HM_HISTORY_DEPTH_SEED_US;
...
static size_t history_queue_ahead(const hm_session *s, uint64_t before_id,
                                  const hm_request **out)
{
    size_t n = 0u;

    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        const hm_request *q = &s->reqs[i];
        size_t            k;
        if (q->state == HM_GATHER_FREE || q->state == HM_GATHER_READY) {
            continue;
        }
        if (q->id >= before_id) {
            continue;
 ...
 */
static void history_eviction_check(hm_session *s, hm_time_us *out_next)
{
    hm_time_us depth = history_depth_estimate_us(s);

    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        const hm_request *ahead[HISTORY_MAX_PENDING];
        hm_request       *r = &s->reqs[i];
        hm_time_us        slack = 0;
        hm_time_us        fixed = 0;
        hm_time_us        turn_at;
        hm_time_us        eviction_at;
        size_t            n;

        /* Only a QUEUED request can be overtaken by eviction; one in flight is
         * being served, and one that is READY has been. */
        if (r->state != HM_GATHER_QUEUED) {
            continue;
        }

        n = history_queue_ahead(s, r->id, ahead);
        for (size_t k = 0; k < n; ++k) {
            hm_time_us width = ahead[k]->req.window.end_us - ahead[k]->req.window.start_us;
        ...
        if (eviction_at < turn_at) {
            ...
            memset(&ev, 0, sizeof(ev));
            ev.type = (uint16_t)HM_EV_HISTORY_EVICTION_RISK;
         ...
         */
        {
            hm_time_us crossing = eviction_at - slack + 1;
...
static void history_arm(hm_session *s)
{
    hm_time_us start = HM_TIME_NEVER;
    hm_time_us deadline = HM_TIME_NEVER;
    hm_time_us eviction = HM_TIME_NEVER;

    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        const hm_request *r = &s->reqs[i];
        if (r->state == HM_GATHER_FREE || r->state == HM_GATHER_READY) {
            continue;
        }
        ...
        if (r->state == HM_GATHER_QUEUED && r->req.window.end_us > s->now_us &&
    ...
    if (start == HM_TIME_NEVER) {
        disarm(s, HM_DUE_HISTORY_START);
    } else {
        arm(s, HM_DUE_HISTORY_START, start);
    }
    if (deadline == HM_TIME_NEVER) {
        disarm(s, HM_DUE_HISTORY_DEADLINE);
    } else {
        arm(s, HM_DUE_HISTORY_DEADLINE, deadline);
    }
    ...
    if (eviction == HM_TIME_NEVER) {
        disarm(s, HM_DUE_HISTORY_EVICTION);
    } else {
        arm(s, HM_DUE_HISTORY_EVICTION, eviction);
    }
}

static void history_service(hm_session *s)
{
    hm_request *pick = NULL;
    ...
    if (s->active >= 0) {
         ...
         * let hm_calibration_begin() through and this line would then open a new
         ...
         */
        hm_request *a = &s->reqs[s->active];
        if (a->state == HM_GATHER_REFILL && !s->bracket_open && !s->pull_needs_live &&
            s->stream == HM_STREAM_RUNNING && !cal_in_progress(s)) {
            history_attempt(s, a);
        }
        return;
    }
     ...
     * forget.  Every hm_calibration_* call already refuses with HM_ERR_BUSY
...
static void history_bracket_opened(hm_session *s)
{
    ...
    hm_fit_set_blind(&s->fit, true);

    if (s->active >= 0 && s->reqs[s->active].state == HM_GATHER_REQUESTED) {
        r = &s->reqs[s->active];
    }
    ...
    r->tick[HM_UNIT_LOWER_ARM] = s->tick[HM_UNIT_LOWER_ARM];
    r->tick[HM_UNIT_PALM] = s->tick[HM_UNIT_PALM];
    ...
    history_progress_event(s, r, HM_EV_HISTORY_STARTED);
}

/* An attempt has ended.  Either the holes are worth another `a1`, or the block
 * is what it is. */
static bool pick_refill_gap(const hm_request *r, hm_index_range *out)
{
     ...
     * that (§7.1) — hm_cmd_history() would refuse the inverted re-wrapped pair
     * and take the whole request down with HM_HIST_ERROR.  Chase the near turn
...
static void history_bracket_closed(hm_session *s)
{
    ...
    if (r->out_of_range > r->out_of_range_reported) {
        warn_now(s, HM_WARN_HISTORY_OUT_OF_RANGE,
    ...
    if (r->req.refill_gaps && r->attempts < budget && s->now_us < r->req.deadline_us &&
        hm_coverage_indices_in(&r->cov, r->indices) < index_width(r->indices) &&
 ...
 */
static void on_frame(hm_session *s, hm_decoded *dec, hm_time_us host_recv_us)
{
    ...
    if (s->stream == HM_STREAM_STARTING) {
        ...
        s->stream = HM_STREAM_RUNNING;
        s->stream_started_us = host_recv_us;
        disarm(s, HM_DUE_STREAM_START);
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_STREAM_STARTED;
        ev.u.stream.stream_id = s->stream_id;
        ev.u.stream.config_bits = s->active_cfg.bits;
        event_push(s, &ev);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        hm_sample *sample = &dec->u.frame.sample[i];
        derive_from_frame(s, sample);
        if ((sample->flags & (uint16_t)HM_SAMPLE_INDEX_MISSING) == 0u) {
            /* ⚠ THE ONE hm_fit_observe() CALL SITE.  Live frames only: history
             * arrival timestamps carry no information at all (§10, §10.1). */
            hm_fit_observe(&s->fit, sample->sample_index, host_recv_us);
        }
    }
    ...
    s->have_live = true;
    arm(s, HM_DUE_LIVE_GAP, host_recv_us + s->policy.live_gap_alarm_us);
    if (s->due[HM_DUE_CLOCK_EVENT] == HM_TIME_NEVER) {
        arm(s, HM_DUE_CLOCK_EVENT, s->now_us + s->policy.clock_event_period_us);
    }
...
/* Message dispatch                                                          */
/* ------------------------------------------------------------------------ */
static void emit_codec_warnings(hm_session *s, uint32_t warnings)
{
    for (int i = 0; i < (int)HM_WARN_CODE_COUNT; ++i) {
        if ((warnings & (1u << (unsigned)i)) != 0u) {
            warn_rate(s, (hm_warning_code)i);
        }
    }
}

static void on_message(hm_session *s, hm_decoded *dec, hm_time_us host_recv_us)
{
    ...
    switch (dec->kind) {
        ...
        case HM_MSGK_STATUS: {
            ...
            s->info.valid |= (uint32_t)HM_INFO_BATTERY;
            memset(&ev, 0, sizeof(ev));
        ...
        case HM_MSGK_MAC: {
            ...
            s->info.valid |= (uint32_t)HM_INFO_MAC;
            memset(&ev, 0, sizeof(ev));
        ...
        case HM_MSGK_SERIAL: {
            ...
            s->info.valid |= (uint32_t)HM_INFO_SERIAL;
            memset(&ev, 0, sizeof(ev));
            ev.type = (uint16_t)HM_EV_IDENTITY;
            ev.u.device_info = s->info;
            event_push(s, &ev);
        ...
        case HM_MSGK_CALIBRATION_RESULT:
            ...
            if (s->cal_phase != HM_CALP_APPLYING) {
                 ...
                 * consumer either way. */
                warn_now(s, HM_WARN_CALIBRATION_UNSOLICITED, (int32_t)s->cal_phase, 0.0);
                break;
            }
            cal_set_phase(s, HM_CALP_VERIFYING, HM_CAL_ABORT_NONE);
            break;
        ...
        case HM_MSGK_CAL_ACK:
            ...
            if (s->cal_phase == HM_CALP_MARKING_POSE0) {
                cal_set_phase(s, HM_CALP_OBSERVING_RAISE, HM_CAL_ABORT_NONE);
            } else if (s->cal_phase == HM_CALP_MARKING_POSE1) {
                cal_set_phase(s, HM_CALP_APPLYING, HM_CAL_ABORT_NONE);
            }
            break;
        ...
        case HM_MSGK_DEVICE_ERROR: {
            ...
            memset(&ev, 0, sizeof(ev));
            ev.type = (uint16_t)HM_EV_DEVICE_ERROR;
            ...
            if (r != NULL) {
                 ...
                 */
                s->active = -1;
                history_finish(s, r, HM_HIST_EVICTED);
            }
            break;
        }
...
/* Lifecycle                                                                 */
/* ------------------------------------------------------------------------ */
static hm_time_us pick(hm_time_us configured, hm_time_us fallback)
{
    return (configured > 0) ? configured : fallback;
}

static void resolve_policy(hm_session_policy *p)
{
    const hm_session_policy d = hm_session_policy_default();

    /*
     * ⚠ 0 means "use the default" for every field here — the opposite of
     * hm_clock_correction, where a flagged 0 is a MEASURED zero.  One is a
    ...
    if (p->keepalive_period_us > (hm_time_us)60 * 1000 * 1000) {
        p->keepalive_period_us = (hm_time_us)60 * 1000 * 1000;
    }
...
hm_status hm_session_create(const hm_session_config *config, hm_session **out_session)
{
    ...
    *out_session = NULL;
    ...
    cfg = (config != NULL) ? *config : hm_session_config_default();
     ...
     */
    live_cap = cap_or(cfg.memory.live_ring_capacity, HM_LIVE_RING_RECOMMENDED);
    event_cap = cap_or(cfg.memory.event_ring_capacity, HM_EVENT_RING_RECOMMENDED);
    wire_cap = cfg.memory.wire_ring_capacity;         /* ⚠ 0 → OFF */
    digest_cap = cfg.memory.digest_ring_capacity;     /* ⚠ 0 → OFF */
    gather_cap = cap_or(cfg.memory.history_gather_capacity, HM_HISTORY_GATHER_RECOMMENDED);
    coverage_cap = cap_or(cfg.memory.coverage_capacity, HM_COVERAGE_RECOMMENDED);

    /* Pass 1: measure. */
    layout.base = NULL;
    layout.used = 0u;
    (void)layout_take(&layout, sizeof(hm_session), 16u);
    (void)layout_take(&layout, WRITE_QUEUE_CAPACITY * sizeof(hm_write_request), 16u);
    if (cfg.memory.live_ring == NULL) {
        (void)layout_take(&layout, live_cap * sizeof(hm_sample), 16u);
    }
    if (cfg.memory.event_ring == NULL) {
        (void)layout_take(&layout, event_cap * sizeof(hm_event), 16u);
    }
    if (cfg.memory.wire_ring == NULL) {
        (void)layout_take(&layout, wire_cap * sizeof(hm_wire_chunk), 16u);
    }
    if (cfg.memory.history_gather == NULL) {
        (void)layout_take(&layout, gather_cap * sizeof(hm_sample), 16u);
    }
    if (cfg.memory.coverage_storage == NULL) {
        (void)layout_take(&layout, coverage_cap * sizeof(hm_index_range), 16u);
    }
    if (cfg.memory.digest_ring == NULL) {
        (void)layout_take(&layout, digest_cap * sizeof(hm_live_digest), 16u);
    }
    ...
    layout.used = 0u;
    s = layout_take(&layout, sizeof(hm_session), 16u);
    s->owned = block;
    s->alloc = alloc;

    {
        void *writes = layout_take(&layout, WRITE_QUEUE_CAPACITY * sizeof(hm_write_request), 16u);
        ...
        if (live == NULL) {
            live = layout_take(&layout, live_cap * sizeof(hm_sample), 16u);
        }
        if (events == NULL) {
            events = layout_take(&layout, event_cap * sizeof(hm_event), 16u);
        }
        if (wire == NULL) {
            wire = layout_take(&layout, wire_cap * sizeof(hm_wire_chunk), 16u);
        }
        if (gather == NULL) {
            gather = layout_take(&layout, gather_cap * sizeof(hm_sample), 16u);
        }
        if (coverage == NULL) {
            coverage = layout_take(&layout, coverage_cap * sizeof(hm_index_range), 16u);
        }
        if (digest == NULL) {
            digest = layout_take(&layout, digest_cap * sizeof(hm_live_digest), 16u);
        }

        ring_init(&s->writes, writes, WRITE_QUEUE_CAPACITY, sizeof(hm_write_request));
        ring_init(&s->live, live, live_cap, sizeof(hm_sample));
        ring_init(&s->events, events, event_cap, sizeof(hm_event));
        ring_init(&s->wire, wire, wire_cap, sizeof(hm_wire_chunk));
        hm_overlap_init(&s->overlap, (hm_live_digest *)digest, digest_cap);
        ...
        s->gather = (hm_sample *)gather;
        s->gather_cap = (gather != NULL) ? gather_cap : 0u;
        s->cov_storage = (hm_index_range *)coverage;
    ...
    /*
     * ⚠ `hm_session_config c = {0}` leaves stream_config.bits at 0x00, which is
     ...
     * hm_stream_config_nonstandard() always writes one, so an entirely empty
    ...
    hm_fit_init(&s->fit, s->policy.accuracy_drift_us_per_s);
    ...
    s->cal_state = HM_CAL_UNCALIBRATED;
    s->cal_prev_state = HM_CAL_UNCALIBRATED;
    s->cal_phase = HM_CALP_IDLE;
    s->cal_phase_prev = HM_CALP_IDLE;
    s->cal_abort_reason = HM_CAL_ABORT_NONE;

    *out_session = s;
    return HM_OK;
}

void hm_session_close(hm_session *session)
{
     ...
     * the consumer.  Each materialises with HM_HIST_CANCELLED and whatever had
     * arrived, and stays collectable until destroy() — a capture should record
     * what it got even when the answer is nothing. */
    history_abandon_all(session, HM_HIST_CANCELLED);

    session->closed = true;
    session->link = HM_LINK_CLOSED;
     ...
     * exactly as hm_history_collect() already works after close, and for the
     * same reason.  A capture should record what it got.
     */
}

void hm_session_destroy(hm_session *session)
{
    hm_allocator alloc;
    void        *block;
    ...
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        if (session->reqs[i].block != NULL) {
            hm_history_block_release(&session->reqs[i].block->pub);
...
/* Transport inputs                                                          */
/* ------------------------------------------------------------------------ */
void hm_session_on_link_up(hm_session *session, int32_t negotiated_mtu, hm_time_us now_us)
{
    static const struct {
        hm_status (*encode)(hm_write_request *);
    } k_bringup[] = {
        { hm_cmd_versions },   /* 80 — ⚠ the only REQUIRED step (§9.1)          */
        { hm_cmd_status },     /* 81                                            */
        { hm_cmd_sensor_map }, /* 84                                            */
        { hm_cmd_status },     /* 81 again, as the vendor app does              */
        { hm_cmd_serial },     /* 86 — the vendor app sends this THREE times    */
        { hm_cmd_mac },        /* 85                                            */
    };
    ...
    if (session->link != HM_LINK_DOWN) {
        return;
    }

    session->mtu_rejected = false;

    if (negotiated_mtu > 0 && negotiated_mtu < HM_MIN_ATT_MTU) {
        /*
        ...
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_MTU_REJECTED;
    ...
    if (negotiated_mtu == 0) {
        ...
        warn_now(session, HM_WARN_MTU_UNKNOWN, 0, 0.0);
    }

    session->link = HM_LINK_BRINGUP;
    ...
    set_calibration_state(session, HM_CAL_UNCALIBRATED);

    wire_meta(session, "link_up");
    event_emit(session, HM_EV_LINK_UP);

    for (size_t i = 0; i < sizeof(k_bringup) / sizeof(k_bringup[0]); ++i) {
        queue_command(session, k_bringup[i].encode);
    }
    arm(session, HM_DUE_BRINGUP, session->now_us + session->policy.bringup_timeout_us);
}

void hm_session_on_link_down(hm_session *session, hm_link_down_cause cause, hm_time_us now_us)
{
    if (session == NULL || session->closed) {
        return;
    }
...
void hm_session_on_bytes(hm_session *session, const uint8_t *data, size_t length,
                         hm_time_us host_recv_us)
{
    ...
    redact = !session->policy.record_identifiers &&
             (data[0] == (uint8_t)HM_MSG_ID_MAC || data[0] == (uint8_t)HM_MSG_ID_SERIAL);
    wire_log(session, HM_WIRE_DEVICE_TO_HOST, data, length, redact);

    st = hm_codec_decode(data, length, session->active_cfg, &dec);
    if (st == HM_ERR_UNKNOWN_MESSAGE) {
        /* §5.1: logged and ignored, never an error condition for the session. */
        hm_event ev;
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_UNKNOWN_MESSAGE;
    ...
    if (st == HM_ERR_TRUNCATED) {
        warn_rate(session, HM_WARN_SHORT_FRAME);
        return;
    }
    if (st < HM_OK) {
        return;
...
void hm_session_on_advertising_seen(hm_session *session, hm_time_us now_us)
{
    if (session == NULL || session->closed) {
        return;
    }
...
/* The core's clock                                                          */
/* ------------------------------------------------------------------------ */
hm_time_us hm_session_next_due_us(const hm_session *session)
{
    hm_time_us best = HM_TIME_NEVER;

    if (session == NULL || session->closed) {
        return HM_TIME_NEVER;
    }
    /*
...
void hm_session_tick(hm_session *session, hm_time_us now_us)
{
    int firings = 0;
    ...
    while (firings < TICK_MAX_FIRINGS) {
        ...
        disarm(session, (hm_due_id)which);
...
/* Outputs                                                                   */
/* ------------------------------------------------------------------------ */
size_t hm_session_poll_writes(hm_session *session, hm_write_request *out, size_t max)
{
    size_t n;
    ...
    if (n > 0u) {
         ...
         * HM_WARN_KEEPALIVE_LATE, which is exactly the failure it has.
         */
        session->last_host_write_us = session->now_us;
        if (session->link == HM_LINK_READY) {
            arm(session, HM_DUE_KEEPALIVE, session->now_us + session->policy.keepalive_period_us);
            arm(session, HM_DUE_KEEPALIVE_ALARM,
                session->now_us + session->policy.keepalive_alarm_us);
        }
        for (size_t i = 0; i < n; ++i) {
            wire_log(session, HM_WIRE_HOST_TO_DEVICE, out[i].data, out[i].length, false);
        }
    }
    return n;
}

size_t hm_session_poll_events(hm_session *session, hm_event *out, size_t max)
{
    /* ⚠ NO `closed` GUARD, unlike the polls below.  Closing MATERIALISES every
     * outstanding reservation and pushes its HM_EV_HISTORY_READY, so refusing to
...
size_t hm_session_poll_live(hm_session *session, hm_sample *out, size_t max)
{
    if (session == NULL || out == NULL || max == 0u || session->closed) {
        return 0u;
    }
...
size_t hm_session_poll_wire(hm_session *session, hm_wire_chunk *out, size_t max)
{
    if (session == NULL || out == NULL || max == 0u || session->closed) {
        return 0u;
    }
...
uint64_t hm_session_dropped_live(const hm_session *session)
{
    return (session != NULL) ? session->live.dropped : 0u;
}

uint64_t hm_session_dropped_events(const hm_session *session)
{
    return (session != NULL) ? session->events.dropped : 0u;
}

uint64_t hm_session_dropped_wire(const hm_session *session)
{
...
/* Session control                                                           */
/* ------------------------------------------------------------------------ */
hm_status hm_session_start_stream(hm_session *session)
{
    hm_write_request req;
    hm_status        st;
    ...
    if (session->link != HM_LINK_READY) {
        return HM_ERR_LINK_DOWN;
    }
    if (session->stream != HM_STREAM_STOPPED) {
    ...
    st = hm_cmd_start_stream(&req, session->requested_cfg);
    if (st < HM_OK) {
        return st;
    }
    ...
    hm_fit_begin_stream(&session->fit, session->stream_id);
    stream_reset_decode(session);
    session->clock_degraded = false;

    session->stream = HM_STREAM_STARTING;
    queue_write(session, &req);
    arm(session, HM_DUE_STREAM_START, session->now_us + session->policy.stream_start_timeout_us);
    ...
    if (session->stream_starts > 1u) {
        ...
        hm_event ev;
        memset(&ev, 0, sizeof(ev));
        ev.type = (uint16_t)HM_EV_STREAM_RESTARTED;
         ...
         */
        if (session->cal_state == HM_CAL_CALIBRATED) {
            set_calibration_state(session, HM_CAL_UNKNOWN);
        }
        session->cal_have_anchor = false;
    }
    if (hm_stream_config_is_legacy(session->active_cfg)) {
        warn_now(session, HM_WARN_LEGACY_STREAM, 0, 0.0);
    } else if (!hm_stream_config_is_observed_default(session->active_cfg)) {
        warn_now(session, HM_WARN_NONSTANDARD_CONFIG, (int32_t)session->active_cfg.bits, 0.0);
    }
    return HM_OK;
}

hm_status hm_session_stop_stream(hm_session *session)
{
    hm_write_request req;
    ...
    if (session->stream == HM_STREAM_STOPPED || session->stream == HM_STREAM_STOPPING) {
        return HM_ERR_INVALID_STATE;
    }
    if (hm_cmd_stop_stream(&req) < HM_OK) {
        return HM_ERR_INVALID_STATE;
    }
     ...
     */
    session->stream = HM_STREAM_STOPPING;
    history_abandon_all(session, HM_HIST_CANCELLED);
    ...
    arm(session, HM_DUE_STREAM_STOP,
        session->now_us + session->policy.stream_start_timeout_us);
    return HM_OK;
}

bool hm_session_is_streaming(const hm_session *session)
{
    return (session != NULL) && session->stream == HM_STREAM_RUNNING;
}

uint64_t hm_session_stream_id(const hm_session *session)
{
    return (session != NULL) ? session->stream_id : 0u;
}

hm_status hm_session_power_off(hm_session *session)
{
    hm_write_request req;
    ...
    if (session->link != HM_LINK_READY && session->link != HM_LINK_BRINGUP) {
        return HM_ERR_LINK_DOWN;
    }
    if (hm_cmd_power_off(&req) < HM_OK) {
        return HM_ERR_INVALID_STATE;
    }
    ...
    history_abandon_all(session, HM_HIST_CANCELLED);
    close_bracket(session);
    stop_stream_locally(session, session->stream != HM_STREAM_STOPPED);
     ...
     * failure or retry into it.  HM_POWER_OFF_LINGER_US is 12 s: deliberate
     * margin over a single 9 s measurement, not a transcription of it.
     */
    disarm(session, HM_DUE_KEEPALIVE);
    disarm(session, HM_DUE_KEEPALIVE_ALARM);
    arm(session, HM_DUE_POWER_OFF_LINGER, session->now_us + HM_POWER_OFF_LINGER_US);
    return HM_OK;
}
...
/* Queries                                                                   */
/* ------------------------------------------------------------------------ */
hm_status hm_session_clock(const hm_session *session, hm_clock_snapshot *out)
{
    hm_session *mutable_session = (hm_session *)(uintptr_t)session;
    ...
    hm_fit_snapshot(&mutable_session->fit, session->now_us, out);
    /* ⚠ `out` is filled either way, so a caller that ignores the status still
     * gets a snapshot whose flags say it carries no fit. */
    return ((out->flags & (uint32_t)HM_CLOCK_HAS_FIT) != 0u) ? HM_OK : HM_ERR_NO_FIT;
}

hm_status hm_session_device_info(const hm_session *session, hm_device_info *out)
{
    if (session == NULL || out == NULL) {
        return HM_ERR_INVALID_ARG;
    }
...
hm_stream_config hm_session_stream_config(const hm_session *session)
{
    if (session == NULL) {
        return hm_stream_config_default();
    }
    return session->active_cfg;
}

hm_status hm_session_set_clock_correction(hm_session *session,
                                          const hm_clock_correction *correction)
{
    if (session == NULL || correction == NULL) {
        return HM_ERR_INVALID_ARG;
    }
    /* ⚠ A zero `fields` is rejected rather than obeyed or ignored — hm_fit
     * enforces it, and clock.h has the three failure shapes that makes
     * unreachable. */
    return hm_fit_set_correction(&session->fit, correction);
}
 ...
 * answer they give:
 *
 *   bracket open  → HM_ERR_BUSY       a retrieval suspends live delivery
 ...
 *   link not up   → HM_ERR_LINK_DOWN
 *   stream not up → HM_ERR_NO_STREAM  ⚠ §8.2: the device observes a CONTINUOUS
 ...
 *   wrong phase   → HM_ERR_INVALID_STATE
 *
 * ⚠ hm_calibration_abort() is exempt from all of it: it writes nothing, and a
 * UI must always be able to cancel a routine it has given up on.
 */
static hm_status cal_guard(const hm_session *session)
{
    if (session == NULL || session->closed) {
        return HM_ERR_INVALID_STATE;
    }
    ...
    if (session->stream != HM_STREAM_RUNNING) {
        return HM_ERR_NO_STREAM;
    }
    return HM_OK;
}

hm_status hm_calibration_begin(hm_session *session)
{
    hm_status st = cal_guard(session);
    if (st < HM_OK) {
        return st;
    }
     ...
     * HM_EV_READY and stays open, so a wait would never be entered, and an
    ...
    cal_set_phase(session, HM_CALP_AWAIT_HORIZONTAL, HM_CAL_ABORT_NONE);
    return HM_OK;
}

hm_status hm_calibration_confirm_horizontal(hm_session *session)
{
    hm_status st = cal_guard(session);
    if (st < HM_OK) {
        return st;
    }
    if (session->cal_phase != HM_CALP_AWAIT_HORIZONTAL) {
        return HM_ERR_INVALID_STATE;
    }
    st = queue_command_arg(session, hm_cmd_calibration_marker, 0u);
    if (st < HM_OK) {
        return st; /* the phase does not move without its marker */
    }
    cal_set_phase(session, HM_CALP_MARKING_POSE0, HM_CAL_ABORT_NONE);
    return HM_OK;
}

hm_status hm_calibration_confirm_raise(hm_session *session)
{
    hm_status st = cal_guard(session);
    if (st < HM_OK) {
        return st;
    }
    if (session->cal_phase != HM_CALP_OBSERVING_RAISE) {
        return HM_ERR_INVALID_STATE;
    }
    st = queue_command_arg(session, hm_cmd_calibration_marker, 1u);
    if (st < HM_OK) {
        return st;
    }
    cal_set_phase(session, HM_CALP_MARKING_POSE1, HM_CAL_ABORT_NONE);
    return HM_OK;
}

hm_status hm_calibration_confirm_reference_pose(hm_session *session)
{
    hm_status st = cal_guard(session);
    if (st < HM_OK) {
        return st;
    }
    if (session->cal_phase != HM_CALP_VERIFYING) {
        return HM_ERR_INVALID_STATE;
    }
    if (session->cal_run_active) {
        return HM_ERR_BUSY; /* one run at a time; the pose has not changed */
    }

    /*
     * ⚠ THE MEASUREMENT DOES NOT EXIST YET.  This starts a run over the next
     * live samples and returns; HM_EV_CALIBRATION_PRESENCE carries the answer,
     * or HM_WARN_PRESENCE_NOT_MEASURED says none could be taken.  Measuring
    ...
    session->cal_run_count = 0u;
    arm(session, HM_DUE_CAL_PRESENCE, session->now_us + PRESENCE_RUN_WINDOW_US);
    return HM_OK;
}

hm_status hm_calibration_abort(hm_session *session)
{
    ...
    if (!cal_in_progress(session)) {
        return HM_ERR_INVALID_STATE; /* nothing to abort */
    }
    /*
     ...
     * HM_CAL_ABORT_CALLER — with the angle NaN and the state still UNKNOWN,
    ...
    if (session->cal_phase == HM_CALP_VERIFYING) {
        ...
        cal_set_phase(session, HM_CALP_COMPLETE, HM_CAL_ABORT_CALLER);
        return HM_OK;
    }
    cal_abort(session, HM_CAL_ABORT_CALLER);
    return HM_OK;
}

hm_calibration_phase hm_calibration_current_phase(const hm_session *session)
{
    return (session != NULL) ? session->cal_phase : HM_CALP_IDLE;
}

hm_calibration_state hm_session_calibration_state(const hm_session *session)
{
    return (session != NULL) ? session->cal_state : HM_CAL_UNKNOWN;
}

float hm_calibration_presence_angle_deg(const hm_session *session)
{
...
hm_status hm_calibration_reference_anchor(const hm_session *session,
...
/* History — phase 4                                                         */
/* ------------------------------------------------------------------------ */
static hm_request *history_find(hm_session *s, uint64_t id)
{
    if (id == 0u) {
        return NULL;
    }
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        if (s->reqs[i].state != HM_GATHER_FREE && s->reqs[i].id == id) {
            return &s->reqs[i];
        }
    }
    return NULL;
 ...
 */
hm_status hm_history_reserve(struct hm_session *session, const hm_history_request *request,
                             uint64_t *out_request_id)
{
    hm_clock_snapshot snap;
    hm_request       *slot = NULL;
    double            span_indices;
    ...
    *out_request_id = 0u;
    ...
    if (request->window.start_us >= request->window.end_us) {
        return HM_ERR_INVALID_ARG; /* empty or inverted */
    }
    ...
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        if (session->reqs[i].state == HM_GATHER_FREE) {
            slot = &session->reqs[i];
            break;
        }
    }
     ...
     * which means the fit check below would refuse it with HM_ERR_NO_FIT and
     ...
     * is immediately collectable carrying HM_HIST_NOT_ALIGNABLE.
     */
    if (hm_stream_config_is_legacy(session->active_cfg)) {
        memset(slot, 0, sizeof(*slot));
        slot->state = HM_GATHER_QUEUED;
        slot->id = session->next_request_id++;
        slot->req = *request;
        slot->reserved_us = session->now_us;
        hm_coverage_init(&slot->cov, NULL, 0u);
        *out_request_id = slot->id;
        history_finish(session, slot, HM_HIST_NOT_ALIGNABLE);
        return HM_OK;
    }
     ...
     *
     * The fit is only reset at hm_session_start_stream(), never at a stop, so
     * HM_CLOCK_HAS_FIT survives one — and a reservation made after the stream
     * ended sailed past every check here, then sat QUEUED while
     * history_service() returned silently at its `stream != RUNNING` line on
     * every pass.  history_issue(), the only producer of HM_HIST_NO_STREAM, was
     * never reached, so the request waited out its whole deadline and
     * materialised HM_HIST_TIMED_OUT — the exact "silent four-second timeout"
     ...
     * refused a few lines below with HM_ERR_NO_FIT if no frame ever arrives.
     */
    if (session->stream != HM_STREAM_RUNNING && session->stream != HM_STREAM_STARTING) {
        memset(slot, 0, sizeof(*slot));
        slot->state = HM_GATHER_QUEUED;
        slot->id = session->next_request_id++;
        slot->req = *request;
        ...
        hm_coverage_init(&slot->cov, NULL, 0u);
        *out_request_id = slot->id;
        history_finish(session, slot, HM_HIST_NO_STREAM);
        return HM_OK;
    }
     ...
     */
    hm_fit_snapshot(&session->fit, session->now_us, &snap);
    if ((snap.flags & (uint32_t)HM_CLOCK_HAS_FIT) == 0u || !(snap.slope_us_per_index > 0.0)) {
    ...
    if (span_indices < 1.0) {
        return HM_ERR_INVALID_ARG; /* narrower than one sample period */
    }
...
hm_status hm_history_collect(struct hm_session *session, uint64_t request_id,
                             hm_history_block **out_block)
{
    hm_request *r;
    ...
    *out_block = NULL;
    /* ⚠ No `closed` guard: blocks stay collectable after hm_session_close() and
     * until hm_session_destroy(), which releases whatever was never taken. */
    r = history_find(session, request_id);
    if (r == NULL) {
        return HM_ERR_INVALID_ARG; /* unknown, or already collected */
    }
    if (r->state != HM_GATHER_READY) {
        return HM_PENDING; /* never blocks */
    }
...
hm_status hm_history_cancel(struct hm_session *session, uint64_t request_id)
{
    hm_request *r;
    ...
    if (r->state == HM_GATHER_READY) {
        return HM_ERR_INVALID_STATE; /* it has already finished; collect it */
    }
    /* ⚠ The block still materialises, with whatever arrived and
     * HM_HIST_CANCELLED, so a capture records what it got. */
    history_abandon(session, r, HM_HIST_CANCELLED);
    return HM_OK;
}

void hm_history_block_release(hm_history_block *block)
{
    hm_history_record *rec;
    hm_allocator       alloc;
    ...
    rec = (hm_history_record *)(void *)block;
    /*
...
size_t hm_history_pending(const struct hm_session *session)
{
    size_t n = 0u;
    ...
    for (int i = 0; i < HISTORY_MAX_PENDING; ++i) {
        if (session->reqs[i].state != HM_GATHER_FREE &&
            session->reqs[i].state != HM_GATHER_READY) {
            n++;
        }
    }
    return n;
 ...
 * HM_HIST_EVICTED block and knows.  So the width reported is `depth_lo` — the
 * widest reach-back the DEVICE ACTUALLY SERVED this connection — and never the
 * estimate the eviction warning runs on.
 *
 * ⚠ HM_PENDING IS NOT A FAILURE AND IT IS NOT AN EMPTY ANSWER.  It means "the
 ...
 * enough to move it to HM_OK.
 */
hm_status hm_history_resident_range(const struct hm_session *session, hm_time_range *out_range)
{
    hm_clock_snapshot snap;
    hm_time_us        depth;
    hm_time_us        head_us;
    ...
    if (session->stream != HM_STREAM_RUNNING || !session->have_index) {
        return HM_ERR_NO_STREAM;
    }
    ...
    hm_fit_snapshot(&mutable_session->fit, session->now_us, &snap);
    if ((snap.flags & (uint32_t)HM_CLOCK_HAS_FIT) == 0u || !(snap.slope_us_per_index > 0.0)) {
        return HM_ERR_NO_FIT;
    }
    head_us = hm_clock_to_host_us(&snap, session->head_index);
    if (head_us == HM_TIME_UNKNOWN) {
        return HM_ERR_NO_FIT;
    }

    depth = session->have_depth_lo ? session->depth_lo_us : HM_HISTORY_DEPTH_SEED_US;
    ...
    return session->have_depth_lo ? HM_OK : HM_PENDING;
}
 ...
 * from hm_history_resident_range()'s HM_OK — the measured case.  Before the
 ...
 */
bool hm_history_coverage_available(const struct hm_session *session, hm_time_us start_us,
                                   hm_time_us end_us)
{
    hm_time_range resident;

    if (session == NULL || start_us >= end_us) {
        return false;
    }
    if (hm_history_resident_range(session, &resident) != HM_OK) {
        return false;
    }
    /* `resident` is half-open at the head like every other hm_time_range here,
