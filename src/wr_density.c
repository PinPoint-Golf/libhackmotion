...
/*
 * wr_density.c — ACHIEVED DENSITY, and it is a SPACING rather than a RATE.
 *
 ...
 * shape that exhausts `wr_coverage`'s caller-provided storage — and on overflow
 ...
 * stays exact.
 *
 * ⚠ `wr_report`'s `history_modal_step` (record.h) is a neighbour, not a copy:
 ...
 */
double wr_sample_step_density(const wr_sample *samples, size_t count)
{
    uint32_t hist[WR_DENSITY_STEP_MAX + 1u];
    uint64_t seen = 0u;
    uint64_t steps;
    uint64_t target;
    size_t   i;
    ...
    for (i = 1u; i < count; ++i) {
        ...
        if ((samples[i].flags & (uint16_t)WR_SAMPLE_INDEX_MISSING) != 0u ||
            (samples[i - 1u].flags & (uint16_t)WR_SAMPLE_INDEX_MISSING) != 0u ||
            samples[i].sample_index <= samples[i - 1u].sample_index) {
            return 0.0;
        }
        step = (uint64_t)samples[i].sample_index - (uint64_t)samples[i - 1u].sample_index;
        if (step <= (uint64_t)WR_DENSITY_STEP_MAX) {
            hist[step]++;
        }
    ...
    for (uint64_t step = 1u; step <= (uint64_t)WR_DENSITY_STEP_MAX; ++step) {
        seen += hist[step];
        if (seen > target) {
            return 1.0 / (double)step;
        }
    }
    /* More than half the gaps are wider than WR_DENSITY_STEP_MAX — fewer than
