 ...
 *    None of them has a path to the wire — hm_command_emit() is the only gate,
...
bool hm_command_is_allowed(uint8_t command_byte)
{
...
size_t hm_command_allowlist(uint8_t *out, size_t max)
{
    if (out != NULL) {
...
{
    if (out == NULL || bytes == NULL || length == 0u || length > HM_MAX_COMMAND_LEN) {
        return HM_ERR_INVALID_ARG;
    }
    if (!hm_command_is_allowed(bytes[0])) {
        return HM_ERR_NOT_ALLOWED;
    }
...
static hm_status emit1(hm_write_request *out, uint8_t id)
{
    uint8_t b[1];
    b[0] = id;
    return hm_command_emit(out, b, sizeof(b), false);
}

hm_status hm_cmd_versions(hm_write_request *out)   { return emit1(out, HM_CMD_VERSIONS); }
hm_status hm_cmd_status(hm_write_request *out)     { return emit1(out, HM_CMD_STATUS); }
hm_status hm_cmd_sensor_map(hm_write_request *out) { return emit1(out, HM_CMD_SENSOR_MAP); }
hm_status hm_cmd_mac(hm_write_request *out)        { return emit1(out, HM_CMD_MAC); }
hm_status hm_cmd_serial(hm_write_request *out)     { return emit1(out, HM_CMD_SERIAL); }
hm_status hm_cmd_stop_stream(hm_write_request *out){ return emit1(out, HM_CMD_STOP_STREAM); }
...
hm_status hm_cmd_power_off(hm_write_request *out)
{
    ...
    return emit1(out, HM_CMD_POWER_OFF);
}

hm_status hm_cmd_start_stream(hm_write_request *out, hm_stream_config cfg)
{
    if (hm_stream_config_is_legacy(cfg)) {
        /* The legacy start takes no configuration byte at all. */
        return emit1(out, HM_CMD_LEGACY_START);
    }
    {
        uint8_t b[3];
        b[0] = HM_CMD_START_STREAM;
        b[1] = 0x01u;
        b[2] = cfg.bits;
        return hm_command_emit(out, b, sizeof(b), false);
    }
...
hm_status hm_cmd_calibration_marker(hm_write_request *out, uint8_t pose)
{
    uint8_t b[2];
    if (pose > 1u) {
        return HM_ERR_INVALID_ARG;
    }
    b[0] = HM_CMD_CALIBRATE;
    b[1] = pose;
    return hm_command_emit(out, b, sizeof(b), false);
}
