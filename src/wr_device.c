...
const wr_uuid WR_UUID_OTA_SERVICE_FORBIDDEN = {
...
const wr_uuid WR_UUID_ISSC_PIPE_INERT = {
...
wr_status wr_uuid_parse(const char *text, wr_uuid *out)
{
...
char *wr_uuid_format(const wr_uuid *uuid, char *out, size_t out_size)
{
    if (out == NULL || out_size == 0) {
        return out;
    }
    if (uuid == NULL || out_size < WR_UUID_STRING_SIZE) {
        out[0] = '\0';
        return out;
    }
...
bool wr_uuid_equal(const wr_uuid *a, const wr_uuid *b)
{
    if (a == NULL || b == NULL) {
        return false;
    }
...
bool wr_looks_like_sensor(const char *local_name, const wr_uuid *advertised_services,
                              size_t service_count)
{
     ...
     */
    if (local_name != NULL &&
        strncmp(local_name, WR_ADVERTISED_NAME_PREFIX, sizeof(WR_ADVERTISED_NAME_PREFIX) - 1u) ==
            0) {
        return true;
    }
    if (advertised_services != NULL) {
        for (size_t i = 0; i < service_count; ++i) {
            if (wr_uuid_equal(&advertised_services[i], &WR_UUID_TRANSPARENT_UART_SERVICE)) {
                return true;
            }
        }
    }
