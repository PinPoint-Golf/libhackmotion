...
/*
 * wr_codec.h — internal.  Wire bytes in, decoded messages out.
 *
...
typedef enum wr_msg_id {
    WR_MSG_ID_LEGACY_FRAME  = 0x7f,
    WR_MSG_ID_VERSIONS      = 0x80,
    WR_MSG_ID_STATUS        = 0x81,
    WR_MSG_ID_STREAM_STARTED= 0x82,
    WR_MSG_ID_STREAM_STOPPED= 0x83,
    WR_MSG_ID_SENSOR_MAP    = 0x84,
    WR_MSG_ID_MAC           = 0x85,
    WR_MSG_ID_SERIAL        = 0x86,
    WR_MSG_ID_FRAME         = 0x90,
    WR_MSG_ID_CALIBRATION   = 0x94,
    WR_MSG_ID_START_ACK     = 0xa0,
    WR_MSG_ID_HISTORY_MARK  = 0xa1,
    WR_MSG_ID_CAL_ACK       = 0xa2,
    WR_MSG_ID_DEVICE_ERROR  = 0xd0,
    WR_MSG_ID_BUTTON        = 0xfb
} wr_msg_id;

/* The `a1` payload byte discriminates the bracket.  Any other value is
 * rejected — the vendor app calls it an "unknown magic cookie". */
#define WR_HISTORY_MARK_END   0x01
#define WR_HISTORY_MARK_START 0x02

typedef enum wr_msg_kind {
    WR_MSGK_UNKNOWN = 0,
    WR_MSGK_FRAME,             /* 0x90 — one or two records                   */
    WR_MSGK_LEGACY_FRAME,      /* 0x7f — one record, no header, no ticks      */
    WR_MSGK_VERSIONS,
    WR_MSGK_STATUS,
    WR_MSGK_STREAM_STARTED,
    WR_MSGK_STREAM_STOPPED,
    WR_MSGK_SENSOR_MAP,
    WR_MSGK_MAC,
    WR_MSGK_SERIAL,
    WR_MSGK_CALIBRATION_RESULT,
    WR_MSGK_START_ACK,
    WR_MSGK_HISTORY_MARK,
    WR_MSGK_CAL_ACK,
    WR_MSGK_DEVICE_ERROR,
    WR_MSGK_BUTTON,
    WR_MSGK_IGNORED            /* in §5.1, nothing for a client to do with it */
} wr_msg_kind;
 ...
 * WR_WARN_UNEXPECTED_RECORD_COUNT, which says "never observed", not "illegal".
 *
 ...
 * They differ only above WR_CODEC_MAX_RECORDS, `consumed` always follows
 ...
 */
#define WR_CODEC_MAX_RECORDS 4u
#define WR_CODEC_SPEC_MAX_RECORDS 2u

typedef struct wr_decoded {
    wr_msg_kind kind;
    uint8_t     message_id;
    size_t      consumed;     /* bytes of the notification that were interpreted */
    uint32_t    warnings;     /* bitmask of (1u << wr_warning_code) */

    union {
        struct {
            size_t    count;         /* decoded — at most WR_CODEC_MAX_RECORDS */
            size_t    count_present; /* ⚠ what the LENGTH says; see above       */
            wr_sample sample[WR_CODEC_MAX_RECORDS]; /* raw + scaled, no host time */
        } frame;
        struct {
        ...
        struct {
             ...
             * WR_SENSOR_LOCATION_MAX codes.  They differ only above that, and
             * a consumer that compares them sees that they did.
             */
            uint8_t count;
            uint8_t location[WR_SENSOR_LOCATION_MAX];
        } sensor_map;
        struct {
            char text[20];           /* MAC (colon-formatted) or serial, NUL-terminated */
        } text;
        struct {
            uint8_t marker;          /* WR_HISTORY_MARK_START / _END */
            bool    valid;
        } history;
        struct {
 ...
 *  - WR_MIN_ATT_MTU is 96 precisely so the 93-byte maximum message fits in one
 ...
 * of records raises WR_WARN_TRAILING_BYTES, which is the signature of a
 * coalescing transport and is meant to be loud rather than absorbed.
 *
 * Returns WR_OK, WR_ERR_TRUNCATED if the notification is shorter than the
 * message its id implies, or WR_ERR_UNKNOWN_MESSAGE (logged and ignored per
 ...
 *
 * ⚠ `cfg` is by POINTER here and by VALUE on wr_codec_decode() above, and that
 * is a decision rather than an oversight.  wr_stream_config grew from 4 to 68
 ...
 * pull.  wr_codec_decode() runs once per notification, where the copy is
...
static inline uint16_t wr_be16(const uint8_t *p)
{
    return (uint16_t)(((uint16_t)p[0] << 8) | (uint16_t)p[1]);
}

static inline int16_t wr_be16s(const uint8_t *p)
{
    return (int16_t)wr_be16(p);
}
