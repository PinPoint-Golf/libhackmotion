...
/* Scaling                                                                   */
/* ------------------------------------------------------------------------ */
void wr_codec_scale_unit(wr_unit_sample *unit, const wr_stream_config *cfg)
{
    const float gyro_div = (float)wr_stream_config_gyro_divisor(*cfg);
    uint8_t mask = 0;

    for (int i = 0; i < 4; ++i) {
        unit->q_world_to_body[i] = (float)unit->q_world_to_body_raw[i] / WR_QUAT_SCALE;
    }
    for (int i = 0; i < 3; ++i) {
        int16_t a = unit->linear_accel_raw[i];
        int16_t g = unit->gyro_raw[i];
        unit->linear_accel_mps2[i] = (float)a * WR_ACCEL_LSB_MPS2;
         ...
         */
        if (a >= WR_PIN_THRESHOLD || a <= -WR_PIN_THRESHOLD) {
            mask |= (uint8_t)(1u << (unsigned)(WR_CH_ACCEL_X + i));
        }
        if (g >= WR_PIN_THRESHOLD || g <= -WR_PIN_THRESHOLD) {
            mask |= (uint8_t)(1u << (unsigned)(WR_CH_GYRO_X + i));
        }
    }
    unit->pinned_mask = mask;
    unit->device_time_us = WR_TIME_UNKNOWN;
}

/* Reads one block.  `block_size` is 22, 20 (0x90 layouts) or 21 (legacy). */
static void decode_block(const uint8_t *p, size_t block_size, const wr_stream_config *cfg,
                         wr_unit_sample *unit)
{
    memset(unit, 0, sizeof(*unit));

    for (int i = 0; i < 4; ++i) {
        unit->q_world_to_body_raw[i] = wr_be16s(p + 2 * i);
    }
    for (int i = 0; i < 3; ++i) {
        unit->linear_accel_raw[i] = wr_be16s(p + 8 + 2 * i);
        unit->gyro_raw[i] = wr_be16s(p + 14 + 2 * i);
    }

    if (block_size == 22u) {
        unit->ticks_raw = wr_be16(p + 20);
        unit->has_ticks = 1u;
    } else {
        unit->ticks_raw = 0u;
        unit->has_ticks = 0u;
    ...
    wr_codec_scale_unit(unit, cfg);
}

static bool quat_norm_ok(const wr_unit_sample *unit)
{
    float n = wr_quat_raw_norm(unit->q_world_to_body_raw);
    float d = n - WR_QUAT_NORM_NOMINAL;
    if (d < 0.0f) {
        d = -d;
    }
    return d <= WR_QUAT_NORM_TOLERANCE;
}

wr_status wr_codec_decode_record(const uint8_t *data, size_t length,
                                 const wr_stream_config *cfg, wr_sample *out)
{
    const size_t block = wr_stream_config_block_size(*cfg);
    const size_t record = wr_stream_config_record_size(*cfg);
    const bool legacy = wr_stream_config_is_legacy(*cfg);
    size_t offset;
    ...
    if (length < record) {
        return WR_ERR_TRUNCATED;
    }

    memset(out, 0, sizeof(*out));
    out->config_bits = cfg->bits;
    out->skew_us = 0;
    out->host_time_us = WR_TIME_UNKNOWN;
    out->host_recv_us = WR_TIME_UNKNOWN;
    ...
    if (legacy) {
        ...
        out->flags |= (uint16_t)(WR_SAMPLE_INDEX_MISSING | WR_SAMPLE_NOT_TIME_ALIGNABLE |
                                 WR_SAMPLE_TICKS_MISSING | WR_SAMPLE_NONSTANDARD_CONFIG);
    } else {
        out->sample_index_raw = wr_be16(data);
        offset = 2;
        if (!wr_stream_config_has_ticks(*cfg)) {
            out->flags |= (uint16_t)WR_SAMPLE_TICKS_MISSING;
        }
        if (!wr_stream_config_is_observed_default(*cfg)) {
            out->flags |= (uint16_t)WR_SAMPLE_NONSTANDARD_CONFIG;
        }
    }
    ...
    if (out->lower_arm.pinned_mask != 0u || out->palm.pinned_mask != 0u) {
        out->flags |= (uint16_t)WR_SAMPLE_PINNED;
    }
    if (!quat_norm_ok(&out->lower_arm) || !quat_norm_ok(&out->palm)) {
        out->flags |= (uint16_t)WR_SAMPLE_QUAT_NORM_SUSPECT;
    }
    return WR_OK;
}
...
{
    switch (id) {
        case WR_MSG_ID_VERSIONS:       return 7;
        case WR_MSG_ID_STATUS:         return 3;
        case WR_MSG_ID_STREAM_STARTED: return 1;
        case WR_MSG_ID_STREAM_STOPPED: return 1;
        case WR_MSG_ID_MAC:            return 12;
        case WR_MSG_ID_SERIAL:         return 9;
        case WR_MSG_ID_START_ACK:      return 1;
        case WR_MSG_ID_HISTORY_MARK:   return 1;
        case WR_MSG_ID_CAL_ACK:        return 1;
        case WR_MSG_ID_DEVICE_ERROR:   return 1;
        case WR_MSG_ID_BUTTON:         return 1;
        default:                       return -1;
    }
}
 ...
 * from the LENGTH, never from the content — see wr_codec.h for why a heuristic
 * would be a silent-corruption hazard rather than a convenience.
 */
static wr_status decode_frame(const uint8_t *data, size_t length, const wr_stream_config *cfg,
                              wr_decoded *out)
{
    const size_t record = wr_stream_config_record_size(*cfg);
    ...
    if (avail % record != 0u) {
        ...
        out->warnings |= WARN(WR_WARN_TRAILING_BYTES);
    }
    if (count > WR_CODEC_SPEC_MAX_RECORDS) {
        out->warnings |= WARN(WR_WARN_UNEXPECTED_RECORD_COUNT);
    }
    if (count > WR_CODEC_MAX_RECORDS) {
        count = WR_CODEC_MAX_RECORDS;
    }

    for (size_t i = 0; i < count; ++i) {
        wr_status st = wr_codec_decode_record(data + 1u + i * record, avail - i * record, cfg,
         ...
         */
        if ((out->u.frame.sample[i].flags & (uint16_t)WR_SAMPLE_QUAT_NORM_SUSPECT) != 0u) {
            out->warnings |= WARN(WR_WARN_QUAT_NORM);
        }
    }
     ...
     * WR_WARN_UNEXPECTED_RECORD_COUNT rather than instead of it: a warning says
 ...
 */
static wr_status decode_sensor_map(const uint8_t *data, size_t length, wr_decoded *out)
{
    size_t n = length - 1u;
    size_t i;
    ...
    out->kind = WR_MSGK_SENSOR_MAP;
    out->u.sensor_map.count = (uint8_t)((n > 255u) ? 255u : n);
    for (i = 0; i < n && i < WR_SENSOR_LOCATION_MAX; ++i) {
 ...
 */
static wr_status decode_calibration(const uint8_t *data, size_t length, wr_decoded *out)
{
    out->kind = WR_MSGK_CALIBRATION_RESULT;
    ...
    /*
     * ⚠ No WR_WARN_TRAILING_BYTES here even when the notification is longer
...
wr_status wr_codec_decode(const uint8_t *data, size_t length, wr_stream_config cfg,
                          wr_decoded *out)
{
    uint8_t id;
    int fixed;
    ...
    if (fixed < 0) {
         ...
         */
        out->kind = WR_MSGK_UNKNOWN;
        out->consumed = length;
        return WR_ERR_UNKNOWN_MESSAGE;
    }
    ...
    switch (id) {
        case WR_MSG_ID_VERSIONS:
            out->kind = WR_MSGK_VERSIONS;
        ...
        case WR_MSG_ID_STATUS:
            out->kind = WR_MSGK_STATUS;
            out->u.status.percent = data[1];
            out->u.status.undecoded = wr_be16(data + 2); /* NOT millivolts */
            break;

        case WR_MSG_ID_STREAM_STARTED:
            out->kind = WR_MSGK_STREAM_STARTED;
            break;

        case WR_MSG_ID_STREAM_STOPPED:
            out->kind = WR_MSGK_STREAM_STOPPED;
            break;

        case WR_MSG_ID_MAC:
            out->kind = WR_MSGK_MAC;
            format_mac(out->u.text.text, sizeof(out->u.text.text), data + 1);
            break;

        case WR_MSG_ID_SERIAL:
            out->kind = WR_MSGK_SERIAL;
            copy_text(out->u.text.text, sizeof(out->u.text.text), data + 1, 9u);
            break;

        case WR_MSG_ID_START_ACK:
            /* ⚠ The app ignores this and so must a client: stream startup is
             * keyed off the first 0x90 frame, not off an acknowledgement. */
            out->kind = WR_MSGK_START_ACK;
            break;

        case WR_MSG_ID_HISTORY_MARK:
            out->kind = WR_MSGK_HISTORY_MARK;
            out->u.history.marker = data[1];
            out->u.history.valid =
                (data[1] == WR_HISTORY_MARK_START || data[1] == WR_HISTORY_MARK_END);
            break;

        case WR_MSG_ID_CAL_ACK:
            out->kind = WR_MSGK_CAL_ACK;
            break;

        case WR_MSG_ID_DEVICE_ERROR:
            out->kind = WR_MSGK_DEVICE_ERROR;
            out->u.device_error.code = data[1];
            break;

        case WR_MSG_ID_BUTTON:
            out->kind = WR_MSGK_BUTTON;
            out->u.button.payload = data[1];
            break;

        default:
            out->kind = WR_MSGK_IGNORED;
            break;
    }
    return WR_OK;
}
